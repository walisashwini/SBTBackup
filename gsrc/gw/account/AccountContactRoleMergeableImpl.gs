...
class AccountContactRoleMergeableImpl extends AbstractMergeableImpl<AccountContactRole> {
  ...
  override function mergeFields(merged : entity.AccountContactRole) : boolean {
    ...
    pcrTable.compare(PolicyContactRole#AccountContactRole, Equals, merged)
      ...
      allEffDates.eachWithIndex(\ sliceDate, idx -> {
        if (period.Locked) {
          ...
          mergedChildrenForSliceDate.each(\ pcr -> {
            ...
            unslicedPCR.setFieldValue(PolicyContactRole.ACCOUNTCONTACTROLE_PROP.get(), Survivor)
          })
        } else {
          ...
          mergeChildren(Survivor, merged, \ acr -> {
            ...
            pcr.setFieldValueForChunk(PolicyContactRole#AccountContactRole.PropertyInfo.Name, acr, sliceDate, nextEventDate)
          })
        }
      })
    })
  ...
  private function getPeriodsReferencingContactFromDB(oldContact : Contact) : IQueryBeanResult<PolicyPeriod> {
    ...
    query.compare(PolicyPeriod#PNIContactDenorm, Equals, oldContact)
    query.compare(PolicyPeriod#ArchiveState, NotEquals, null)
    return query.select()
  }
}

...
@Export
class FormPatternValidation extends PCValidationBase implements IFormPatternValidation {
  ...
  function noSpacesInFormCode() {
    ...
    if (_formPattern.Code.matches(".*\\s+.*")) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.CodeHasSpaces", _formPattern.DisplayName, _formPattern.Code))
    }
  }
  ...
  function formCodeUnique() {
    Context.addToVisited(this, "formCodeUnique")
      ...
      q.compare(FormPattern#Code, Equals, _formPattern.Code)
      for (var f in q.select()) {
        if (f.ID != _formPattern.ID) {
          Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.CodeInUse", _formPattern.DisplayName, _formPattern.Code))
          return
        }
      }
  }
  ...
  function productsNotEmpty() {
    ...
    if (_formPattern.FormPatternProducts.IsEmpty) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.EmptyProducts", _formPattern.DisplayName))
    }
  }
  ...
  function noDuplicateProducts() {
    ...
    for (var formPatternProduct in _formPattern.FormPatternProducts) {
      if (seenProducts.contains(formPatternProduct.ProductCode)) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.ProductDefinedMoreThanOnce", _formPattern.DisplayName,formPatternProduct.ProductCode))
      }
  ...
  function jobsNotEmpty() {
    ...
    if (_formPattern.FormPatternJobs.IsEmpty) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.EmptyJobs", _formPattern.DisplayName))
    }
  }
  ...
  function noDuplicateJobs() {
    ...
    for (var formPatternJob in _formPattern.FormPatternJobs) {
      if (seenJobs.contains(formPatternJob.JobType)) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.JobDefinedMoreThanOnce", _formPattern.DisplayName,formPatternJob.JobType.Code))
      }
  ...
  function allJobsAreOfValidType() {
    ...
    var allowedJobTypes = (typekey.Job.Type as ITypeList).getTypeKeysByFilterName("FormPatternJobs")
    for (formPatternJob in _formPattern.FormPatternJobs) {
      if (!allowedJobTypes.contains(formPatternJob.JobType)) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.InvalidJobType", _formPattern.DisplayName, formPatternJob.JobType))
      }
    }
  }
  ...
  function useInsteadFormExists() {
    ...
    if (_formPattern.UseInsteadOfCode != null) {
      ...
      if (useInsteadForm == null) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.UseInsteadFormDoesNotExist", _formPattern.DisplayName,_formPattern.UseInsteadOfCode))
      }
    }
  }
    ...
    if (_formPattern.UseInsteadOfCode != null) {
      ...
      if (useInsteadForm != null) {
        if (_formPattern.InternalGroupCode == null) {
          Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.UseInsteadFormRequiresGroup", _formPattern.DisplayName))
        } else if (useInsteadForm.InternalGroupCode == null) {
          Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.UseInsteadFormInNoGroup", _formPattern.DisplayName,useInsteadForm.DisplayName))
        } else if (_formPattern.InternalGroupCode != useInsteadForm.InternalGroupCode) {
          Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.UseInsteadFormInDifferentGroup", _formPattern.DisplayName,useInsteadForm.DisplayName, useInsteadForm.GroupCode))
        }
      }
    }
  }
  ...
  function noUseInsteadCycle() {
    ...
    while (f.UseInsteadOfCode != null) {
      if (visited.contains(f.UseInsteadOfCode)) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.UseInsteadFormCycle", _formPattern.DisplayName, f.DisplayName, _formPattern.GroupCode))
        return
      }
  ...
  function formNumberDoesNotOverlapOtherGroupCode() {
    ...
    if (_formPattern.FormNumber != _formPattern.InternalGroupCode) {
      var q = Query.make(FormPattern)
        q.compare(FormPattern#InternalGroupCode, Equals, _formPattern.FormNumber)
          .join(FormPatternProduct#FormPattern)
          .compareIn(FormPatternProduct#ProductCode, _formPattern.FormPatternProducts*.ProductCode)
      if (not q.select().Empty) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.FormNumberOverlapsOtherGroupCode", _formPattern.DisplayName,_formPattern.FormNumber))
      }
    }
  }
  ...
  function groupCodeIsUnique() {
    ...
    if (_formPattern.InternalGroupCode != null) {
      var q = Query.make(FormPattern)
      q.compare(FormPattern#FormNumber, Equals, _formPattern.InternalGroupCode)
      q.or(\ or1 -> {
        or1.compare(FormPattern#InternalGroupCode, NotEquals, _formPattern.InternalGroupCode)
        or1.compare(FormPattern#InternalGroupCode, Equals, null)
      })
      if (not q.select().Empty) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.NonUniqueGroupCode", _formPattern.DisplayName,_formPattern.GroupCode))
      }
    }
  }
  ...
  function inferenceTimesInGroupAreEqual() {
    ...
    if (_formPattern.GroupCode != null) {
      ...
      q.compare(FormPattern#InferenceTime, NotEquals, _formPattern.InferenceTime)

      if (not q.select().Empty) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.GroupInferenceTimesUnequal", _formPattern.DisplayName,_formPattern.GroupCode))
      }
    }
  }
  ...
  function removalEndorsementFlagsInGroupAreEqual() {
    ...
    if (_formPattern.GroupCode != null) {
      var q = _formPattern.makeGroupQuery(true)
        q.compare(FormPattern#InternalRemovalEndorsement, NotEquals, _formPattern.InternalRemovalEndorsement)
        .join(FormPatternProduct#FormPattern)
        .compareIn(FormPatternProduct#ProductCode, _formPattern.FormPatternProducts*.ProductCode)

      if (not q.select().Empty) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.RemovalEndorsementFlagsUnequal", _formPattern.DisplayName,_formPattern.GroupCode))
      }
    }
  }
  ...
  function reissueOnChangeFlagsInGroupAreEqual() {
    ...
    if (_formPattern.GroupCode != null) {
      var q = _formPattern.makeGroupQuery(true)
        q.compare(FormPattern#InternalReissueOnChange, NotEquals, _formPattern.InternalReissueOnChange)
        .join(FormPatternProduct#FormPattern)
        .compareIn(FormPatternProduct#ProductCode, _formPattern.FormPatternProducts*.ProductCode)

      if (not q.select().Empty) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.ReissueOnChangeFlagsUnequal", _formPattern.DisplayName,_formPattern.GroupCode))
      }
    }
  }
    ...
    for (var lookup in _formPattern.Lookups index i) {
      if (lookup.StartEffectiveDate != null and lookup.EndEffectiveDate != null) {
        if (not lookup.EndEffectiveDate.after(lookup.StartEffectiveDate)) {
          Result.addError(_formPattern, TC_DEFAULT,
            DisplayKey.get("Validation.FormPattern.InvalidLookupDateRange", _formPattern.DisplayName, lookup.StartEffectiveDate.ShortFormat, lookup.EndEffectiveDate.ShortFormat, i + 1))
        }
      }
    }
  }
      ...
      for (var thisLookup in grouping) {
        if (prevLookup != null) {
          if (prevLookup.EndEffectiveDate == null or thisLookup.StartEffectiveDate == null or
            ...
            Result.addError(_formPattern, TC_DEFAULT,
              DisplayKey.get("Validation.FormPattern.OverlappingLookupDates", _formPattern.DisplayName, thisLookup.Jurisdiction, thisLookup.UWCompanyCode,
  ...
  function endorsementNumberedIfReissueOnChange() {
    ...
    if (_formPattern.ReissueOnChange) {
      if (_formPattern.RemovalEndorsementFormNumber != null and not _formPattern.EndorsementNumbered) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.RequiresEndorsementNumbering", _formPattern.DisplayName, _formPattern.RemovalEndorsementFormNumber))
      }
    }
  }
  ...
  function removalEndorsementValid() {
    ...
    if (_formPattern.InternalRemovalEndorsement and not _formPattern.hasJob(typekey.Job.TC_POLICYCHANGE))
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.RemovalEndorsementMustHavePolicyChange", _formPattern.DisplayName))
    if (not _formPattern.RemovalEndorsement) {
      ...
      if (referringForms.HasElements) {
        ...
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.RemovalEndorsementInUse", _formPattern.DisplayName, referringFormLabels.join(", ")))
      }
    }
  }
  ...
  function removalEndorsementFormNumberValid() {
    ...
    if (_formPattern.RemovalEndorsementFormNumber != null) {
      ...
      if (removalEndorsementForms.Empty) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.RemovalEndorsementDoesNotExist", _formPattern.DisplayName, _formPattern.RemovalEndorsementFormNumber))
      } else if (removalEndorsementForms.where(\ f -> f.PolicyLinePatternCode == _formPattern.PolicyLinePatternCode).Empty) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.NonMatchingPolicyLine", _formPattern.DisplayName))
      } else {
        for (var removalEndorsementForm in removalEndorsementForms) {
          if (not removalEndorsementForm.RemovalEndorsement) {
            Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.NonRemovalEndorsement", _formPattern.DisplayName, _formPattern.RemovalEndorsementFormNumber, removalEndorsementForm.DisplayName))
          }
        }
      }
    }
    ...
    if (_formPattern.RemovalEndorsementFormNumber != null) {
      ...
      for (var removalEndorsementForm in removalEndorsementForms) {
        ...
        if (not productCodesSet.equals(otherProductCodesSet)) {
          ...
          Result.addWarning(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.NonMatchingProducts", _formPattern.DisplayName, productCodesCsv, otherProductCodesCsv))
        }
      }
    }
  }
  ...
  function genericInferenceClassValid() {
    ...
    if (inferenceClassName == null) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.NullClassName", _formPattern.DisplayName))
      return
    }

    var inferenceClassType = TypeSystem.getByFullNameIfValid(inferenceClassName)
    if (inferenceClassType == null) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.InvalidClassName", _formPattern.DisplayName, inferenceClassName))
      return
    }

    if (not gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(gw.forms.GenericFormInference.Type, inferenceClassType)) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.Interface", _formPattern.DisplayName, inferenceClassName))
    }
    if (not gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(gw.forms.FormData.Type, inferenceClassType)) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.Inheritance", _formPattern.DisplayName, inferenceClassName))
    }
    if (inferenceClassType.Abstract) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.CannotBeAbstract", _formPattern.DisplayName, inferenceClassName))
    }
    if (inferenceClassType.TypeInfo.getConstructor({}) == null) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.RequiresNoArgConstructor", _formPattern.DisplayName, inferenceClassName))
    }
  }
  ...
  function policyLineExists() {
    ...
    if (_formPattern.PolicyLinePatternRef.Pattern == null) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.LineDoesNotExist", _formPattern.DisplayName, _formPattern.PolicyLinePatternCode))
    }
  }
  ...
  function allProductsContainSelectedPolicyLine() {
    ...
    if (invalidProducts.HasElements) {
      var productCodesCsv = invalidProducts.map(\ p -> p.PublicID).join(", ")
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.LineNotInProducts", _formPattern.DisplayName, _formPattern.PolicyLinePatternRef.DisplayName, productCodesCsv))
    }
  }
  ...
  function policyLinePatternContainsSelectedClausePattern(){
    ...
    if(_formPattern.ClausePattern.CoverageCategory.PolicyLinePattern.PublicID != _formPattern.PolicyLinePatternCode){
     Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.ClauseNotInPolicyLine", _formPattern.DisplayName, _formPattern.ClausePattern,_formPattern.PolicyLinePatternRef.DisplayName))
    }
  }
  ...
  function everyProductContainsSelectedClausePattern(){
    ...
    var clausePolicyLineCode = _formPattern.ClausePattern.CoverageCategory.PolicyLinePattern.PublicID
    var violatingProduct =_formPattern.Products.firstWhere( \p -> not p.ProductPolicyLinePatterns.map(\productLine -> productLine.PolicyLinePattern.PublicID).contains(clausePolicyLineCode))
    if(violatingProduct != null)
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.ClauseNotValidForAnyLineOnProduct", _formPattern.DisplayName, _formPattern.ClausePattern,violatingProduct.DisplayName))
  }
  ...
  function policyLinePatternIsValidForGenericInferenceClass(){
    ...
    var inferenceClassName = _formPattern.GenericInferenceClass
    if (inferenceClassName != null) {
      var inferenceClass = new FormInferenceClassRef(TypeSystem.getByFullNameIfValid(inferenceClassName) as Type<GenericFormInference>)
      if(not inferenceClass.ValidPolicyLines.map( \lp -> lp.PublicID).contains(_formPattern.PolicyLinePatternCode)) {
        Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.Inference.PolicyLinePatternIsNotValidForInferenceClass", _formPattern.DisplayName, _formPattern.PolicyLinePatternCode,inferenceClassName))
      }
    }
  }
  ...
  function allCoverablePropertiesAreValid() {
    ...
    var validCoverableProps = coverableType.TypeInfo.Properties.where(
        \ i -> i typeis IEntityPropertyInfo
                and not (i typeis ILinkPropertyInfo)
                and not (i typeis IArrayPropertyInfo)).map(\ i -> i.Name)
    ...
    if (invalidProperties.HasElements) {
      Result.addError(_formPattern, TC_DEFAULT, DisplayKey.get("Validation.FormPattern.InvalidCoverableProperties", _formPattern.DisplayName, _formPattern.ClausePattern.OwningEntityType, invalidProperties.join(", ")))
    }
  }
  ...
  private function findReferringRemovalEndorsementForms(formPattern : FormPattern) : Collection<FormPattern> {
    ...
    q.compare(FormPattern#RemovalEndorsementFormNumber, Equals, formPattern.FormNumber)
  ...
  private function findRemovalEndorsementForms(formPattern : FormPattern) : Collection<FormPattern> {
    var q = Query.make(entity.FormPattern)
    q.compare(FormPattern#FormNumber, Equals, formPattern.RemovalEndorsementFormNumber)
    var formPatterns = q.select().toSet()    

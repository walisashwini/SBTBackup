...
class PolicyHoldValidation extends PCValidationBase {
  ...
  function atLeastOneHoldRulePerHold(){
    ...
    if(_policyHold.Rules.IsEmpty){
      Result.addError(_policyHold,TC_DEFAULT, DisplayKey.get("Validation.PolicyHold.EmptyHoldRules"))
    }
  }
  ...
  function noDuplicateHoldRules(){
    ...
    for(rule in _policyHold.Rules){
      ...
      if(ruleList.Count < oldCount - 1){
        Result.addError(_policyHold, TC_DEFAULT, DisplayKey.get("Validation.PolicyHold.DuplicateHoldRuleFound"))
      }
    }
  }
  ...
  function noSpacesInPolicyHoldCode() {
    ...
    if (_policyHold.PolicyHoldCode.matches(".*\\s+.*")) {
      Result.addError(_policyHold, TC_DEFAULT, DisplayKey.get("Validation.PolicyHold.CodeHasSpaces", _policyHold.PolicyHoldCode))
    }
  }
  ...
  function policyHoldCodeUnique() {
    ...
    q.compare(PolicyHold#PolicyHoldCode, Equals, _policyHold.PolicyHoldCode)
    for (var f in q.select()) {
      if (f.ID != _policyHold.ID) {
        Result.addError(_policyHold, TC_DEFAULT, DisplayKey.get("Validation.PolicyHold.CodeInUse", _policyHold.PolicyHoldCode))
        return
      }
    }
  }
  ...
  function newHoldRegionUnique(region : PolicyHoldZone) {
    ...
    if (_policyHold.PolicyHoldZones.hasMatch(\ p -> p.Country.equals(region.Country) and
      ...
      Result.addError(region, TC_DEFAULT, DisplayKey.get("Validation.PolicyHold.RegionCodeInUse", region.ZoneType, region.Code))
    }
  }

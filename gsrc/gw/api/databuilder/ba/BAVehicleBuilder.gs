...
class BAVehicleBuilder extends DataBuilder<entity.BusinessVehicle, BAVehicleBuilder> {
  ...
  private function vehicleWithVin(line : BusinessAutoLine, vin : String) : BusinessVehicle {
    ...
    for (vehicle in line.Vehicles) {
      if (vehicle.Vin == vin) {
        if (ret != null) {
          throw new IllegalStateException(DisplayKey.get("Builder.BusinessVehicle.Error.DuplicateVin", vin))
        }
        ret = vehicle
      }
    }
  ...
  class EULocationPopulator implements BeanPopulator<BusinessVehicle> {
    ...
    override function execute(bean : BusinessVehicle) {
        ...
        if (value == null) {
            throw new RuntimeException(DisplayKey.get("Builder.BusinessVehicle.EULocationBuilder.Error.CreateNotCalled", _builder.Class.Name))
        }
        bean.Location = value
    }
  }
  ...
  function withBAVehicleAdditionalInterest(addInterestBuilder : BAVehicleAdditionalInterestBuilder) : BAVehicleBuilder {
    addPopulator(new BuilderArrayPopulator<BusinessVehicle>(BusinessVehicle.Type.TypeInfo.getProperty("AdditionalInterests") as IArrayPropertyInfo, addInterestBuilder))
    return this
  }

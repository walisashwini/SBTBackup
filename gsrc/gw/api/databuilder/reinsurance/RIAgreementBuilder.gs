...
abstract class RIAgreementBuilder<T extends RIAgreement, B extends RIAgreementBuilder> extends RIContractBuilder<T, B> {
  ...
  construct(t : IType) {
    ...
    addPopulator(MONETARY_AMOUNT_ORDER + 1, new BeanPopulator<T>() {
      override function execute(agreement : T) {
  ...
  final function withCoverageGroup(groupType : RICoverageGroupType) : B {
    if (gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(Facultative.Type, Type)) {
  ...
  final function withBroker(value : Contact) : B {
    set(RIAgreement.Type.TypeInfo.getProperty("Broker"), value)
    return this as B
  }
}
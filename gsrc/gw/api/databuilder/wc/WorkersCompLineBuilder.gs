...
class WorkersCompLineBuilder extends PolicyLineBuilder<entity.WorkersCompLine, WorkersCompLineBuilder> {
  ...
  function withPolicyOwnerOfficer(ownerOfficer : PolicyOwnerOfficerBuilder) : WorkersCompLineBuilder {
    addPopulator(new BuilderArrayPopulator<WorkersCompLine>(WorkersCompLine.Type.TypeInfo.getProperty("PolicyOwnerOfficers") as IArrayPropertyInfo, ownerOfficer))
    return this
  }

  function withPolicyLaborClient(policyLaborClient : PolicyLaborClientBuilder) : WorkersCompLineBuilder {
    addPopulator(new BuilderArrayPopulator<WorkersCompLine>(WorkersCompLine.Type.TypeInfo.getProperty("PolicyLaborClients") as IArrayPropertyInfo, policyLaborClient))
    return this
  }

  function withPolicyLaborContractor(policyLaborContractor : PolicyLaborContractorBuilder) : WorkersCompLineBuilder {
    addPopulator(new BuilderArrayPopulator<WorkersCompLine>(WorkersCompLine.Type.TypeInfo.getProperty("PolicyLaborContractors") as IArrayPropertyInfo, policyLaborContractor))
    return this
  }

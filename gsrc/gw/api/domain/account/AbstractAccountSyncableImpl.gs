...
abstract class AbstractAccountSyncableImpl<S extends AccountSyncable> implements AccountSyncable {
  ...
  /**
   * Most account syncables are editable when the policy period status is not "Quoting", "Rated", "Quoted',
   ...
   *
   * While "Quote/Rate Requested" are technically non-editable statuses as far as the UI is concerned, the syncable
   * write-through only happens when their respective work item is processed so for syncable field purposes we're still
   * in "editable" status
   */
  protected property get InEditablePolicyPeriodStatus() : boolean {
    return not NotEditablePeriodStatuses.contains(PolicyPeriod.Status)
  }
  
  protected property get ClosingOrClosedPolicyPeriodStatuses() : Set<PolicyPeriodStatus> {
    return PolicyPeriodStatusSets.CLOSING_OR_CLOSED_STATUSES
  }

  property get NotEditablePeriodStatuses() : Set<PolicyPeriodStatus> {
    return PolicyPeriodStatusSets.NOT_EDITABLE_POLICY_PERIOD_STATUSES
  }
  ...
  override function copyPolicyContractData() {
    if (SyncedToAccount) {
      ...
      throw new IllegalStateException(DisplayKey.get("Java.AccountSyncable.UnsyncedCopyContractDataError"))
    }
  }

...
enhancement ContactBillingEnhancement : entity.Contact {
  ...
  private property get BillingIntegationLogger() : Logger {
    return PCLoggerCategory.BILLING_SYSTEM_PLUGIN
  }
  ...
  function retryBillingEventMessages(){
    ...
    // messageQuery.compareIn(Message#Account, this.AccountContacts*.Account)
    messageQuery.compare(Message#DestinationID, Equals, BillingMessageTransport.DEST_ID)
    messageQuery.compare(Message#Status, Equals, MessageStatus.RETRYABLE_ERROR)
    messageQuery.compare(Message#ErrorCategory, Equals, ErrorCategory.TC_CONTACT_UNSYNCED)
    // add criteria to check that the message is blocked by THIS contact
    messageQuery.compare(Message#ErrorDescription, Equals, this.PublicID)

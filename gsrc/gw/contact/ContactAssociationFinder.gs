...
class ContactAssociationFinder {
  ...
  public function findPolicyPeriods() : List<PolicyPeriod> {
    ...
    PolicyPeriodQueryFilters.bound(pcrQuery)
    var policyContactRoleTable = pcrQuery.join(PolicyContactRole#BranchValue)
    policyContactRoleTable.compare(PolicyContactRole#ContactDenorm, Equals, _contact)
    ...
    PolicyPeriodQueryFilters.bound(pniContactDenormQuery)
    pniContactDenormQuery.compare(PolicyPeriod#PNIContactDenorm, Equals, _contact)
  ...
  public function createTopLevelAccountToRolesMap() : Map<String,String> {
    var accountToRoleListMap = Query.make(AccountContactRole)
                   .join(AccountContactRole#AccountContact)
                   .compare(AccountContact#Contact,Equals, _contact)
                   .select()
                   .toList()
                   .partition(\a -> a.AccountContact.Account)
    accountToRoleListMap.retainWhereKeys(\a -> {
      var currentUser : User = User.util.CurrentUser
      return currentUser.canView(a)
    })
  ...
  public function findWorkOrders(status : PolicyPeriodStatus) : IQueryBeanResult<Job> {
    ...
    policyPeriodTable.compare(PolicyPeriod#Locked, Relop.Equals, Boolean.FALSE)
    if (status != null) {
      policyPeriodTable.compare(PolicyPeriod#Status, Equals, status)
    }
  ...
  public function findWorkOrders(completedJobsOnly : Boolean, jobType : typekey.Job, product : Product, user : User) : Job[] {
    ...
    policyPeriodTable.compare(PolicyPeriod#ArchiveState, Equals, null)
    var policyContactRoleTable = policyPeriodTable.join(PolicyContactRole#BranchValue)
    policyContactRoleTable.compare(PolicyContactRole#ContactDenorm, Equals, _contact)
    ...
    pniPolicyPeriodTable.compare(PolicyPeriod#ArchiveState, NotEquals, null)
    pniPolicyPeriodTable.compare(PolicyPeriod#PNIContactDenorm, Equals, _contact)
    pniContactDenormQuery.withDistinct(true)
    ...
    return unionQuery.select().toList()?.toTypedArray()
  }

...
/**
 * A copier that handles copying {@link entity.Clause} types and their associated cov term fields and values.
...
class ClausePatternCopier extends Copier<Coverable>{
  ...
  override function copy(target : Coverable) {
    ...
    target.ReferenceDateInternal = _source.OwningCoverable.ReferenceDateInternal
  }
  ...
  function getCovTermValue(covTermString : String) : String {
    var covTerm = _source.getCovTerm(CovTermPatternLookup.getByPublicID(covTermString))

...
class CustomizableImportValidationMapper {
  ...
  private static function fieldValidationToString(issue : ValidationIssue) : String {
    ...
    buf.append(DisplayKey.get("Import.ValidationMapper.Column", fieldValidation.FieldName)).append("\n")
    buf.append(DisplayKey.get("Import.ValidationMapper.BadValue", fieldValidation.BadValue)).append("\n")
  ...
  private static function generalValidationToString(issue : ValidationIssue) : String {
    var buf = new StringBuilder()
    buf.append(DisplayKey.get("Import.ValidationMapper.IssueType", issue.Type)).append("\n")
    buf.append(DisplayKey.get("Import.ValidationMapper.IssueReason", issue.Reason)).append("\n")
    return buf.toString()    
  }  
}

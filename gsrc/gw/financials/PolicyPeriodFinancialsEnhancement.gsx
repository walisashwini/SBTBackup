...
enhancement PolicyPeriodFinancialsEnhancement : PolicyPeriod {
  ...
  * Start was defined by a previous period
  ...
  property get NonPremiumCosts() : Cost[] {
    return this.AllCosts.where(\c -> c.ChargePattern != ChargePattern.TC_PREMIUM)?.toTypedArray()
  }
  ...
  function calculateTransactions() {
    var logMsg = DisplayKey.get("PolicyPeriod.Quote.Transactions.Calculating")
    ...
    PCFinancialsLogger.logInfo(DisplayKey.get("PolicyPeriod.Quote.Transactions.Done", logMsg))
  }
  ...
  function determineJobType(subjectToReporting : boolean, whenWritten: boolean): JobType {
    switch(true) {
      ...
      case (this.Audit.AuditInformation.AuditScheduleType != TC_FINALAUDIT) :
  ...
  function loadPolicyFXRates() : Collection<PolicyFXRate> {
    return Query.make(PolicyFXRate)
        .compare(PolicyFXRate#PolicyPeriod, Equals, this).select().toCollection()
  }
  ...
  function findOrCreatePolicyFXRate(fromCurrency : Currency, toCurrency : Currency) : PolicyFXRate {
    ...
    return findOrCreatePolicyFXRate(rate, this.PolicyFXRates?.toList())
  }
    ...
    if (this.Audit != null) {
      ...
      if (info.IsPremiumReport) {
        ...
        if (outOfBoundsCosts.Count > 0) {
          throw new IllegalStateException(DisplayKey.get("Rating.CostDatesOutOfBounds", outOfBoundsCosts.join("\n")))
        }
      }
    }
  }

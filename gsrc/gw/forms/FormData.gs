 ...
 */
@Export
abstract class FormData implements IFormData {
  ...
  var _exposedStatesByDate : Map<Date, Set<Jurisdiction>> = {} // Map of slice date to the set of states exposed on the period for that slice
  ...
  protected function calculateExposedStates(context : FormInferenceContext) : Set<Jurisdiction> {
    if (Pattern.PolicyLinePatternCode != null) {
      ...
      return line != null ? context.getLineCoveredStates(line).union({line.BaseState}) : Collections.emptySet<Jurisdiction>()
    }
   ...
   * lookup.
   */
  function getLookupDates(context : FormInferenceContext) : Map<Jurisdiction, Date> {
   ...
   * on the type of form.
   */
  protected function getLookupDate(context : FormInferenceContext, state : Jurisdiction) : Date {
    if (Pattern.PolicyLinePatternCode != null) {
  ...
  static function verifyValidXMLElementName(elementName : String) {
    ...
    if (elementName.length >= 3 && 
        Character.toLowerCase(elementName.charAt(0)) == (Coercions.makePCharFrom("x")) &&
        Character.toLowerCase(elementName.charAt(1)) == (Coercions.makePCharFrom("m")) &&
        Character.toLowerCase(elementName.charAt(2)) == (Coercions.makePCharFrom("l"))) {
   ...
   */
  protected static reified function mapArrayToSet<I, O>(array : I[], pred : block(entry : I) : boolean, op : block(entry : I) : O) : Set<O> {
    if (pred == null) {
  ...
  /**
   * Determines if the form pattern associated with this FormData should be inferred for the given PolicyLine, based
   * only on the pattern's coverage part types. Returns true if no coverage part types are specified for the form
   * pattern, or if the given PolicyLine has at least one coverable whose coverage part type appears on the form
   * pattern's list of coverage part types.
   */
  protected function isInferredByCoveragePart(line : PolicyLine) : boolean {
    var formCoveragePartTypes = Pattern.CoveragePartTypes
    return formCoveragePartTypes.IsEmpty
        or formCoveragePartTypes.intersect(line.AllCoverables*.AssociatedCoveragePartTypes).Count > 0
  }
}

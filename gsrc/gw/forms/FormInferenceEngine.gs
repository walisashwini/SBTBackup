...
class FormInferenceEngine {
  ...
  function inferPostQuoteFormsInSlices(pr : PolicyPeriod, oosSliceDates_ : java.util.Date[]) {
    ...
    var formPatterns = findPotentialForms(pr, {TC_QUOTE,TC_BIND})
  ...
  function inferPreBindForms(pr : PolicyPeriod) {
    ...
    var formPatterns = findPotentialForms(pr, {TC_BIND})
   ...
   * job on the period. This just does a very coarse first-level pass by filtering only based on Jurisdiction,
   * any date or other state-based availability calculations have to be done on a per-slice, per-state,
   * per-pattern basis later on during the creation of the FormData objects.
   */
  private function findPotentialForms(pr : PolicyPeriod, inferenceTimes : FormInferenceTime[]) : List<FormPattern> {
    return PCProfilerTag.FORM_FIND_PATTERNS.evaluate(\ -> {
      // Get potential forms based on product, job, form inference time (quote-time or bind-time) and
      // policy line patterns. For specific policy line patterns, the form patterns are only included
      // if the PolicyPeriod has that policy line.
      var potentialForms = FormPatternUtil.findPotentialInferenceForms(pr, inferenceTimes)
      var policyPeriodJurisdictions = pr.AllCoveredStates.union({pr.BaseState})
      // Filter potential forms based on policy jurisdictions
      // Note: PossibleStates returns a list of possiblyAvailableStates from FormPatternCache.
      // For the states that do not have any lookup available, the cache adds all states to the list.
      return potentialForms.where(\ formPattern -> formPattern.PossibleStates.intersect(policyPeriodJurisdictions).HasElements)
    })
  }

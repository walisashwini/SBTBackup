...
class FullAppJobProcessAction implements JobProcessAction {
    ...
    try {
      ...
      if (!helper.hasWebMessageToRender()) {
        ...
        if (policyPeriod.Job.Complete) {
          ...
          saveDraftAndIgnoreValidation(true /* skipWidgetValidation */)
        }
  ...
  function saveDraftAndIgnoreValidation(skipWidgetValidation : boolean) {
    PCValidationContext.doWhileIgnoringPageLevelValidation( \ ->
        helper.Wizard.saveDraft(true /* should start editing */, skipWidgetValidation))
  }

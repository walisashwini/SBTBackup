...
class SideBySideCovTermInfo {
  ...
  /**
   * Open for edit the period associated to this cov term if it's RATED/QUOTED
   */
  function editAssociatedPeriodIfRatedOrQuoted() {
    CovInfo.PeriodInfo.Period.editIfRatedOrQuoted()
  }
  ...
  property get DateValue() : Date {
    return getValue<Date>()
  }

  property set DateValue(val : Date) {
    setValue<Date>(val)
  }
  ...
  private reified function getValue<V>() : V {
    if (CovTerm == null) {
      return null
    }
  ...
  private reified function setValue<V>(val : V) {
    if (CovTerm typeis GenericCovTerm<V>) {
      CovTerm.Value = val
    } else {
      var typeName = V.Type.RelativeName
  ...
  property get AssociatedPeriodRatedOrQuoted() : boolean {
    return CovInfo.AssociatedPeriodRatedOrQuoted
  }
  ...
  function getPostOnChangeMode(lineCovSet : SideBySideCoverageSet) : String {
    if (AssociatedPeriodRatedOrQuoted or AlwaysPostOnChange

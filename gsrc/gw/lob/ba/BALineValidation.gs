...
class BALineValidation extends PolicyLineValidation<entity.BusinessAutoLine> {
  ...
  function fleetTypeMatchesVehicleInfo() {
    ...
    if (Context.isAtLeast(TC_QUOTABLE)) {
      if (baLine.Fleet == null) {
        Result.addError(baLine, TC_QUOTABLE, DisplayKey.get("Web.Policy.BA.Validation.fleetType"), "PolicyInfo")
      } else if (baLine.Fleet == TC_FLEET and baLine.Vehicles.Count < 10 or
                 baLine.Fleet == TC_NONFLEET and baLine.Vehicles.Count >= 10) {
        Result.addError(baLine, TC_QUOTABLE, DisplayKey.get("Web.Policy.BA.Validation.NumberOfVehicles", baLine.Fleet.DisplayName, baLine.Vehicles.Count), "BusinessVehicles")
      } else {
        for (vehicle in baLine.Vehicles) {
          if (vehicle.validateVehicleClassCode() != null ) {
            Result.addError(baLine, TC_QUOTABLE, DisplayKey.get("Web.Policy.BA.Validation.ClassCode", vehicle.VehicleClassCode, vehicle.DisplayName, baLine.Fleet), "BusinessVehicles")
          }
        }
      }
    }
  ...
  function vinIsUnique() {
    ...
    for (vehicle in baLine.Vehicles) {
      ...
      if (matches > 1) {
        Result.addError( baLine, TC_DEFAULT, DisplayKey.get("Web.Policy.BA.Validation.VinNumbers") )
      }
    }
  }
  ...
  function atLeastOneHiredAutoState() {
    ...
    if (baLine.BAHiredCollisionCovExists or baLine.BAHiredCompCovExists or baLine.BAHiredLiabilityCovExists or
      ...
      if (hasHiredAutoCovJuris == false) {
        Result.addError(baLine, TC_DEFAULT, DisplayKey.get("Web.Policy.BA.Validation.AtLeastOneHiredAutoState"), "baLineStep")
      }
    } else if (hasHiredAutoCovJuris) {
      Result.addError(baLine, TC_DEFAULT, DisplayKey.get("Web.Policy.BA.Validation.AtLeastOneHiredAutoCov"), "baLineStep")
    } 
  }
  ...
  function atLeastOneNonownedState() {
    ...
    if (baLine.BANonownedLiabCovExists) {
      if (hasNonownedCovJuris == false) {
        Result.addError(baLine, TC_DEFAULT, DisplayKey.get("Web.Policy.BA.Validation.AtLeastOneNonownedState"), "baLineStep")
      }
    } else if (hasNonownedCovJuris) {
      Result.addError(baLine, TC_DEFAULT, DisplayKey.get("Web.Policy.BA.Validation.AtLeastOneNonownedCov"), "baLineStep")
    }
  }
  ...
  function nonOwnedBasisSumGreaterThanZero() {
    ...
    for (juris in nonOwnedJuris) {
      ...
      if (sum <= 0) {
        Result.addError(baLine, TC_DEFAULT, DisplayKey.get("Web.Policy.BA.Validation.NonOwnedBasisSumGreaterThanZero"), "baLineStep")
      }
    }
  }
  ...
  function checkLiabilityCoverage() {
    ...
    if (!baLine.BAOwnedLiabilityCovExists and baLine.Vehicles.Count > 0) {
      Result.addError(baLine, TC_QUOTABLE, DisplayKey.get("Web.Policy.BA.Validation.LiabilityRequired") )
    }
    if (baLine.BAOwnedLiabilityCovExists) {
      ...
      if (!baLine.isPackageAvailable(thepackage)) {
        Result.addError(baLine, TC_QUOTABLE, DisplayKey.get("Web.Policy.BA.Validation.LiabilityPackageInvalid", thepackage))          
      }
    }
  } 

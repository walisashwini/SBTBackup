...
enhancement BusinessAutoLineEnhancement : entity.BusinessAutoLine {
  ...
  property get GarageStatesAsSet() : Set<State> {
    ...
    for (vehicle in this.Vehicles) {
      if (vehicle.Location == null or vehicle.Location.State == null) {
        throw new IllegalStateException(DisplayKey.get("BusinessAuto.Vehicle.NoState"))
      }
  ...
  function getCostOfHireValidation(value : Integer, anyExposure : boolean) : String {
    if (not anyExposure and value == null) {
      return DisplayKey.get("Web.Policy.BA.CostHireRequired")
    }
    return null
  }
  ...
  function isPackageAvailable(thepackage : String) : boolean {
    ...
    var vehicleStates = VehicleStateList
  ...
  function getOtherthanHiredAndNonOwnedCoverages(state : Jurisdiction) : Coverage[] {
    ...
    return getCoverageThatExistOtherThan(juris.Coverages, new Coverage[] {this.BAHiredLiabilityCov, this.BAHiredCollisionCov, this.BAHiredCompCov,
  ...
  function setCoveredAutoSymbols() {
    var liabilitySymbol = this.getCoverageSymbolGroup(PCCoercions.makeProductModel<CoverageSymbolGroupPattern>("Liability"))
    var collisionSymbol = this.getCoverageSymbolGroup(PCCoercions.makeProductModel<CoverageSymbolGroupPattern>("Collision"))
    var comprehensiveSymbol = this.getCoverageSymbolGroup(PCCoercions.makeProductModel<CoverageSymbolGroupPattern>("Comprehensive"))
    var specCauseSymbol = this.getCoverageSymbolGroup(PCCoercions.makeProductModel<CoverageSymbolGroupPattern>("SpecCause"))
    var UMUIMSymbol = this.getCoverageSymbolGroup(PCCoercions.makeProductModel<CoverageSymbolGroupPattern>("UMUIM"))
    var towingSymbol = this.getCoverageSymbolGroup(PCCoercions.makeProductModel<CoverageSymbolGroupPattern>("Towing"))

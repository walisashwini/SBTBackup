...
class BOPPolicyLineMethods extends AbstractPolicyLineMethodsImpl {
  ...
  override property get AllCoverages() : Coverage[] {
    ...
    coverages.addAll(_line.BOPLocations.arrays("Coverages").toArray(new BOPLocationCov[0]).toList())
    //noinspection MultipleAsteriskDotOperators
    coverages.addAll(_line.BOPLocations.arrays("Buildings").arrays("Coverages").toArray(new entity.BOPBuildingCov[0]).toList())
  ...
  */
  ...
  override function shouldQuoteAsynchronously() : boolean {
    return _line.Branch.BOPLine.BOPLocations.arrays("Buildings").toArray(new BOPBuilding[0]).Count > AsyncQuoteCoverableThresholdByLine.BOPBuildingThreshold
  }
}

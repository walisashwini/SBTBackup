...
class CPBuildingValidation extends AbstractBuildingValidation<CPClassCodeSearchCriteria> {
  ...
  private function addlInterestDetailUnique() {
    ...
    for (lst in detailsPartitionedByUniqueKey.Values) {
      if (lst.Count > 1) {
        Result.addError(_building, TC_DEFAULT, DisplayKey.get("EntityName.PolicyLine.Validation.AddlInterestDetailUnique", lst.first().DisplayName), "CPBuildings")
      }
    }
  }
  ...
  private function requiredFieldsSet() {
    ...
    if (_building.CoverageForm == null) {
      Result.addError(_building, TC_DEFAULT, DisplayKey.get("Web.Policy.Validation.MissingRequiredField", 
              DisplayKey.get("Web.Policy.CP.Validation.Building", _building.CPLocation, _building),
  ...
  private function atLeastOneBaseCov() {
    ...
    if (!covExists) {
      if (Context.isAtLeast(TC_QUOTABLE)) {
        ...
        DisplayKey.get("Web.Policy.CP.Validation.atLeastOneCovRequired", _building.DisplayName, _building.CPLocation.DisplayName), "CPBuildings")
      } else {
        Result.addWarning(_building, TC_DEFAULT,
        DisplayKey.get("Web.Policy.CP.Validation.atLeastOneCovRequired", _building.DisplayName, _building.CPLocation.DisplayName), "CPBuildings")
      }
    }
  }
  ...
  private function atLeastOneBusinessIncomeLimit() {
    ...
    if (!limitExists) {
        Result.addError(_building, TC_DEFAULT,
        DisplayKey.get("Web.Policy.CP.Validation.atLeastOneBusIncLimitRequired"), "CPBuildings")
    }
  }
  ...
  override protected function addClassCodeError() : void {
    Result.addError(_building, TC_DEFAULT,
        DisplayKey.get("Web.Policy.CP.Validation.UnavailableClassCode", ClassCodeCode))
  }
}

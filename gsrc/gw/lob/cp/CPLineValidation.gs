...
class CPLineValidation extends PolicyLineValidation<entity.CommercialPropertyLine> {
  ...
  override function doValidate() {
    ...
    var buildings = cpLine.CPLocations.arrays("Buildings").toArray(new CPBuilding[0])
  ...
  function validateBlanketLimit(){
    ...
    for(blnk in blankets) {
      ...
      if(blnk.BuildingCoverages.HasElements) {
        if (buildingLimitSum != blanketLimit as double) {
          if (Context.isAtLeast(TC_QUOTABLE)) {
            Result.addWarning(blnk, TC_DEFAULT, DisplayKey.get("Web.Policy.CP.Validation.BlanketLimitDoesNotMatchTotalLimit", blnk.CPBlanketNum),"CPBlanket")
          }
        }
      }
    }
  ...
  function atLeastOneBuildingPerLocation() {
    ...
    if (loc != null or cpLine.CPLocations.IsEmpty) {
      if (Context.isAtLeast(TC_QUOTABLE)) {
        Result.addError( cpLine, TC_DEFAULT, DisplayKey.get("Web.Policy.CP.Validation.atLeastOneBuilding"), "CPBuildings")
      } else {
        Result.addWarning( cpLine, TC_DEFAULT, DisplayKey.get("Web.Policy.CP.Validation.atLeastOneBuilding"), "CPBuildings")
      }
    }
  }
  ...
  function validateAtleastTwoCoveragePerBlanket() {
    ...
    for (blanket in blankets) {
      if (blanket.BuildingCoverages.Count < 2) {
        if (Context.isAtLeast(TC_QUOTABLE)) {
          Result.addError(cpLine, TC_DEFAULT,DisplayKey.get("Web.Policy.CP.Validation.MustHaveAtLeastTwoBuildingCoverage", blanket.CPBlanketNum),"CPBlanket")
        } 
      }
    } 
  }

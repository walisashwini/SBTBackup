...
uses gw.lob.cp.rating.dtobased.CPDTORatingEngine
...
class CPPolicyLineMethods extends AbstractPolicyLineMethodsImpl {
  ...
  override property get AllCoverages() : Coverage[] {
    ...
    coverages.addAll(locations.arrays("Buildings").arrays("Coverages").toArray(new CPBuildingCov[0]).toList())
    coverages.addAll(locations.arrays("Coverages").toArray(new CPLocationCov[0]).toList())
  ...
  override property get AllCoverables() : Coverable[] {
    ...
    var coverables : List<Coverable> = {}
    coverables.addAll(locations.arrays("Buildings").toArray(new CPBuilding[0]).toList())
  ...
  override function canSafelyDeleteLocation(location : PolicyLocation) : String {
    ...
    if (not (currentOrFutureCPLocationsMap["current"] == null or currentOrFutureCPLocationsMap["current"].Empty)) {
      return DisplayKey.get("CommercialProperty.Location.CannotDelete.HasCPLocation", location)
      ...
      return DisplayKey.get("CommercialProperty.Location.CannotDelete.HasFutureCPLocation", location, futureDatesStr)
    }
    return super.canSafelyDeleteLocation(location)
  }

  override property get LocationsInUseOnOrAfterSliceDate(): Set<Key> {
    var policyLocationsInUseNowOrInFuture = _line.VersionList.CPLocations.allVersionsFlat<CPLocation>().where(\CPLoc -> CPLoc.ExpirationDate > _line.SliceDate)*.PolicyLocation*.FixedId
  ...
  override function getAllLineBuildingsEver() : List<LineSpecificBuilding> {
    return _line.VersionList.CPLocations.arrays<CPBuildingVersionList>("Buildings").allVersionsFlat<CPBuilding>()
  }
  ...
  override protected function getCannotDeleteBuildingMessage(building : Building) : String {
    return DisplayKey.get("CommercialProperty.Building.CannotDelete.HasCPBuilding", building)
  }

  override protected function getCannotDeleteBuildingFutureMessage(building : Building, dates : String) : String {
    return DisplayKey.get("CommercialProperty.Building.CannotDelete.HasFutureCPBuilding", building, dates)
  }
  ...
  override function createRatingEngine(method: RateMethod, parameters: Map<RateEngineParameter, Object>): AbstractRatingEngine {
    ...
    if (PCConfigParameters.EnableCPDTOParallelRating.Value) {
      return new CPDTORatingEngine(_line as CPLine, parameters[RateEngineParameter.TC_RATEBOOKSTATUS] as RateBookStatus)
    } else {
  ...
  override function shouldQuoteAsynchronously() : boolean {
    return _line.Branch.CPLine.CPLocations.arrays("Buildings").toArray(new CPBuilding[0]).Count > AsyncQuoteCoverableThresholdByLine.CPBuildingThreshold
  }

...
class GL7LocationSchedCovImpl extends AbstractScheduleWithDescriptionImpl<GL7LocationSchedCov> {

  construct(delegateOwner : entity.GL7LocationSchedCov) {
    super(delegateOwner)
  }
  ...
  override function createAndAddScheduledItem() : ScheduledItem {
    var scheduledItem = new GL7LocationSchedCovItem(Owner.Branch)
  ...
  override function removeScheduledItem(item : ScheduledItem) {
    Owner.removeFromGL7ScheduledItems(item as GL7LocationSchedCovItem)
  ...
  override property get CurrentAndFutureScheduledItems() : KeyableBean[] {
    ...
    Owner.Branch.OOSSlices
        ...
        .each(\ p ->  {
      var matchingSlicedScheduleCov = p.GL7Line.CoveragesFromCoverable.firstWhere(\ c -> c.FixedId == Owner.FixedId) as GL7LocationSchedCov
    ...
    return schedItems.map(\ item -> item as GL7LocationSchedCovItem).toTypedArray()
  }

...
uses entity.GL7LocSchedCovItemCov
uses entity.GL7LocSchedCovItemCond
uses entity.GL7LocationSchedCovItem
uses typekey.Currency
uses entity.GL7LocSchedCovItemExcl

class GL7LocationSchedCovItemAdapter extends AbstractScheduledItemAdapter {

  var _owner : GL7LocationSchedCovItem as readonly Owner

  construct(item : GL7LocationSchedCovItem) {
    _owner = item
  }
  ...
  override property get PolicyLine() : PolicyLine {
    return _owner.ScheduleCoverage.GL7Location.SublineType.Line
  }
  ...
  override function addCoverage(coverage : Coverage) {
    _owner.addToCoverages(coverage as GL7LocSchedCovItemCov)
  }

  override function removeCoverage(coverage : Coverage) {
    _owner.removeFromCoverages(coverage as GL7LocSchedCovItemCov)
  }
  ...
  override function addCondition(cond: PolicyCondition) {
    _owner.addToConditions(cond as GL7LocSchedCovItemCond)
  }

  override function removeCondition(cond: PolicyCondition) {
    _owner.removeFromConditions(cond as GL7LocSchedCovItemCond)
  }
  ...
  override function addExclusion(exclusion : Exclusion) {
    _owner.addToExclusions(exclusion as GL7LocSchedCovItemExcl)
  }

  override function removeExclusion(exclusion : Exclusion) {
    _owner.removeFromExclusions(exclusion as GL7LocSchedCovItemExcl)
  }

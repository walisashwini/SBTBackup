...
class GL7SublineSchedExclImpl extends AbstractScheduleWithDescriptionImpl<GL7SublineSchedExcl> {

  construct(delegateOwner : GL7SublineSchedExcl) {
    super(delegateOwner)
  }
  ...
  override function createAndAddScheduledItem() : ScheduledItem {
    var scheduledItem = new GL7SublineSchedExclItem(Owner.Branch)
  ...
  override function removeScheduledItem(item : ScheduledItem) {
    Owner.removeFromGL7ScheduledItems(item as GL7SublineSchedExclItem)
  ...
  override property get CurrentAndFutureScheduledItems() : KeyableBean[] {
    ...
    Owner.Branch.OOSSlices
      ...
      .each(\ p ->  {
        var matchingSlicedScheduleExcl = p.GL7Line.ExclusionsFromCoverable.firstWhere(\ c -> c.FixedId == Owner.FixedId) as GL7SublineSchedExcl

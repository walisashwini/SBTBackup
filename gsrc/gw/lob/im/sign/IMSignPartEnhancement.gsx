...
enhancement IMSignPartEnhancement : IMSignPart {
   ...
   */
   property get CurrentAndFutureIMSigns() : IMSign[] {
     var signs = this.IMSigns.toSet()

     this.InlandMarineLine.getAllExistingCoverageParts(IMSignPart)
         .each(\signPart -> signs.addAll(signPart.IMSigns
             .where(\sign -> sign.ExpirationDate.after(this.Branch.EditEffectiveDate)).fastList()))

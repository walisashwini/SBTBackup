...
class PAPolicyLineMethods extends AbstractPolicyLineMethodsImpl {
  ...
  override property get AllCoverages() : Coverage[] {
    ...
    covs.addAll(_line.Vehicles.arrays<Coverage>("Coverages"))
  ...
  override function canSafelyDeleteLocation(location : PolicyLocation) : String {
    ...
    if (not (currentOrFutureVehiclesMap["current"] == null or currentOrFutureVehiclesMap["current"].Empty)) {
      ...
      return DisplayKey.get("PersonalAuto.Location.CannotDelete.HasVehicles", currentVehiclesStr)
      ...
      return DisplayKey.get("PersonalAuto.Location.CannotDelete.HasFutureVehicles", futureVehiclesStr, futureDatesStr)
    }
  ...
  override property get LocationsInUseOnOrAfterSliceDate(): Set<Key> {
    var locationsInUseOnOrAfterSliceDate = _line.VersionList.Vehicles.allVersionsFlat<PersonalVehicle>()
        .where(\veh -> veh.ExpirationDate > _line.SliceDate)*.GarageLocation*.FixedId
    // PA doesn't care about the primary location, so there is no need to add Primary location to the list of 'in-use' locations.
    return locationsInUseOnOrAfterSliceDate.toSet()
  }
  ...
  override property get Copier() : CompositeCopier<PolicyPeriod, entity.PersonalAutoLine> {
  ...
  override function onBeginIssueJob() {
    if (gw.plugin.Plugins.isEnabled(gw.plugin.motorvehiclerecord.IMotorVehicleRecordPlugin)) {
  ...
  override function getWorksheetRootNode(showConditionals : boolean) : RowTreeRootNode {
      ...
      if (garagedVehicles.Count > 0) {
        ...
        garagedVehicles.each(\ vehicle -> {
          ...
          if (assignmentWorksheet != null) {
            var driverContainer = new WorksheetTreeNodeContainer(DisplayKey.get("Web.Policy.RatingWorksheet.DriverAssignment"))
            ...
            if (assignmentWorksheet != null) {
              var driverContainer = new WorksheetTreeNodeContainer(DisplayKey.get("Web.Policy.RatingWorksheet.DriverAssignmentForDriver", drv.DisplayName))
              vehicleContainer.addChild(driverContainer)
              driverContainer.addChildren(WorksheetTreeNodeUtil.buildTreeNodes(assignmentWorksheet, showConditionals))
            }
          }
    ...
    var otherCosts = _line.PACosts.where(\ p -> p.Vehicle == null and not p.IsTaxOrSurcharge)

...
enhancement ProcessMVRsWFEnhancement : entity.ProcessMVRsWF {
  ...
  function setupActivity(activity: Activity){
    ...
    if(allClear){
      subjectSuffix = DisplayKey.get("Web.PersonalAuto.MotorVehicleRecord.Clear")
      descriptionSuffix = ""
    }
    else{
      subjectSuffix = DisplayKey.get("Web.PersonalAuto.MotorVehicleRecord.NotClear")
      descriptionSuffix = DisplayKey.get("Web.PersonalAuto.MotorVehicleRecord.ReviewRiskAnalysisPage")
    }
  
    activity.assignToCreator(this)
    activity.Account = this.PolicyPeriod.Policy.Account
    activity.setFieldValue("Job", this.Job)
    activity.Subject = DisplayKey.get("Web.PersonalAuto.MotorVehicleRecord.MVRsReceived") + subjectSuffix
    activity.Description = DisplayKey.get("Web.PersonalAuto.MotorVehicleRecord.MVRsReceived") + descriptionSuffix

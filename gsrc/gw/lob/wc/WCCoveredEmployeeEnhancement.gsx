...
enhancement WCCoveredEmployeeEnhancement : entity.WCCoveredEmployee {
  ...
  /**
   * Yet another way to calculate a basis for rating.  This is a slightly different means of getting the Basis for
   * Rating used by the UI.
   */
  property get AuditRatingBasis() : int {
    var policyPeriod = this.Branch
    var info = policyPeriod.Audit.AuditInformation
    if (info.IsFinalAudit) {
      if (info.IsRevision) {
        return this.LastBilledCoveredEmployee.BasisForRating
      } else {
        return this.BasisAmount
      }
    }
    return this.ProratedEstimatedAmount
  }
  ...
  function getIfAnyOrExposureAmount() : String {
    if (this.isIfAnyExposure() != null and this.isIfAnyExposure()) {
      return DisplayKey.get("Java.WorkersComp.IfAnyExposure")
    }

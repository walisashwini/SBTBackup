...
class WCPolicyLineMethods extends AbstractPolicyLineMethodsImpl {
  ...
  override property get AllCoverages() : Coverage[] {
    ...
    covs.addAll(_line.Jurisdictions.arrays<Coverage>("Coverages"))
  ...
  override function onSubmissionBeginEditing() {
    ...
    if (state != null && _line.getJurisdiction( state ) == null) {
      _line.addJurisdiction( state ) 
    }
  }
  ...
  override function prorateBasesFromCancellation() {
    for (jurisdiction in _line.Jurisdictions) {
      ...
      // As the RPSD will be created with a Normalized date in addRatingPeriodStartDate
      // we need to call canAddRPSD against a Normalized date
      var normalizedDate = PCDateUtil.mergeDateAndTime(cancellationDate, jurisdiction.PolicyLine.Branch.PeriodStart)
      if (jurisdiction.canAddRPSD(normalizedDate) == null){
        jurisdiction.addRatingPeriodStartDate(cancellationDate, TC_AUDIT)
      }
    }
    _line.updateWCExposuresAndModifiers()
  }
  ...
  override function shouldQuoteAsynchronously() : boolean {
    return _line.Branch.WorkersCompLine.WCCoveredEmployees.Count > AsyncQuoteCoverableThresholdByLine.WCExposureThreshold
  }
}

...
class WCCoveredEmployeeRater {
   ...
   */
  private static function getManualRateAndUpdateMinimumPremiums(engine : WCSysTableRatingEngine, covEmp : WCCoveredEmployee, ratingDate : Date) : BigDecimal {
    ...
    if (rate == null) {
     theRate = calculateDemoRate(convertClassCodeToNumber(criteria.NormalizedClassCode))  // seed with the normalized class code
      theMinPremium = 50
  ...
  private static function convertClassCodeToNumber(classCode : String) : BigDecimal {
    var number = classCode.replaceAll("[^0-9]+", "")
    if (number.isEmpty()) {
      number = "0";
    }
  ...
  private static function getUSLHFactor(covEmp : WCCoveredEmployee, ratingDate : Date) : BigDecimal {

...
class DuplicateContactsPopupHelper {
  ...
  function checkForDuplicatesOrUpdate(commit()) {
    if (not _dupesCheckedOnUpdate and hasDuplicateResults()) {
      ...
      if (_sameContactAlreadyExistsInParentPageLV) {
        ...
        throw new DisplayableException(DisplayKey.get("Web.Contact.DuplicatePolicyContactRoleError", _originalContact))
      }
      commit()
    }
  }
  ...
  private function hasDuplicateResults() : boolean {
    try {
    ...
    } catch (rfe : wsi.remote.gw.webservice.ab.ab900.abcontactapi.faults.RequiredFieldException) {
      ...
      return false
    } catch (rfe : wsi.remote.gw.webservice.ab.ab1000.abcontactapi.faults.RequiredFieldException) {
      // in this case, we are trying to commit the contact, but don't have enough info to check dupes - allow the user to commit anyway
      return false
  ...
  private function validate() {
    try {
      ...
      throw new DisplayableException(DisplayKey.get("Web.Contact.CheckForDuplicates.Error.MissingRequiredFields", rfe.Message))
    } catch (rfe : wsi.remote.gw.webservice.ab.ab1000.abcontactapi.faults.RequiredFieldException) {
      throw new DisplayableException(DisplayKey.get("Web.Contact.CheckForDuplicates.Error.MissingRequiredFields", rfe.Message))
    } catch (rfe : gw.api.webservice.exception.RequiredFieldException) {
      throw new DisplayableException(DisplayKey.get("Web.Contact.CheckForDuplicates.Error.MissingRequiredFields", rfe.Message))
    ...
    if (_duplicateResultsContainer.Results.Empty) {
      throw new DisplayableException(DisplayKey.get("Web.Contact.CheckForDuplicates.Error.NoResults"))
    }
  }
  ...
  function copyDataFromDuplicateABContactToPCContact(abDuplicate : DuplicateContactResult ) {
    ...
    if (_existingContactsInParentPageLV.hasMatch(\ c -> {return c.AddressBookUID == abDuplicate.ContactAddressBookUID})) {
      ...
      var existingContactWithABUID = gw.transaction.Transaction.getCurrent().add(Query.make(Contact)
  ...
  /**
   * Change account holder contact to existingPCContactWithABContactMatch and remove acctContact from db
   * @param account account to change account holder
   * @param acctContact account contact to be removed
   */
  function changeAndRemoveAccountHolder(account: entity.Account,acctContact: entity.AccountContact){
    var existingContactMatch = acctContact.Contact.Bundle.add(existingPCContactWithABContactMatch)
    existingContactMatch.copyUIFieldsFromContact(acctContact.Contact)
    account.changeAccountHolderTo(existingContactMatch)

    account.removeFromAccountContacts(acctContact)

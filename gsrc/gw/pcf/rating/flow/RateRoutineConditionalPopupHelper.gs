...
class RateRoutineConditionalPopupHelper {
  ...
  function checkForUnbalancedParentheses() {
    ...
    if (balance > 0) {
      throw new DisplayableException(DisplayKey.get("Web.Rating.Errors.MissingCloseParen"))
    } else if (balance < 0) {
      throw new DisplayableException(DisplayKey.get("Web.Rating.Errors.MissingOpenParen"))
    }
  }
    ...
    if (rightOp.OperandType == TC_COLLECTION) {
      if (leftOp.IsTypeKey and rightOp.OperatorType.Categories.contains(CalcStepOperatorCategory.TC_INCLUSION)) {
        if (leftOp.TypeDeclaration != rightOp.TypeDeclaration) {
          return DisplayKey.get("Validation.Rating.RateRoutineDefinition.TypeMismatch", leftOp.TypeDeclaration, rightOp.TypeDeclaration)
        }
      } else {
        return DisplayKey.get("Validation.Rating.RateRoutineDefinition.OnlyTypekeysCanBeComparedToLists")
      }
    }
  ...
  static function mightBeBoolean(conditionalSubOperand : CalcStepDefinitionOperand, availLocalVars : List<LocalVariable>) : Boolean {
    ...
    if (conditionalSubOperand.OperandType == TC_LOCALVAR) {
      ...
      if (localVar.VarType == null) {
        ...
        return gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(java.lang.Boolean.Type, localVar.VarType)

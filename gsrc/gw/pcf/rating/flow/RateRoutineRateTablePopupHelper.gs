...
uses gw.rating.RatingConstants
...
class RateRoutineRateTablePopupHelper {
  ...
  function onTableChange(targetDataTypes : java.util.Set<gw.lang.reflect.IType>) {
    ...
    if (_operand.TableCode != null and _operand.RateTableDefinition != null) {
      ...
      _operand.updateArgumentSources()
  ...
  function getFactorRange(tabCode : String, targetDataTypes : Set<IType>) : String[] {
    ...
    if ((targetableFactorNames.Count > 1) and
      ...
      targetableFactorNames.add(RatingConstants.AllFactorsCode)
    }
    return targetableFactorNames.toTypedArray()
  }

  function getColumnFactorLabel(factorColumnName : String) : String {
    if (factorColumnName == RatingConstants.AllFactorsCode) {
      return DisplayKey.get("Web.Rating.Flow.CalcRoutine.RateTableReturnAllFactorsLabel")
  ...
  function noRateTableError(targetDataTypes: Set<IType>): String {
    return targetDataTypes == null ?
        DisplayKey.get("Web.Rating.Errors.NoTargetTypes") :
        DisplayKey.get("Web.Rating.Errors.UnableToFindRateTableWithFactorType", targetDataTypes.join(","))
  }
}

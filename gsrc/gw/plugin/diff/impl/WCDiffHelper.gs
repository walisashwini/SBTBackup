...
class WCDiffHelper extends DiffHelper<entity.WorkersCompLine>
{
  ...
  override function addDiffItems(diffItems : List<DiffItem>) : List<DiffItem> {
    ...
    createDiffsForEntity(diffItems, Line1.RetrospectiveRatingPlan, Line2.RetrospectiveRatingPlan)
    createDiffsForEntity(diffItems, Line1.ParticipatingPlan, Line2.ParticipatingPlan)
  ...
  private function addWCJurisdictionDeductibleDiffs(diffItems : List<DiffItem>) : List<DiffItem> {
    return addSplittableDiffs(diffItems, \ w -> getCoveragesWithTermValues(w, PCCoercions.makeProductModel<CovTermPattern>("WCDeductible")), \ s -> getCoverageKey(s))
  }

...
class NotificationPlugin implements INotificationPlugin {
  ...
  private function forAllLinesAndJurisdictions(lineToJurisdictions : Map<PolicyLinePattern, Jurisdiction[]>,
    ...
    for (lineOfBusiness in lineToJurisdictions.Keys) {
      if (lineOfBusiness == null) {
        throw new DisplayableException(DisplayKey.get("NotificationPlugin.Error.PolicyLinePatterns.NullValue"))
      }
      var jurisdictions = lineToJurisdictions.get(lineOfBusiness)
      if (jurisdictions.IsEmpty) {
        throw new DisplayableException(DisplayKey.get("NotificationPlugin.Error.Jurisdictions.Empty", lineOfBusiness))
      }
      for (jurisdiction in jurisdictions) {
        if (jurisdiction == null) {
          throw new DisplayableException(DisplayKey.get("NotificationPlugin.Error.Jurisdictions.NullValue", lineOfBusiness))
        }
  ...
  private static function checkArguments(lineToJurisdictions : Map<PolicyLinePattern, Jurisdiction[]>, actionType : NotificationActionType) {
    ...
    if (actionType == null) {
      throw new DisplayableException(DisplayKey.get("NotificationPlugin.Error.ActionType.NullValue"))
    }
  }
  ...
  private static function checkArguments(lineToJurisdictions : Map<PolicyLinePattern, Jurisdiction[]>, category : NotificationCategory) {
    assertPolicyLinePatternsNotEmpty(lineToJurisdictions)
    if (category == null) {
      throw new DisplayableException(DisplayKey.get("NotificationPlugin.Error.Category.NullValue"))
    }
  }
  ...
  private static function assertPolicyLinePatternsNotEmpty(lineToJurisdictions : Map<PolicyLinePattern, Jurisdiction[]>) {
    if (lineToJurisdictions.Empty) {
      throw new DisplayableException(DisplayKey.get("NotificationPlugin.Error.PolicyLinePatterns.Empty"))
    }
  }
   ...
   */
  private static function getLeadTime(effDate : Date, lineOfBusiness : PolicyLinePattern, jurisdiction : Jurisdiction, actionType : NotificationActionType) : int {
    ...
    if (config == null) {
      throw new NotificationPluginNoMatchException(DisplayKey.get("NotificationPlugin.Error.ActionType.NoMatch", effDate, lineOfBusiness, jurisdiction, actionType))
    }
   ...
   * @throws DisplayableException if there are no notification configuration rows matching the passed criteria
   */
  private static function getMaximumLeadTime(effDate : Date, lineOfBusiness : PolicyLinePattern, jurisdiction : Jurisdiction, category : NotificationCategory) : int {
   ...
   * @throws DisplayableException if there are no notification configuration rows matching the passed criteria
   */
  private static function getMinimumLeadTime(effDate : Date, lineOfBusiness : PolicyLinePattern, jurisdiction : Jurisdiction, category : NotificationCategory) : int {
   ...
   * @throws DisplayableException if there are no notification configuration rows matching the passed criteria
   */
  private static function getLeadTime(effDate : Date, lineOfBusiness : PolicyLinePattern, jurisdiction : Jurisdiction, category : NotificationCategory, isAscending : boolean) : int {
    ...
    if (config == null) {
      throw new NotificationPluginNoMatchException(DisplayKey.get("NotificationPlugin.Error.Category.NoMatch", effDate, lineOfBusiness, jurisdiction, category))
    }
    return config.LeadTime
  }
   ...
   * @param category       notification category to use for this notification configuration lookup
   */
  private static function createDefaultSortedQueryResult(effDate : Date, lineOfBusiness : PolicyLinePattern,
   ...
   *                       </ul>
   */
  private static function createQuery(effDate : Date, lineOfBusiness : PolicyLinePattern, jurisdiction : Jurisdiction,
                                      actionType : NotificationActionType, category : NotificationCategory) : Query<NotificationConfig> {
    var query = Query.make(NotificationConfig)
    query.and( \ restriction -> {
      addWildCardedRestriction(restriction, JURISDICTION_COLUMN, jurisdiction)
      addWildCardedRestriction(restriction, LINE_OF_BUSINESS_COLUMN, lineOfBusiness.PublicID)
      if (actionType != null) {

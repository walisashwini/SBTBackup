...
class PolicyPlugin implements IPolicyPlugin {
  ...
  override function canStartIssuance( policy: Policy ) : String {
    ...
    appendIfFalse(policy.OpenJobs.Count == 0, DisplayKey.get("Job.Error.OtherOpenJobs"), sb)      
    appendIfFalse(not policy.Issued, DisplayKey.get("Job.Error.PolicyIssuedAlready"), sb) 
    appendIfFalse(perm.Issuance.create, DisplayKey.get("Job.Error.NoIssuancePermission"), sb)
    appendIfFalse(period.CancellationDate == null, DisplayKey.get("Job.Cancellation.Error.IssuePolicy"), sb)
  ...
  override function canStartPolicyChange( policy: Policy, effectiveDate: Date ) : String {
    ...
    var sb = new StringBuilder()
    appendIfFalse(policy.Issued, DisplayKey.get("Job.Error.PolicyNotIssued"), sb)
    appendIfFalse(perm.PolicyPeriod.change(period), DisplayKey.get("Job.Error.NoChangePermission"), sb)
    appendIfFalse(not policy.hasOpenRewriteJob(), DisplayKey.get("Job.Error.OpenRewriteJob"), sb)
    appendIfFalse(not policy.hasOpenIssuanceJob(), DisplayKey.get("Job.Error.OpenIssuanceJob"), sb)
    appendIfFalse(not policy.hasOpenRewriteNewAccountJobOnNewPolicy(), DisplayKey.get("Job.RewriteNewAccount.Error.OpenRewriteNewAccountJob"), sb)
    ...
    if (destinationPolicy != null) {
      ...
      appendIfFalse(effectiveDate < startDate,
          DisplayKey.get("Job.RewriteNewAccount.Error.PeriodStartAfterDestinationStart", startDate.ShortFormat), sb)
    }
    return errorString(sb)
  }
  ...
  override function canStartCancellation( policy: Policy, effectiveDate: Date ) : String {
    ...
    var sb = new StringBuilder()
    appendIfFalse(perm.PolicyPeriod.cancel(period), DisplayKey.get("Job.Cancellation.Error.NoCancelPermission"), sb)
    appendIfFalse(not period.Canceled or effectiveDate < period.CancellationDate, DisplayKey.get("Job.Cancellation.Error.AlreadyCanceled"), sb)
    appendIfFalse(not policy.hasOpenRewriteJob(), DisplayKey.get("Job.Error.OpenRewriteJob"), sb)    
    appendIfFalse(not policy.hasOpenIssuanceJob(), DisplayKey.get("Job.Error.OpenIssuanceJob"), sb)
    appendIfFalse(not policy.hasOpenRewriteNewAccountJobOnNewPolicy(), DisplayKey.get("Job.RewriteNewAccount.Error.OpenRewriteNewAccountJob"), sb)
    return errorString(sb)
  ...
  override function canStartReinstatement( policyPeriod: PolicyPeriod ) : String {
    ...
    if (period == null) {
      return DisplayKey.get("Job.Reinstatement.Error.NoRecentBoundPeriod")
    } else if (period.PolicyTerm.CheckArchived) {
      return DisplayKey.get("Web.Job.Warning.ArchivedTerm", period.EditEffectiveDate.ShortFormat)
    } else if (PCArchivingUtil.hasFutureArchivedTerms(period)) {
      return DisplayKey.get("Web.Job.Warning.ArchivedFutureTerm", period.EditEffectiveDate.ShortFormat)
    ...
    var sb = new StringBuilder()
    appendIfFalse(perm.PolicyPeriod.reinstate(period), DisplayKey.get("Job.Reinstatement.Error.NoReinstatePermission"), sb)
    appendIfFalse(period.CancellationDate != null, DisplayKey.get("Job.Error.PolicyNotCanceled"), sb)
    appendIfFalse(not period.Policy.hasOpenRewriteJob(), DisplayKey.get("Job.Error.OpenRewriteJob"), sb)
    appendIfFalse(not period.Policy.hasOpenIssuanceJob(), DisplayKey.get("Job.Error.OpenIssuanceJob"), sb)
    appendIfFalse(not period.Policy.hasOpenReinstateJobForPeriod(period), DisplayKey.get("Job.Reinstatement.Error.OpenReinstate"), sb)
    appendIfFalse(not period.Policy.hasOpenRewriteNewAccountJobOnNewPolicy(), DisplayKey.get("Job.RewriteNewAccount.Error.OpenRewriteNewAccountJob"), sb)
    appendIfFalse(not period.isRewrittenToNewAccountAndCanceledLocked(), DisplayKey.get("Job.RewriteNewAccount.Error.CanceledAndExistingRewriteNewAccountJob"), sb)
    appendIfFalse(period.Policy.Issued, DisplayKey.get("Job.Error.PolicyNotIssued"), sb)
    if (period.CancellationDate != null) {
      ...
      appendIfFalse(reinstatementExpDate > period.CancellationDate, DisplayKey.get("Job.Reinstatement.Error.OverlapPeriod"), sb)
    }
    return errorString(sb)
  }
  ...
  override function canStartRewrite( policy: Policy, effectiveDate: Date, expirationDate: Date ) : String {
    ...
    var sb = new StringBuilder()
    appendIfFalse(perm.PolicyPeriod.rewrite(period), DisplayKey.get("Job.Rewrite.Error.NoRewritePermission"), sb)
    appendIfFalse(period.CancellationDate != null, DisplayKey.get("Job.Error.PolicyNotCanceled"), sb)
    appendIfFalse(effectiveDate >= period.CancellationDate, DisplayKey.get("Job.Rewrite.Error.EffectiveBeforeCancelDate"), sb)            
    appendIfFalse(policy.OpenJobs.countWhere(\j -> j.Subtype != TC_AUDIT) == 0, DisplayKey.get("Job.Rewrite.Error.OpenJobs"), sb)
    appendIfFalse(not period.Policy.hasOpenRewriteNewAccountJobOnNewPolicy(), DisplayKey.get("Job.RewriteNewAccount.Error.OpenRewriteNewAccountJob"), sb)
    appendIfFalse(not period.isRewrittenToNewAccountAndCanceledLocked(), DisplayKey.get("Job.RewriteNewAccount.Error.CanceledAndExistingRewriteNewAccountJob"), sb)
    appendIfFalse(policy.Issued, DisplayKey.get("Job.Error.PolicyNotIssued"), sb)
    var hasOverlap = period.isOverlappingBoundPeriods(effectiveDate, expirationDate)
    appendIfFalse(!hasOverlap, DisplayKey.get("Job.Rewrite.Error.OverlapPeriod"), sb)
    return errorString(sb)
  }
  ...
  override function canStartRewriteNewAccount(basedOn : PolicyPeriod, effectiveDate : Date, expirationDate : Date, account : Account) : String {
    if (basedOn.PolicyTerm.CheckArchived) {
      return DisplayKey.get("Web.Job.Warning.ArchivedTerm", basedOn.EditEffectiveDate.ShortFormat)
    }
    
    var sb = new StringBuilder()
    var earliestDate = basedOn.Canceled ? basedOn.CancellationDate : basedOn.PeriodEnd
    appendIfFalse(effectiveDate.afterOrEqual(earliestDate), DisplayKey.get("Job.RewriteNewAccount.Error.NotCanceledOrExpired", effectiveDate), sb)
    appendIfFalse(perm.PolicyPeriod.rewritenewaccount(basedOn), DisplayKey.get("Job.RewriteNewAccount.Error.NoPermission"), sb)
    appendIfFalse(basedOn.Policy.findMostRecentNoncancelledBoundPeriodWithinDateRange(effectiveDate, null) == null, DisplayKey.get("Job.RewriteNewAccount.Error.PolicyInForce"), sb)
    appendIfFalse(basedOn.Policy.Issued, DisplayKey.get("Job.Error.PolicyNotIssued"), sb)
    appendIfFalse(basedOn.Policy.OpenJobs.countWhere(\j -> j.Subtype != typekey.Job.TC_AUDIT) == 0, DisplayKey.get("Job.RewriteNewAccount.Error.OpenJobs"), sb)
    appendIfFalse(basedOn.Policy.Account != account, DisplayKey.get("Job.RewriteNewAccount.Error.SameAccount"), sb)
    return errorString(sb)
  }
  ...
  override function canStartRenewal( policy: Policy ) : String {
    ...
    if (period == null) {
      return DisplayKey.get("Job.Renewal.Error.NoPeriod")
    }
    if (period.PolicyTerm.CheckArchived) {
      return DisplayKey.get("Web.Job.Warning.ArchivedTerm", period.EditEffectiveDate.ShortFormat)
    }

    var sb = new StringBuilder()
    appendIfFalse(perm.Renewal.create, DisplayKey.get("Job.Renewal.Error.NoCreateRenewPermission"), sb)
    appendIfFalse(policy.Issued, DisplayKey.get("Job.Error.PolicyNotIssued"), sb)
    appendIfFalse(not policy.hasOpenRewriteJob(), DisplayKey.get("Job.Error.OpenRewriteJob"), sb)
    appendIfFalse(not policy.hasOpenRenewalJob(), DisplayKey.get("Job.Renewal.Error.OpenRenewal"), sb)
    appendIfFalse(policy.RewrittenToNewAccountDestination == null, DisplayKey.get("Job.RewriteNewAccount.Error.ExistingRewriteNewAccountJob"), sb)
    appendIfFalse(perm.PolicyPeriod.renew(period), DisplayKey.get("Job.Renewal.Error.NoRenewPermission"), sb)
    appendIfFalse(not period.Canceled, DisplayKey.get("Job.Renewal.Error.PolicyCanceled"), sb)
    return errorString(sb)
  }
  ...
  override function canStartAudit( policy: Policy, effectiveDate : Date ) : String {
    ...
    var sb = new StringBuilder()
    appendIfFalse(perm.Audit.start, DisplayKey.get("Job.Error.NoAuditPermission"), sb)
    appendIfFalse(policy.Issued, DisplayKey.get("Job.Error.PolicyNotIssued"), sb)
    return errorString(sb)
  }

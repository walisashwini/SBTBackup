...
class PolicyHoldJobEvalPlugin implements IPolicyHoldJobEvalPlugin {
  ...
  override function buildQueryForJobsToEvaluate(aQuery: IQueryBuilder<PolicyHoldJob>) {
    /*
     * This method originally had a distinct.  PC-35552 removes it because it is unnecessary and causes
     * a error in PostgreSQL due to a bug in the PostgreSQL JDBC drivers.
     */
  ...
  override function evaluate(policyHoldJob : PolicyHoldJob) {
    ...
    var periods = job.Periods.where(\ p ->
      p.Locked == false and p.UWIssuesActiveOnly.hasMatch(\u ->
          u.IssueKey == policyHold.PolicyHoldCode and u.IssueType.isSameAs(policyHold.IssueType))
    )
  ...
  private function generateActivity(policyHoldJob : PolicyHoldJob, policyHold : PolicyHold) {
    ...
    var activity = ActivityPattern.finder.getActivityPatternByCode("policy_hold_released")
                          .createJobActivity(policyHoldJob.Bundle, job,
                                             DisplayKey.get("PolicyHold.Released.Activity.Subject", policyHold.PolicyHoldCode),
                                             DisplayKey.get("PolicyHold.Released.Activity.Description", policyHold.PolicyHoldCode),
                                             null, null, null, null, null)
    // attempt to assign to the producer
    var success = false
    var producer = job.getUserRoleAssignmentByRole(TC_PRODUCER)
    if (producer != null) {

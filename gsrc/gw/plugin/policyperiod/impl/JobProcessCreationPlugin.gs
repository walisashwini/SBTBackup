...
class JobProcessCreationPlugin implements IJobProcessCreationPlugin {
  ...
  override function createJobProcess(period: PolicyPeriod): IJobProcess {
    if (period == null) {
      throw new DisplayableException(DisplayKey.get("Java.PolicyPeriod.Error.NullPeriod"))
    }
    if (period.Job == null) {
      throw new DisplayableException(DisplayKey.get("Java.PolicyPeriod.Error.NullJob"));
    }

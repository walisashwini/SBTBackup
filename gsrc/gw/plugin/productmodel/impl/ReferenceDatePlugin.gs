...
class ReferenceDatePlugin implements IReferenceDatePlugin {
   ...
   */
  override function getCoverageReferenceDate(coveragePattern : CoveragePattern, coverable : Coverable) : Date {
   ...
   * date is computed based on the coverable's data.
   */
  override function getExclusionReferenceDate(exclusionPattern : ExclusionPattern, coverable : Coverable) : Date {
   ...
   * date is computed based on the coverable's data.
   */
  override function getConditionReferenceDate(conditionPattern : ConditionPattern, coverable : Coverable) : Date {
   ...
   */
  private function getCoverageExclusionConditionReferenceDate(patternRefDateType : typekey.ReferenceDateByType,coverable : Coverable) : Date {
    if (patternRefDateType == TC_APPLICABLEOBJECT) {
    ...
    if (jobIsQuoting(period)) {
      if (patternRefDateType == TC_DEFINEDOBJECT) {
        key = new MultiKey(policyLine, coverable.CoverableState)
      } else /* "PolicyTerm" */ {
        key = new MultiKey("PolicyTerm", period.FirstIssuedPeriodInTerm)
      }
      /**
       * Perform lookup
       */
      refDate = Coercions.makeDateFrom(jobProcess.BeanCache.get(key))
      if (refDate != null) {
        return refDate
      }
    }
    ...
    if (patternRefDateType == TC_DEFINEDOBJECT) {
      ...
      var firstPeriod = period.FirstIssuedPeriodInTerm
   ...
   */
  override function getModifierReferenceDate(modifierPattern : ModifierPatternBase, modifiable : Modifiable) : Date {
  ...
  private function getReferenceDateForCurrentJobFromPeriod(period : PolicyPeriod, state : Jurisdiction) : Date {
    return getActualPeriodReferenceDate(period, getReferenceDateType(period, state), state)
  }

  private function getActualPeriodReferenceDate(period : PolicyPeriod, dateType : ReferenceDateType, state : Jurisdiction) : Date {
  ...
  private function getReferenceDateForCurrentJobFromLine(policyLine : PolicyLine, state : Jurisdiction) : Date{
  ...
  override function getPeriodReferenceDate(period : PolicyPeriod, dateType : ReferenceDateType, state : Jurisdiction) : Date {
    ...
    var retVal : Date
    if (dateType == TC_EFFECTIVEDATE) {
      retVal = period.EditEffectiveDate
    } else if (dateType == TC_WRITTENDATE) {
      retVal = period.WrittenDate
    } else if (dateType == TC_RATINGPERIODDATE) {
      retVal = null
      if (period.WorkersCompLine != null and state != null) {
        ...
        if (jurisdiction != null) {
          retVal = jurisdiction.getPriorRatingDate(line.Branch.EditEffectiveDate)
        } else {
          retVal = line.Branch.EditEffectiveDate
        }
      }     else if(period.GL7Line != null){
        retVal = period.EditEffectiveDate
      }
      else {
    ...
    return retVal != null ? retVal.trimToMidnight() : Date.Today
  }
}

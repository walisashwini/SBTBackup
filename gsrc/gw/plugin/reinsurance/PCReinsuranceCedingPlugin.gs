      ...
      for (ririsk in r.RIVersionList.AllVersions) {
        ...
        if (ririsk.FacRINeeded.IsPositive) {
          ...
          if (uwIssue == null or (uwIssue.ApprovalValue != null and ririsk.FacRINeeded > new MonetaryAmount(uwIssue.ApprovalValue))) {
            createActivity(period, activityUser, DisplayKey.get("Web.Reinsurance.RIRisk.Validation.InvalidAttachmentForRisk", r), DisplayKey.get("UWIssue.Reinsurance.NetRetentionGreaterThanTarget.ShortDesc"))
            break;
          }
        }
        ...
        if (validator.Result.Errors.HasElements) {
          ...
          createActivity(period, activityUser, DisplayKey.get("Web.Reinsurance.RIRisk.Validation.InvalidAttachmentForRisk", r), desc)
          break;
        }
      }
    })
  ...
  override function logErrorForInvalidPrograms(programs : List<RIProgram>) {
    ...
    programs.each(\ r -> {
      ...
      if (validator.Result.Errors.HasElements) {
        ...
        _logger.warn(DisplayKey.get("Web.Reinsurance.Program.Validation.InvalidProgram", r.DisplayName))
      }
    })
  }
}

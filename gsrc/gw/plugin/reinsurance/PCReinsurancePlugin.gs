...
class PCReinsurancePlugin implements IReinsurancePlugin {
  ...
  override function bindBranch(branch : PolicyPeriod) {
    ...
    branch.RIRiskVersionLists.where(\ vl -> !vl.Bound).each(\ versionList -> {
    ...
    try {
      branch.activateDraftFacAgreements()
        ...
        var activity = branch.Job.createRoleActivity(TC_UNDERWRITER,
                    ...
                    DisplayKey.get("Reinsurance.Activity.FacActivate.Subject"),
                    DisplayKey.get("Reinsurance.Activity.FacActivate.Description"))
    }
  }
  ...
  override function isContactDeletable(contact : Contact) : boolean {
    var agreementParticipantQuery = Query.make(AgreementParticipant)
     .compare("Participant", Equals, contact)
     .select()
    if (agreementParticipantQuery.HasElements) {
      return false
    }
    var agreementBrokerQuery = Query.make(RIAgreement)
  ...
  override function getRisksInALocationRiskGroup(locationRiskGroup : String, date : Date) : List<String> {
    if (locationRiskGroup?.trim()?.length() == 0) {
      ...
      return query.withDistinct(true).select({QuerySelectColumns.path(Paths.make(RIRiskVersionList#RiskNumber))}).transformQueryRow(\row -> row.getColumn(0) as String).toList()
    }
  }
  ...
  private function findOrCreateVersionList(reinsurable : Reinsurable, branch : PolicyPeriod) : RIRiskVersionList {
    ...
    if(versionList == null){
      ...
      if (riskNumber != null) {
        // If RiskNumber is not null, then most likely there is an exising version list
  ...
  protected function createRIRiskInfo(riRisk : RIRisk, date : Date) : RIRiskInfo {
    ...
    riskInfo.Description = DisplayKey.get("Web.Reinsurance.ReinsurableRisk.Description", riRisk.Reinsurable.CoverageGroup.DisplayName, riRisk.Reinsurable.DisplayName)
  ...
  private function include_attachment(attachment : RIAttachment) : boolean {
    // Exclude projected and excluded attachments, draft agreements, and proportional agreements that are not ceded to
    return not attachment.IsProjected
       and attachment.InclusionType != TC_EXCLUDED
       and attachment.Agreement.Status != TC_DRAFT

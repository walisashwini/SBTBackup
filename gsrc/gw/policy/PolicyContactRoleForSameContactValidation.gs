...
class PolicyContactRoleForSameContactValidation extends PCValidationBase {
    ...
    while (pcrs.Count > 1) {
      ...
      for (pcr in pcrs) {
        ...
        if (not mismatchedFields.Empty) {
          Result.addError(pcr, TC_QUOTABLE, DisplayKey.get("Web.PolicyContactRole.Validation.MismatchedFields", pcr.AccountContactRole.AccountContact, (typeof currentPCR).TypeInfo.DisplayName, (typeof pcr).TypeInfo.DisplayName,
  ...
  private function getMismatchedFieldString(pcr1 : PolicyContactRole, pcr2 : PolicyContactRole, field : AccountSyncedField< AccountSyncable, Object >) : String {
  ...
  private function getAccountSyncedFieldsWithDifferentPolicyEntityFieldValues(pcr1 : PolicyContactRole, pcr2 : PolicyContactRole) : List<AccountSyncedField< AccountSyncable, Object >> {
    if (pcr1 == pcr2) {
      return {}
    }

...
class PolicyLocationSearchCriteria extends EntitySearchCriteria<PolicyLocation> {
  ...
  construct() {
    ...
    UnitOfDistance = typekey.UnitOfDistance.get(PCConfigParameters.DefaultProximitySearchUnitOfDistance.Value)
    LOBs = {PCCoercions.makeProductModel<PolicyLinePattern>("BOPLine")}
  }
  ...
  override protected function doSearch() : IQueryBeanResult {
    ...
    if ( !CenterOfSearch.isSuccessfullyGeocoded() ) {
      throw new IllegalArgumentException( DisplayKey.get("Java.ProximitySearch.Error.NonGeocodedCenter") );
    }
  ...
  public function addLob(lob : String) {
    _lobs.add(PCCoercions.makeProductModel<PolicyLinePattern>(lob))
  }
  ...
  protected function prepareQuery(joinTables : Map<String, Table>) : Query<PolicyLocation> {
    if (this.Radius == null) {
      throw new IllegalArgumentException(DisplayKey.get("Java.ProximitySearch.Error.NullArgument", DisplayKey.get("Java.ProximitySearch.Argument.Radius")))
    }
    var query = Query.make(PolicyLocation)
    query.and(\ res -> {
  ...
  private function addPolicyPeriodRestrictions(query : Query<PolicyLocation>) {
    ...
    if (!inForceBound && !inProcessSubs) {
      throw new IllegalArgumentException(DisplayKey.get("Java.ProximitySearch.Error.PolicyLocation.NoPeriodTypeSelected"))
    }

    // The query should contain at least 1 line of business.
    var lobTypes = this.LOBs
    if (lobTypes.IsEmpty) {
      throw new IllegalArgumentException(DisplayKey.get("Java.ProximitySearch.Error.PolicyLocation.EmptyLOBs"))
    }
    ...
    periodTable.and(\ andRes -> {
      ...
      lineTable.compareIn("PatternCode", this.LOBs.map(\ lobType -> lobType.PublicID))

      // Add policy period type restrictions.
      andRes.or(\ orRes -> {
        if (inForceBound) {
          orRes.and(\ res -> {
            PolicyPeriodQueryFilters.boundInForce(res)
          })
        }
        if (inProcessSubs) {
          orRes.and(\ res -> {
  ...
  private function addAccountLocationRestriction(query : Query<PolicyLocation>) : Table<PolicyLocation> {
    ...
    if (this.LimitToSameAccount) {
      ...
      if (center == null) {
        throw new IllegalArgumentException(DisplayKey.get("Java.ProximitySearch.Error.NullArgument", DisplayKey.get("Java.ProximitySearch.Argument.Center")))
      }

      var account = center.Account
      if (account == null) {
        throw new IllegalArgumentException(DisplayKey.get("Java.ProximitySearch.Error.PolicyLocation.NullCenter"))
      }
  ...
  private function addPolicyPeriodAndLOBRestrictions(restriction : Restriction, joinTables : Map<String, Table>) {
    ...
    (periodTable as Table<PolicyPeriod>).and(\ andRes -> {
      
      // Add line of business restriction.
      var lineTable = andRes.subselect("ID", InOperation.CompareIn, PolicyLine, "BranchValue")
      lineTable.compareIn("PatternCode", LOBs.map(\ p -> p.PublicID))
      ...
      andRes.or(\ orRes -> {
        ...
        if (IncludeInProcessNewSubmissions) {
          orRes.and(\ res -> {
            ...
            // Filter down to just open jobs.
            var openStatuses = PolicyPeriodStatus.TF_OPEN.TypeKeys.toSet()
  ...
  override protected property get MinimumSearchCriteriaMessage() : String {
    if (this.Radius == null) {
      return DisplayKey.get("Java.ProximitySearch.Error.NullArgument", DisplayKey.get("Java.ProximitySearch.Argument.Radius"))
    }
    if ( CenterOfSearch == null ) {
       return DisplayKey.get("Java.ProximitySearch.Error.NullArgument", DisplayKey.get("Java.ProximitySearch.Argument.Center" ))
    }
    return null
  }

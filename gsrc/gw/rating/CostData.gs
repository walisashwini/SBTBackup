 ...
 */
@Export
abstract class CostData<S extends entity.Cost, V extends PolicyLine> implements ICostData<S, V> {
  ...
  var _costCode : CostCode as CostCode
  var _billGroup : String as BillGroup
  ...
  public function getCostClassType() : Class {
    return S
  }
  ...
  private class LazyFXConversion extends LockingLazyVar<MonetaryAmount> {
    ...
    function scaleWithLogging(logger: WorksheetLogger) : MonetaryAmount {
      var unconverted = _source()
      if (unconverted == null) {
        return null
      }

      var unscaledAmt = logger.let(DisplayKey.get("Web.Policy.RatingWorksheet.ConvertedTermAmount"),  \ ->
       logger.Term.vr(DisplayKey.get("Web.Policy.RatingWorksheet.TermAmount"), unconverted) *
            logger.Multiplication.vr(DisplayKey.get("Web.Policy.RatingWorksheet.ForeignExchangeRate"), _policyFXRate.Rate))

      var converted = unconverted.ofCurrency(Currency).convertAmount(_policyFXRate)
      assert unscaledAmt == converted.Amount

      var scaledAmt = logger.let(DisplayKey.get("Web.Policy.RatingWorksheet.ConvertedTermAmountRounded"),
          \ -> logger.Term.startFn(gw.rating.flow.CalcRoutineRoundingMethod, "setScale", "Rounding").calcValue(
              gw.rating.flow.CalcRoutineRoundingMethod.setScale(  logger.Term.startArg("Value").calcValue(logger.Term.vr(DisplayKey.get("Web.Policy.RatingWorksheet.ConvertedTermAmount"), converted)),
                  logger.Term.startArg("Scale").calcValue(typekey.RoundingScaleType.get(CurrencyUtil.getStorageScale(SettlementCurrency)+"")),
                  logger.Term.startArg("Mode").calcValue(typekey.RoundingModeType.getRoundingModeTypeForRoundingMode(CurrencyUtil.RoundingMode)))))

      var rescaled = converted.rescale()
      assert scaledAmt == rescaled.Amount
  ...
  construct(effDate : Date, expDate : Date) {
    if (CurrencyUtil.isMultiCurrencyMode()) {
      throw new IllegalStateException(DisplayKey.get("Java.Rating.Error.CostDataInitialization"))
    }
    initializeCostData(effDate, expDate, CurrencyUtil.getDefaultCurrency(), null, null)
  }
  ...
  construct(effDate : Date, expDate : Date, costCode : CostCode) {
    ...
    initializeCostData(effDate, expDate, CurrencyUtil.getDefaultCurrency(), null, costCode)
  }
   ...
   */
  construct(effDate : Date, expDate : Date, asRatedCurrency : Currency, rateCache : PolicyPeriodFXRateCache) {
    initializeCostData(effDate, expDate, asRatedCurrency, rateCache, null)
  }

  construct(effDate : Date, expDate : Date, asRatedCurrency : Currency, rateCache : PolicyPeriodFXRateCache, costCode : CostCode) {
    initializeCostData(effDate, expDate, asRatedCurrency, rateCache, costCode)
  }
   ...
   */
  private function initializeCostData(effDate : Date, expDate : Date, asRatedCurrency : Currency, rateCache : PolicyPeriodFXRateCache, costCode : CostCode) {
    ...
    ActualAdjRate = 1
    ActualBaseRate = 1
    Overridable = costCode.Overrideable?:true
    SubjectToReporting = false
    EffectiveDate = effDate
    ExpirationDate = expDate
    CostCode = costCode
    RateAmountType = costCode.RateAmountType?:TC_STDPREMIUM
    OverrideAdjRate = null
    OverrideAmount = null
    OverrideBaseRate = null
    OverrideTermAmount = null
    ChargePattern = costCode.ChargePatern
    ChargeGroup = null
    BillGroup = null
    OverrideReason = null
    ...
    RoundingLevel = null
    RoundingMode = null
    ProrationMethod = costCode.ProrationMethod?:TC_PRORATABYDAYS
    RateBook = null
  }
  ...
  property get ChargePattern() : ChargePattern {
    return _chargePattern == null
             ? (RateAmountType.isOneOf(RateAmountType.TF_TAXESANDSURCHARGES.TypeKeys)
  ...
  protected function createNewCost(line : V) : S {
    var constructor = S.Type.TypeInfo.getConstructor({PolicyPeriod, Date, Date})
   ...
   * @return
   */
  protected function createCostDataInitWrapper(cost: S) : ICostDataInitWrapper {
    return cost == null ? null : ProxyCostDataInitWrapper.create(cost)
  }
   ...
   */
  override function eachKey(b : block(o : Object)) {
  ...
  function copyOverridesFromCost(c : S) {
    copyOverridesFromCostOverrideWrapper(createCostDataInitWrapper(c))
  }
   ...
   */
  function copyOverridesFromCostOverrideWrapper(c : ICostDataInitWrapper) {
    if (c != null and not (c.CostCurrencyChangedFromPreferredCurrency)) {
      OverrideBaseRate = c.OverrideBaseRate
      OverrideAdjRate = c.OverrideAdjRate
      OverrideAmount = c.OverrideAmount
      OverrideTermAmount = c.OverrideTermAmount
      OverrideReason = c.OverrideReason
      OverrideSource = c.OverrideSource

      var level = c.CostRoundingLevel ?: RoundingLevel ?: c.QuoteRoundingLevel
      var mode = c.CostRoundingMode.ModeValue ?: RoundingMode ?: c.QuoteRoundingMode
      var p = Prorater.forRounding(level, mode, c.ProrationMethod ?: ProrationMethod ?: TC_PRORATABYDAYS)
      if (OverrideAmount != null and (c.CostEffectiveDate != EffectiveDate or c.CostExpirationDate != ExpirationDate)) {
        var proratedAmount = p.prorate(c.CostEffectiveDate, c.CostExpirationDate, EffectiveDate, ExpirationDate, OverrideAmount)
    ...
    if (Plugins.get(IRateRoutinePlugin).worksheetsEnabledForLine(line.PatternCode)) {
      if (_entries.Count > 0) {
        ...
        if (SettlementCurrency != Currency) {
          ...
          log.WorksheetEntryContainer = this

          _standardTermAmountBilling.scaleWithLogging(log)
  ...
  */
  ...
  private function findExistingCost(costVL : gw.pl.persistence.core.effdate.EffDatedVersionList) : S {
    ...
    if (cost == null) { // no cost at this slice date, so clone the first one to preserve the FixedID
      cost = (costVL.AllVersionsUntyped.first() as S).cloneCost() as S
    }
  ...
  protected final function assertKeyType(id : Key, type : Type) {
    if (!gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(type, id.Type)) {

...
class CostDataKey {
  ...
  construct(cd : CostData, keyValues : List<Object>) {
    _cd = cd
    //APD specific change to also include Currency, BillGroup and CostCode to the key
    _keyValues = {cd.Currency, cd.ChargePattern, cd.ChargeGroup, cd.RateAmountType, cd.BillGroup, cd.CostCode}

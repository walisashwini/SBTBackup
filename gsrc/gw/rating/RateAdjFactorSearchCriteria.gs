...
class RateAdjFactorSearchCriteria
{
  ...
  construct( factorName : String, findDate : Date )
  {
    ...
    _findDate   = Coercions.makeDateFrom(findDate.clone())
  }
  ...
  function matchInRange( findNumber : Double, factorState : Jurisdiction ) : BigDecimal
  {
    return matchInRange( null, findNumber, factorState )
  }

  function matchInRange( lookup : String, findNumber : Double, factorState : Jurisdiction ) : BigDecimal
  {
  ...
  private function findAllByLookups( lookup : String, lookup2 : String, factorState : Jurisdiction ) : List<RateAdjFactorExt>
  {
    ...
    if (_lookupCache.containsKey( queryElems )) {
      ...
      var query = Query.make(RateAdjFactorExt)
            .compare(RateAdjFactorExt#factorName, Equals, _factorName)
            .compare(RateAdjFactorExt#factorState, Equals, factorState.Code)
            .compare(RateAdjFactorExt#stringOption, Equals, lookup)
            .compare(RateAdjFactorExt#stringOption2, Equals, lookup2)
            .and(\ andRestriction -> andRestriction
              .or(\orRestriction -> {
                orRestriction.compare(RateAdjFactorExt#effDate, Equals, null)
                orRestriction.compare(RateAdjFactorExt#effDate, LessThanOrEquals, _findDate)
              })
              .or(\orRestriction -> {
                orRestriction.compare(RateAdjFactorExt#expDate, Equals, null)
                orRestriction.compare(RateAdjFactorExt#expDate, GreaterThan, _findDate)
              })
            ).select()
      result = query.toList()
   ...
   * then the one with a PublicID later in the alphabet.
   */
  private function findInRange( lookup : String, findNumber : Double, factorState : Jurisdiction ) : RateAdjFactorExt
  {
    ...
    if (_rangeCache.containsKey( queryElems )) {
      ...
      var factorQuery = Query.make(RateAdjFactorExt)
            ...
            .compare(RateAdjFactorExt#factorState, Equals, factorState.Code)
            .compare(RateAdjFactorExt#stringOption, Equals, lookup)
            .compare(RateAdjFactorExt#minNumber, LessThanOrEquals, findNumber as BigDecimal)
            .compare(RateAdjFactorExt#maxNumber, GreaterThanOrEquals, findNumber as BigDecimal)
            .and(\andRestriction -> andRestriction
                .or(\orRestriction -> {
                  orRestriction.compare(RateAdjFactorExt#effDate, Equals, null)
                  orRestriction.compare(RateAdjFactorExt#effDate, LessThanOrEquals, _findDate)
  ...
  private function getFirstResult( factorQuery : IQueryBeanResult<RateAdjFactorExt> ) : RateAdjFactorExt
  {
    if ( factorQuery != null )
    {
      // get first the latest one, with the earliest PublicID
      factorQuery.orderByDescending(QuerySelectColumns.path(Paths.make(RateAdjFactorExt#effDate))).thenBy(QuerySelectColumns.path(Paths.make(RateAdjFactorExt#PublicID)))

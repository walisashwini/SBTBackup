...
class RIProgramValidation extends PCValidationBase {
  ...
  function validateDates() {
    ...
    if (_program.EffectiveDate != RIUtil.adjustEffectiveTimeForRI(_program.EffectiveDate)) {
        Result.addError(_program, TC_DEFAULT, 
            DisplayKey.get("Web.Reinsurance.Program.Validation.EffectiveDateNormalization", _program))
    ...
    if (_program.ExpirationDate != RIUtil.adjustEffectiveTimeForRI(_program.ExpirationDate)) {
        Result.addError(_program, TC_DEFAULT, 
            DisplayKey.get("Web.Reinsurance.Program.Validation.ExpirationDateNormalization", _program))
        // should we fix this one so it doesn't generate anything spurious?
        // _program.setExpirationDateWithDefaultTime(_program.ExpirationDate)
    }
    if (_program.ExpirationDate < _program.EffectiveDate) {
      Result.addError(_program, TC_DEFAULT, 
          DisplayKey.get("Web.Reinsurance.Program.Validation.ExpiresBeforeEffective", _program))
    }
  }
  ...
  public function validateProgram(){
    ...
    for(treaty in _program.Treaties){
      ...
      if (treaty.Currency != _program.Currency) {
        Result.addError(_program, TC_DEFAULT,DisplayKey.get("Web.Reinsurance.Program.Validation.InconsistentCurrency", treaty.Name, _program.Name))
      ...
      if (treaty.EffectiveDate.after(_program.EffectiveDate) or treaty.ExpirationDate.before(_program.ExpirationDate)){
        Result.addError(_program, TC_DEFAULT,DisplayKey.get("Web.Reinsurance.Program.Validation.InvalidTreatyEffectiveDates", treaty.Name))
      }
      ...
      if(treaty typeis PerRisk){
        if (_program.SingleRiskMaximum != null and (!(treaty typeis NetExcessOfLossRITreaty))
          ...
          Result.addError(_program, TC_DEFAULT,DisplayKey.get("Web.Reinsurance.Program.Validation.LimitTooHigh", treaty.Name))
        }
      }
      //check all treaties active if program is active
      if (_program.Active and not treaty.Active) {
        Result.addError(_program, TC_DEFAULT,DisplayKey.get("Web.Reinsurance.Program.Validation.ActiveProgramDraftTreaty", _program.Name, treaty.Name))
      }
    }
    ...
    for(covGroup in _program.CoverageGroups){
      if(not _program.Treaties.allMatch(\ r -> r.CoverageGroups.contains(covGroup) )){
        Result.addError(_program, TC_DEFAULT,DisplayKey.get("Web.Reinsurance.Program.Validation.InvalidCovGroup", covGroup))
      }
    }

    if(_program.CoverageGroups.Empty){
      Result.addError(_program, TC_DEFAULT,DisplayKey.get("Web.Reinsurance.Program.Validation.MissingCoverageGroup"))
    }
    ...
    if (impliedNetRetention != null && _program.TargetMaxRetention != null &&
       ...
       Result.addError(_program, TC_DEFAULT,DisplayKey.get("Web.Reinsurance.Program.Validation.NetRetentionGreaterThanTargetNetRetention"))
    }
  }
  ...
  private function validateTreaties(){
    for(treaty in _program.Treaties){
      try {
        ...
        Result.addError(_program, TC_DEFAULT, DisplayKey.get("Web.Reinsurance.Program.Validation.InvalidTreaty", treaty.Name))
      }
    }
  }
  ...
  */
  ...
  private function checkForOverlaps(treaties : RIAgreement[], asErrors : boolean) {
     ...
     errors.each(\ e -> {
         if (asErrors) {
           Result.addError(_program, TC_DEFAULT, e)
         } else {
           Result.addWarning(_program, TC_DEFAULT, e)             
         }
     })
  }
    ...
    while (treatyList.Count > 1) {
      ...
      for (treaty in treatyList) {
        if (treaty.MaxRetention != compareToTreaty.MaxRetention) {
          Result.addError(_program, TC_DEFAULT,DisplayKey.get("Web.Reinsurance.Agreement.Verify.SurplusMaxRetentionNotEqual", treaty.Name, compareToTreaty.Name)) 
        }
      }
    }
  }
    ...
    for (quotaShare in quotaShareList) {
      for (surplus in surplusList) {
        if (!quotaShare.CoverageLimit.equals(surplus.MaxRetention)) {
          Result.addWarning(_program, TC_DEFAULT, DisplayKey.get("Web.Reinsurance.Agreement.Verify.SurplusMaxRetNotEqualToQuotaShareLimit", surplus.Name, quotaShare.Name))
        }
      }
    }
  }

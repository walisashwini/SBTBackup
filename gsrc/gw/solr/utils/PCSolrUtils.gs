 ...
 /**
  ...
  static function getAllRelatedPolicyPeriods(contact : Contact) : Collection<PolicyPeriod> {
    if (contact.New) {
      ...
      //noinspection SelectWithLimitingFunctions We intentionally allow customers to post-filter here
  ...
  static function shouldIndexPeriod(period : PolicyPeriod) : boolean{
    ...
    if (period.Status != TC_BOUND) {
      return !(period.Job typeis Cancellation)
          and !(period.Preempted) 
          and !(PolicyPeriodStatus.TF_INELIGIBLEFORSOLRINDEX.TypeKeys.contains(period.Status))
    }

...
class ReinsuranceUIHelper {
  ...
  public static function makeAllCheckedAgreementsActive(checkedValues : RIAgreement[]) {
    ...
    if (errorFound) {
      gw.api.util.LocationUtil.addRequestScopedInfoMessage(DisplayKey.get("Web.Reinsurance.Agreement.Verify.MakeActiveFailedError"))
    }
  }
  ...
  public static function attachFacs(ririsk : RIRisk, agreements : RIAgreement[]) {
    for (agreement in agreements) {
      if(agreement.Status == TC_ACTIVE or agreement.IsValid){
        ...
        gw.api.util.LocationUtil.addRequestScopedErrorMessage(DisplayKey.get("Web.Reinsurance.Validation.CannotAddInvalidFac", agreement.AgreementNumber))
      }
    }
  }
  ...
  public static function doProximitySearch(slice : Reinsurable) {
    ...
    if (not locRisk.AccountLocation.SuccessfullyGeocoded) {
      throw new DisplayableException(DisplayKey.get("Web.Reinsurance.ProximitySearch.LocationNotGeocoded"))
    }
  ...
  public static function delete(program : entity.RIProgram) {
    ...
    if (program.isAttachedToAnyRIRisk()) {
      throw new DisplayableException(DisplayKey.get("Web.Reinsurance.Program.DeleteError"))
    } else {

package gw.webservice.pc.pc1000
 ...
 */
@WsiWebService("http://guidewire.com/pc/ws/gw/webservice/pc/pc1000/MessagingToolsAPI")
...
class MessagingToolsAPI {
  ...
  function retryRetryableSomeErrorMessages(destID : int, retryLimit : int) : boolean {
    checkForInvalidDestinationAndThrowIfNecessary(destID)
    if(retryLimit < 0) {
      throw new RequiredFieldException(DisplayKey.get("Webservice.Error.MissingRequiredField", "retryLimit"))
    }
  ...
  private function checkForInvalidDestinationAndThrowIfNecessary(destinationId: int) {
    if (destinationId <= 0) {
      throw new IllegalArgumentException(DisplayKey.get("Webservice.Messaging.DestinationIdInvalid", destinationId))
    }
  ...
  private function getDelegate() : PCMessagingToolsImpl {

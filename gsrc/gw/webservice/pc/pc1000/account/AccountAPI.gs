package gw.webservice.pc.pc1000.account
 ...
 */
@Export
@WsiWebService("http://guidewire.com/pc/ws/gw/webservice/pc/pc1000/account/AccountAPI")
class AccountAPI {
  ...
  function hasActivePolicies(accountNumber : String) : boolean {
    ...
    var issuedPolicies : IQueryBeanResult<PolicyPeriodSummary> = account.IssuedPolicies
  ...
  function findAccounts(accountSearchInfo : gw.webservice.pc.pc1000.account.AccountSearchInfo) : String[] {
    ...
    SOAPUtil.convertToSOAPException(PCLoggerCategory.ACCOUNT_API, \ -> {
        var accountQueryBuilder = new AccountQueryBuilder()
          .withAccountStatus(AccountStatus.get(accountSearchInfo.AccountStatus))
          ...
          .withAccountOrgType(AccountOrgType.get(accountSearchInfo.AccountOrgType))
          .withOriginationDate(accountSearchInfo.OriginationDate)
          .withPrimaryLanguage(LanguageType.get(accountSearchInfo.PrimaryLanguage))
        ...
        if (accountSearchInfo.FirstName.NotBlank
          ...
          var contactQueryBuilder = new ContactQueryBuilder()
              ...
              .withFirstNameKanjiRestricted(accountSearchInfo.FirstNameKanji, StartsWithIgnoringCase)
              ...
              .withStateDenorm(State.get(accountSearchInfo.State))
          if (accountSearchInfo.AddressLine1.NotBlank
            ...
            contactQueryBuilder.withPrimaryAddress(new AddressQueryBuilder()
                ...
                .withAddressLine1KanjiStarting(accountSearchInfo.AddressLine1Kanji)
                .withAddressLine2KanjiStarting(accountSearchInfo.AddressLine2Kanji)
                .withCountry(Country.get(accountSearchInfo.Country))
        ...
        foundAccountNumbers = (accountQueryBuilder.build() as ISelectQueryBuilder<Account>).select({QuerySelectColumns.path(Paths.make(Account#AccountNumber))}).transformQueryRow(\row -> row.getColumn(0) as String).toTypedArray()
    })
  ...
  function addAccount(externalAccount : gw.webservice.pc.pc1000.gxmodel.accountmodel.types.complex.Account) : String {
  ...
  function addLocationToAccount(accountNumber : String,
      externalLocation : gw.webservice.pc.pc1000.gxmodel.accountlocationmodel.types.complex.AccountLocation) : String {
    SOAPUtil.require(accountNumber, "accountNumber")
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      gw.webservice.pc.pc1000.gxmodel.SimpleValuePopulator.populate(externalLocation, accountLocation)
  ...
  function moveAccountContactToAccount(accountNumber : String, accountContactID : String) : String {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      if (accountContact.hasRole(TC_ACCOUNTHOLDER)) {
        throw new SOAPException(DisplayKey.get("AccountAPI.AddContact.Error.CannotAddAccountHolder"))
      }
  ...
  function addContactToAccount(externalContact : gw.webservice.pc.pc1000.gxmodel.contactmodel.types.complex.Contact,
  ...
  function updateAccountContactsAddressFields(addressPublicID : String,
      externalAddress : gw.webservice.pc.pc1000.gxmodel.addressmodel.types.complex.Address) : String {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      gw.webservice.pc.pc1000.gxmodel.SimpleValuePopulator.populate(externalAddress, address)
    })
  ...
  function findContactOnAccount(accountNumber : String, contactDisplayName : String) : String {
    ...
    if (acctContact.Count == 0) {
      throw new SOAPException(DisplayKey.get("AccountAPI.Contact.Error.NotFound", contactDisplayName))
    } else if (acctContact.Count > 1) {
      throw new SOAPException(DisplayKey.get("AccountAPI.Contact.Error.MultipleMatches", contactDisplayName))
    }
  ...
  function transferPolicies(policyPublicIDs : String[], fromAccountNumber : String, toAccountNumber : String) {
    ...
    if (fromAccountNumber == toAccountNumber) {
      throw new IllegalArgumentException(DisplayKey.get("AccountAPI.TransferPolicies.Error.SameAccount"))
    }
  ...
  function addAndAutoAssignActivityForAccount(accountNumber : String,
        ...
        externalActivity : gw.webservice.pc.pc1000.gxmodel.activitymodel.types.complex.Activity) {
    SOAPUtil.require(accountNumber, "accountNumber")
    ...
    if(activityPattern == null){
      throw new BadIdentifierException(DisplayKey.get("PolicyAPI.ActivityPattern.NotFound", activityType, activityPatternCode))
    }
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      gw.webservice.pc.pc1000.gxmodel.SimpleValuePopulator.populate(externalActivity, accountActivity)
    })
  }
  ...
  private function findRequiredAccount(accountNumber : String) : Account{
    var account = Account.finder.findAccountByAccountNumber(accountNumber)
    if(account == null){
      throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindAccount", accountNumber))
    }
    return account
  }
  ...
  public function activateAccountContact(accountNumber : String, contactPublicID : String, activate : boolean) : boolean {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      if (accountContacts.Count == 0) {
        throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindContactByPublicID", contactPublicID))
      }
      ...
      if (account.AccountHolder.AccountContact == accountContact)
        throw new SOAPException(DisplayKey.get("Webservice.Error.CannotDeactivateAccountHolder"))
  ...
  function addAccountContactRole(
        ...
        accountContactRoleInfo : gw.webservice.pc.pc1000.gxmodel.accountcontactrolemodel.types.complex.AccountContactRole) : String {
    SOAPUtil.require(accountNumber, "accountNumber")
    SOAPUtil.require(contactPublicID, "contactPublicID")
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      if (accountContactRoleInfo.Subtype == null) {
        throw new SOAPException(DisplayKey.get("AccountAPI.Error.AccountContactRoleInfo.Type.Required"))
      }
      ...
      if (accountContactRoleType == typekey.AccountContactRole.TC_ACCOUNTHOLDER) {
        throw new SOAPException(DisplayKey.get("AccountAPI.Error.Adding.AccountHolder.Not.Allowed"))
      }
  ...
  function updateAccountContactRoleFields(
      accountNumber : String,
      contactPublicID : String,
      contactRoleInfo : gw.webservice.pc.pc1000.gxmodel.accountcontactrolemodel.types.complex.AccountContactRole) : String {
    SOAPUtil.require(accountNumber, "accountNumber")
    SOAPUtil.require(contactPublicID, "contactPublicID")
  ...
  function deleteAccountContact(accountNumber : String, contactPublicID : String) : String {
    ...
    if (accountContact == null) {
      throw new IllegalArgumentException(DisplayKey.get("Webservice.Error.ContactNotOnAccount", contact, account))
    }

    if (account.AccountHolderContact == contact) {
      throw new IllegalArgumentException(DisplayKey.get("Webservice.Error.CannotRemoveAccountHolder", contact, account))
    }

    if (accountContact.Roles.hasMatch(\ a -> not a.canRemove())) {
      throw new IllegalStateException(DisplayKey.get("Webservice.Error.ContactToRemoveHasAtLeastOneRoleInUse", contact, accountContact.Roles.firstWhere(\ a -> not a.canRemove())))
    }
    Transaction.runWithNewBundle(\ bundle -> {
  ...
  public function deleteAccountContactRole(
    ...
    if (roleTypeName ==
      ...
      throw new SOAPException(DisplayKey.get("AccountAPI.Error.AccountContactRole.CannotDeleteAccountHolderRole"))
    }
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      SOAPUtil.convertToSOAPException(PCLoggerCategory.ACCOUNT_API, \ -> {  // unnecessary, but kept for backwards-compatibility
        ...
        if (not accountContactRole.canRemove()) {
          throw new gw.api.util.DisplayableException(DisplayKey.get("AccountAPI.Error.AccountContactRole.CannotRemoveInUseRole", accountContactRole))
        }
  ...
  private function getPolicyByPublicID(policyId : String) : Policy {
    ...
    if (policy == null) {
      throw new DataConversionException(DisplayKey.get("AccountAPI.Error.InvalidPolicyPublicID", policyId))
    }
    return policy
  }
  ...
  private function getAccountContactRole(
    ...
    if (roleType == null) {
     throw new BadIdentifierException(DisplayKey.get("AccountAPI.Error.Unknown.AccountContactRole.Type",  roleType))
    }
    ...
    if (results.Empty) {
     throw new BadIdentifierException(DisplayKey.get("AccountAPI.Error.AccountContactRole.NotFound", 
       accountNumber, contactPublicID, roleType))
    }
    if (results.Count > 1) {
     throw new BadIdentifierException(DisplayKey.get("AccountAPI.Error.AccountContactRole.FoundMultiple", 
       accountNumber, contactPublicID, roleType))
    }

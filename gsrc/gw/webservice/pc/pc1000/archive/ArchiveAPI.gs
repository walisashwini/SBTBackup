package gw.webservice.pc.pc1000.archive
 ...
 *
 */
@WsiWebService("http://guidewire.com/pc/ws/gw/webservice/pc/pc1000/archive/ArchiveAPI")
@Export
class ArchiveAPI {
  ...
  function requestRestore(policyNumber : String, effectiveDate : Date) {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      period.PolicyTerm.createRestoreRequest(User.util.CurrentUser, DisplayKey.get("ArchiveAPI.RequestRestore.Reason"), false)
    })
  ...
  private function findPolicy(policyNumber : String) : Policy {
    ...
    if (resultPolicy.Count == 0) {
      throw new BadIdentifierException(DisplayKey.get("ArchiveAPI.Error.InvalidPolicyNumber", policyNumber))
    }
  ...
  private function findLatestBoundPeriodInPolicy(policyNumber : String, effectiveDate : Date) : PolicyPeriod {
    var period = Policy.finder.findPolicyPeriodByPolicyNumberAndAsOfDate(policyNumber, effectiveDate)
    if (period == null) {
      throw new BadIdentifierException(DisplayKey.get("ArchiveAPI.Error.InvalidPolicyNumberAndEffectiveDate", policyNumber, effectiveDate))
    }
    return period
  }
}

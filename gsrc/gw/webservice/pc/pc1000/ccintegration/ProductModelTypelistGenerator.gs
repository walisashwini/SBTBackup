...
uses gw.api.productmodel.ProductModelDisplayKey
uses gw.api.system.PCLocaleCatalog
...
class ProductModelTypelistGenerator {
  ...
  static final var _covTermPatternCCAddedTypelistCategories : String[] = {"LossCause"}
  ...
  static final var _regexTypeCodeCodeInComments = "\\w*<typecode\\s*code=\\\"\\w+\\\""
  static final var _regexCategoryCodeCodeInComments = "\\w*<category\\s*code=\\\"(\\w+|\\w+-\\w+)\\\""

  // Exception report
  var _exceptionFile : FileOutputStream
  var exceptionFileName = "productModelGenReport.txt"

  static final var _commentText = "The product model generator (PMG) creates default entries in four typelist files (CoverageType.ttx, CoverageSubType.ttx, ExposureType.ttx, and LossPartyType.ttx) based on the product model as defined in PolicyCenter. They associate every Coverage with an ExposureType. " +
      "If you want to disallow exposure creation against a specific Coverage, you should comment out the related entries in all three files for that Coverage. Do not just delete these entries, because then future runs of the PMG will recreate them."
  ...
  private function genCCProductModelXML() : XMLNode {
    ...
    var filesByLocale = PCLocaleCatalog.getFilesByLocale(new File(dirPrior), FileCCTypeListProperties)

    for (fileEntry in filesByLocale.entrySet()) {
      var languageCode = fileEntry.getKey()
      var languageType = LanguageType.get(languageCode)
      if (languageType != null) {
        var localizedFile = fileEntry.getValue()
        var propertyFile = TypelistL10NPropertiesGateway.readFrom(localizedFile)
        LanguageTypeListMap.put(languageType, propertyFile)
      } else {
        writeExceptionReport("Warning: Language code [" + languageCode + "] could not be mapped to any language type.")
      }
    }
    ...
    //Get all the typecodes codes from CoverageSubtype.ttx that have been commented out.
    //The GW parser will miss the typecodes that have been commented out at the bottom of the file.
    //Get the codes using the GW parser, and also using Java's Dom parser
    var allTypeCodeCodesInCommentsFromGWParser = getAllTypeCodeCodesInComments(priorSubCovTypeXML)
    var commentsAtEndOfCoverageSubTypeFile = parseCoverageSubTypeWithDomToFindMissedCommentString(allTypeCodeCodesInCommentsFromGWParser)
    var allMissingTypeCodes = getAllCodesWithRegexMatcher(commentsAtEndOfCoverageSubTypeFile, _regexTypeCodeCodeInComments)
    var allTypeCodesCodesCoverageSubtypeInComments = new LinkedHashSet<String>(allTypeCodeCodesInCommentsFromGWParser)
    allTypeCodesCodesCoverageSubtypeInComments.addAll(allMissingTypeCodes)
    ...
    for (lob in LOBProductMap.Keys) {
      ...
      var LOBNode = createTypeCodeNode(LOBCodeRoot, callTypecodeMappingAndLogWarningMessages(lob), lob.CodeIdentifier , lob.Name, lob.Description)

      // Add category elements for each product that the LOB is used for
      var lobProductEntry = (LOBProductMap.get(lob) as ArrayList<Product>)
      if (lobProductEntry != null) {
        for (prod in lobProductEntry) {
          createCategoryNode(LOBNode, "PolicyType", CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(prod))
        }
      }
      ...
      var lobLossEntry = (LOBLossTypeMap.get(CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(lob)) as ArrayList<String>)
      if (lobLossEntry != null) {
        ...
        writeExceptionReport("Warning: LOB Code [" + lob.PublicID + "] is not mapped to any loss types.")
      }
    ...
    for (prod in gw.api.productmodel.ProductLookup.getAll() ) {
      ...
      var PolicyTypeNode = createTypeCodeNode(PolicyTypeRoot, callTypecodeMappingAndLogWarningMessages(prod), prod.CodeIdentifier , prod.Name, prod.Description)

      var previousCoverageTypes = priorCoverageMap[prod.PublicID]

      // Add category elements for each LOB
      for (lob in prod.LinePatterns) {
        createCategoryNode(PolicyTypeNode, "LOBCode", CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(lob))
        ...
        for (cov in lob.CoverageCategories.flatMap(\ cat -> cat.CoveragePatterns)) {
          createCategoryNode(PolicyTypeNode, "CoverageType", CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(cov))
          if (previousCoverageTypes != null) previousCoverageTypes.remove(cov.PublicID) //remove anything we are getting from pc; pc is the authoriative source
        }
      }

      // Add category elements for any coverages for this product that were previously included
      ...
      for (policyNode in priorPolicyTypeXML.Children) {
        ...
        if (isMatchingTypecode(policyNode, CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(prod))) {
    ...
    var firstPartyCovs = new LinkedHashSet<String>()
    var thirdPartyCovs = new LinkedHashSet<String>()

    // Create the XML Nodes for the CoverageTypes
    for (lob in LOBProductMap.Keys) {
      for (cov in lob.CoverageCategories.flatMap(\ cat -> cat.CoveragePatterns) ) {
        var code = callTypecodeMappingAndLogWarningMessages(cov)
        // Create a typelist node for the coverage
        var CovTypeNode = createTypeCodeNode(CoverageTypeRoot, code, cov.CodeIdentifier,  cov.Name, cov.Description)
        ...
        for (prod in LOBProductMap.get(lob)) {
          createCategoryNode(CovTypeNode, "PolicyType", CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(prod))
        }
        ...
        for (covTerm in cov.CovTerms) {
          ...
          var CovTermNode = createTypeCodeNode(CovTermPatternRoot, callTypecodeMappingAndLogWarningMessages(covTerm), covTerm.CodeIdentifier, covTerm.Name, covTerm.Description)
          ...
          createCategoryNode(CovTermNode, "CoverageType", CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(cov))

          // Add a category node to indicate that this CovTerm came from PolicyCenter
          createPCSourceCategoryNode(CovTermNode)

          // Add a category node to identify the model type field on CovTermPattern
          if (covTerm?.ModelType?.Code != null) {
            createCategoryNode(CovTermNode, "CovTermModelType", covTerm.ModelType.Code)
          }
        }
        ...
        var subtypeNodes = priorCovSubcovMap.get(CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(cov))

        //The Coverage typecode has now been commented out, so no longer add it back to the typelist
        if (subtypeNodes == null and not allTypeCodesCodesCoverageSubtypeInComments.contains(code)) {
          ...
          createCategoryNode(CovTypeNode, "CoverageSubtype", CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(cov))

          // Create a typelist node for the coverage subtype
          var subCovNode = createTypeCodeNode(CoverageSubtypeRoot, code, cov.CodeIdentifier, cov.Name, cov.Description)
          ...
          createCategoryNode(subCovNode, "CoverageType", CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(cov))
          ...
          if (mapNewCovToGeneralDamageExpo and generalDamageExposureNode != null) {
            ...
            writeExceptionReport("Warning: A new default coverage subtype was created for [" + cov.PublicID + "].  It is not mapped to any exposure types.")
          }

          // Add the sub-coverage code to either the first party or third party list
          if (cov.CoveredPartyType==TC_FIRSTPARTY) {
            firstPartyCovs.add(cov.PublicID)
          } else {
            thirdPartyCovs.add(cov.PublicID)
          }
    ...
    var mapTypeCodeCodeToCategoryCommentNode = parseCoverageTypeWithDomMapCommentsToTypeCodeCode()
    fixUpCommentsForPCTypecodesForCategory(CoverageTypeRoot, mapTypeCodeCodeToCategoryCommentNode)
    copyAllNonPCTypecodesForCategory(mapTypeCodeCodeToCategoryCommentNode, CoverageTypeRoot, priorCovTypeXML)

    copyAllNonPCCovSubtypes(CoverageSubtypeRoot, priorSubCovTypeXML, priorCovTypeXML)
    if (commentsAtEndOfCoverageSubTypeFile.HasContent) {
      CoverageSubtypeRoot.Children.last().setComment(commentsAtEndOfCoverageSubTypeFile)  //If there were commented out typecodes at the end of the CoverageSubType.ttx add them back
    }
    ...
    copyAllNonPCTypeCodesAndAddCCTypelistCategoryForPCTypeCodes(CovTermPatternRoot, priorCovTermPatternXML)
    ...
    var priorLossPartyCommentMap = parseXMLWithDomAndMapCategoryCommentsToTypeCode()
    ...
    var firstPartyNode = createTypeCodeNode(LossPartyTypeRoot, "insured", "insured", "Insured's loss", "Insured's loss")
    var insuredComment = priorLossPartyCommentMap.get("insured")
    var insuredCommentString = insuredComment?.join("\n    ")
    var thirdPartyComment = priorLossPartyCommentMap.get("third_party")
    var thirdPartyCommentString = thirdPartyComment?.join("\n    ")
    var allLossPartyCommentCodes = getAllCodesWithRegexMatcher(insuredCommentString, _regexCategoryCodeCodeInComments)
    allLossPartyCommentCodes.addAll(getAllCodesWithRegexMatcher(thirdPartyCommentString, _regexCategoryCodeCodeInComments))

    //Create all the <category> nodes that have not been commented out for firstPartyCov
    for (code in firstPartyCovs) {
      if (not allLossPartyCommentCodes.contains(code)) {
        createCategoryNode(firstPartyNode, "CoverageSubType", code)
      }
    }

    //Add to the first <category> all the commented out <categories>
    if (insuredComment != null and insuredCommentString.HasContent) {
      firstPartyNode.Children.first().setComment(insuredCommentString)
    }

    var thirdPartyNode = createTypeCodeNode(LossPartyTypeRoot, "third_party", "third_party", "Third-party liability", "Third-party liability")

    //Create all the <category> nodes that have not been commented out for thirdPartyCov
    ...
    //Add to the first <category> all the commented out <categories>
    if (thirdPartyComment != null and thirdPartyCommentString.HasContent) {
      thirdPartyNode.Children.first().setComment(thirdPartyCommentString)
    }
  ...
  /**
   * Parse through CoverageType.ttx and map all the <typecode code> with <categories> that have
   * been commented out.
   * @return Map of <typecode code> to list of commented out <category> nodes
   */
  function parseCoverageTypeWithDomMapCommentsToTypeCodeCode() : Map<String, Set<Node>> {
    var doc = parseFileIntoDocument(dirPrior + _fileCCCoverageType)
    if (doc == null) {
      return {}
    }

    var commentNodes = findCommentNodes(doc.ChildNodes)

    var filterCatgeoryCommentNodes = commentNodes.where(\elt -> {
      var pattern = Pattern.compile(_regexCategoryCodeCodeInComments, Pattern.MULTILINE)
      var matcher = pattern.matcher(elt.TextContent)
      return matcher.find()
    })

    var mapTypeCodeCodeToCommentedCategorynodes = new HashMap<String, Set<Node>>()
    filterCatgeoryCommentNodes.each(\elt -> {
      var parentNodeCode = elt.ParentNode.Attributes.getNamedItem("code").TextContent
      var nodes = mapTypeCodeCodeToCommentedCategorynodes.getOrDefault(parentNodeCode, new HashSet<Node>())
      nodes.add(elt)
      mapTypeCodeCodeToCommentedCategorynodes.put(parentNodeCode, nodes)
    })

    return mapTypeCodeCodeToCommentedCategorynodes
  }

  /**
   * The GW XML parser doesn't find comments at the end of the file
   * The Java Dom parser does.  Re-parses CoverageSubtype.ttx using
   * the Dom parser to find all comments, and then returns all
   * comments that was not found in the GW XML parser
   * @param commentCodes In the comments
   * @return
   */
  function parseCoverageSubTypeWithDomToFindMissedCommentString(commentCodes : Set<String>) : String {
    var doc = parseFileIntoDocument(dirPrior + _fileCCCoverageSubtype)
    if (doc == null) {
      return ""
    }
    ...
    //we are only getting the first few words of comment text to match
    var matchingCommentText = _commentText.substring(0, 33)
    var missingCommentNodes = commentNodes.where(\node -> not (commentCodes.hasMatch(\commentCode -> node.TextContent.contains(commentCode) || node.TextContent.contains(matchingCommentText))))

    return missingCommentNodes.map(\elt -> elt.TextContent).join("\n")
  }

  function parseFileIntoDocument(fileName : String) : org.w3c.dom.Document {
  ...
  function findCommentNodes(nodes: NodeList) : Set<Node> {
    var commentNodes = new LinkedHashSet<Node>()
    for (0..|nodes.Length index i) {
      ...
      if (node.hasChildNodes()) {
        commentNodes.addAll(findCommentNodes(node.ChildNodes))
      }
    }
  ...
  /**
   * Parses LossPartyType.ttx for all the <category> that have been commented out.
   * @return a map of <typecode code> to a set of all commented out <categories>
   */
  function parseXMLWithDomAndMapCategoryCommentsToTypeCode() : Map<String, Set<String>> {
    var doc = parseFileIntoDocument(dirPrior + _fileCCLossPartyType)
    if (doc == null) {
      return {}
    }
    ...
    findAllCommentNodes(doc.ChildNodes, map)

    return map
  }

  static function isCommentNode(node : Node) : boolean {
    return node.NodeType == node.COMMENT_NODE
  }

  /**
   * Recursive function call to find all nodes that are a comment node.
   * It will then map the <typecode code> to all a set of all commemtend out <category code>
   * @param nodes the nodes to find comments
   * @param map the <typecode code> to all a set of all commemtend out <category code>
   */
  static function findAllCommentNodes(nodes: NodeList, map : Map<String, Set<String>>) {
    for (0..|nodes.Length index i) {
      var node = nodes.item(i)
      ...
      if (isCommentNode(node)) {
        ...
        if (parentNode.hasAttributes()) {
          var code = parentNode.getAttributes().getNamedItem("code").getTextContent()
          var setOfComments = map.getOrDefault(code, new LinkedHashSet<String>())
          setOfComments.add(node.TextContent)
          map.put(code, setOfComments)
        }
      }

      if (node.hasChildNodes()) {
        findAllCommentNodes(node.ChildNodes, map)
      }
    }
  }

  public function typeListFileForLanguage(language : LanguageType) : String {
    var fileName = PCLocaleCatalog.localizedFileName(FileCCTypeListProperties,language.Code)

    return dirNew + File.separator + fileName
  }
  ...
  private function calcLOBProductMap() : HashMap<PolicyLinePattern, List<Product>> {
    var LOBProductMap = new HashMap<PolicyLinePattern, List<Product>>()
    for (prod in gw.api.productmodel.ProductLookup.getAll() ) {
      for (lob in prod.LinePatterns) {
        // Look for an entry for this LOB already
        var lobEntry = (LOBProductMap.get(lob) as ArrayList<Product>)
        // If none, create an entry for it
        if (lobEntry == null) {
          lobEntry = new ArrayList<Product>()
  ...
  function getAllTypeCodeCodesInComments(xmlNode : XMLNode) : Set<String> {
    var setOfCodes = new HashSet<String>();

    for (node in xmlNode?.Children) {
      if (node.Comment?.HasContent) {
        setOfCodes.addAll(getAllCodesWithRegexMatcher(node.Comment, _regexTypeCodeCodeInComments))
      }
    }
    return setOfCodes
  }
  ...
  /**
   * This will return you a list of the words inbetween the code="XXX" using a regex
   * expression that will match *code=XXX"*
   * @param comment the string we will be searching through
   * @param regex expression
   * @return all of the found elements in comment
   */
  function getAllCodesWithRegexMatcher(comment : String, regex : String) : Set<String> {
    if (comment == null or not comment.HasContent) {
      return {}
    }
    ...
    while (matcher.find()) {
      var foundString = matcher.group(0)
      setOfCodes.add(GosuStringUtil.substringBetween(foundString, "code=\"", "\""))
    }
  ...
  private function createTypeCodeNode(parent : XMLNode, ccCode : String, codeIdentifier : String, name : String, desc : String) : XMLNode {
    var node = new XMLNode()
    node.setQName(new QName("http://guidewire.com/typelists", "typecode"))
    parent.addChild(node)
    node.setAttributeValue("name", name)
    node.setAttributeValue("desc", desc)
    node.setAttributeValue("code", ccCode)
    node.setAttributeValue("identifierCode", codeIdentifier)
  ...
  private function isPCCovSubtypeNode(node : XMLNode, PCCovs : XMLNode) : Double {
  ...
  /**
   * The CoverageType nodes have all been completed.  For all the PC nodes we need to add back
   * the commented out <categories>.  Also the GW parser won't recognize if the last <category>
   * is commented out, and will re-add into the typecode.  This will first put all the commented
   * <cateogries> at the top, and remove the <category> that was commented out right before the
   * enclosure of </typecode>
   *
   * @param coverageRootNode The start XMLNode of CoverageType.ttx
   * @param mapTypeCodeCodeToComments The map of <typecode code> to all commented out <category> nodes
   */
  function fixUpCommentsForPCTypecodesForCategory(coverageRootNode : XMLNode, mapTypeCodeCodeToComments : Map<String, Set<Node>>) {
    var codesWithComments = mapTypeCodeCodeToComments.keySet()

    for (node in coverageRootNode.Children) {
      var nodeCode = node.Attributes.get("code")
      if (isPCNode(node) and codesWithComments.contains(nodeCode)) {
        var commentString = mapTypeCodeCodeToComments.get(nodeCode).map(\elt -> elt.TextContent).join("\n    ")
        var allCommentedCategoryNode = getAllCodesWithRegexMatcher(commentString, _regexCategoryCodeCodeInComments)

        //Remove commented out nodes
        var removeCommentedOutNodes = new HashSet<XMLNode>()
        for (categoryNode in node.Children) {
          if (allCommentedCategoryNode.contains(categoryNode.Attributes.get("code"))) {
            removeCommentedOutNodes.add(categoryNode)
          }
        }
        removeCommentedOutNodes.each(\elt -> node.removeChildElement(elt))

        node.Children.first().setComment(commentString)
      }
    }
  }

  /**
   * Copies all the previous CoverageType.ttx nodes that are not PC typecodes.
   * For the non PC typecodes that have comments add them to the first <category>.
   * If all the <category> nodes are commented out. We will add a text that states
   * all of the category nodes have been commented out, and add it to the <typecode>
   * comment section.
   *
   * @param mapTypeCodeCodeToComments of <typecode code> to all commented out <category code>
   * @param parent is the XMLNode for CoverageType.ttx that will be written
   * @param priorXML is the XMLNode for the old CoverageType.ttx
   */
  function copyAllNonPCTypecodesForCategory(mapTypeCodeCodeToComments : Map<String, Set<Node>>, parent : XMLNode, priorXML : XMLNode) {
    ...
    for (node in priorXML.Children) {
      var nodeCode = node.Attributes.get("code")
      clearAllComments(node)
      if (!isPCNode(node) and not codesWithComments.contains(nodeCode)) {
        copyNodeFromPriorXML(parent, node)
      } else if (!isPCNode(node) and codesWithComments.contains(nodeCode)) {
        if (not node.Children.Empty) {
          node.Children.first().setComment(mapTypeCodeCodeToComments.get(nodeCode).map(\elt -> elt.TextContent).join("\n"))
          copyNodeFromPriorXML(parent, node)
        } else {
          var commentString = "Commented out category codes for <typecode code=\"" + nodeCode + "\"/>\n    "
          commentString += mapTypeCodeCodeToComments.get(nodeCode).map(\elt -> elt.TextContent).join("\n")
          node.setComment(commentString)
          copyNodeFromPriorXML(parent, node)
        }
      }
    }
  ...
  private function clearAllComments(node : XMLNode) {
  ...
  private function copyAllNonPCTypeCodesAndAddCCTypelistCategoryForPCTypeCodes(parent : XMLNode, priorXML : XMLNode) {
    // Return early if there was no prior XML
    if (priorXML == null) return

    for (node in priorXML.Children) {
      if (isPCNodeAndContainsCCTypelistCategory(node)) {
        var foundNode = findCorrespondingNodeInPC(parent, node)
        foundNode.addAllChildren(findAllAddedCCTypelistCategories(node))
      } else if (!isPCNode(node)) {
        this.copyNodeFromPriorXML(parent, node)
      }
    }
  }

  private function isPCNodeAndContainsCCTypelistCategory(node : XMLNode) : boolean {
    var sourceSystem = false
    var containsCCCatgoery = false
    for (catNode in node.Children) {
      if (isCategoryForTypelist(catNode, "SourceSystem") and
          getCategoryCode(catNode).equalsIgnoreCase("PC")) {
        sourceSystem = true
      }
      containsCCCatgoery = _covTermPatternCCAddedTypelistCategories.hasMatch(\elt -> isCategoryForTypelist(catNode, elt))
    }

    return sourceSystem && containsCCCatgoery
  }

  private function findCorrespondingNodeInPC(parent : XMLNode, previous : XMLNode) : XMLNode {
    return parent.Children.singleWhere(\elt -> elt.Attributes.get("code") == previous.Attributes.get("code"))
  }

  private function findAllAddedCCTypelistCategories(xmlNode : XMLNode) : List<XMLNode> {
    return xmlNode.Children.where(\node -> _covTermPatternCCAddedTypelistCategories.hasMatch(\elt -> isCategoryForTypelist(node, elt)))
  }
  ...
  private function writeTypelistFile(productModelXML : XMLNode, nodeElementName : String, name : String,
    ...
    switch(nodeElementName) {
      case _covSubRootName:
      case _covRootName:
      case _expoRootName:
      case _lossPartyRootName:
        root.setComment(_commentText)
        break
  ...
  private function localizeNameAndDescription(prodModel : ProductModelClass, typeListName : String,
    ...
    var ccTypeListDisplayKeyName = TypelistL10NPropertiesGateway.generateKey(true, typeListName, CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(prodModel))
    ...
    var ccTypeListDisplayKeyDescription = TypelistL10NPropertiesGateway.generateKey(false, typeListName, CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(prodModel))
  ...
  private function callTypecodeMappingAndLogWarningMessages(pmObject: ProductModelClass) : String {
    return CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(pmObject,_exceptionFile)
  }

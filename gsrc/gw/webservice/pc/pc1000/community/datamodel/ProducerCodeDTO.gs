...
@Export
@WsiExportable("http://guidewire.com/pc/ws/gw/webservice/pc/pc1000/community/datamodel/ProducerCodeDTO")
final class ProducerCodeDTO {

  var _address : gw.webservice.pc.pc1000.community.datamodel.AddressDTO as Address
  var _appointmentDate : Date as AppointmentDate
  var _branchPublicID : String as BranchPublicID
  var _code : String as Code
  var _commissionPlanIDs : gw.webservice.pc.pc1000.community.datamodel.CommissionPlanDTO[] as CommissionPlans = {}
  ...
  function populateFromProducerCode(producerCode : ProducerCode) {
    if (this.Address == null) {
      this.Address = new gw.webservice.pc.pc1000.community.datamodel.AddressDTO()
    }
  ...
  private function createCommissionPlanDTOs(commissionPlans : CommissionPlan[]) : gw.webservice.pc.pc1000.community.datamodel.CommissionPlanDTO[] {
    var plans = new ArrayList<gw.webservice.pc.pc1000.community.datamodel.CommissionPlanDTO>()
    commissionPlans.each( \ elt -> {
      var plan = new gw.webservice.pc.pc1000.community.datamodel.CommissionPlanDTO()
  ...
  private function createCommissionPlan(planDTO: gw.webservice.pc.pc1000.community.datamodel.CommissionPlanDTO) : CommissionPlan {
  ...
  private reified function findBeanByPublicIDOrThrow<T extends KeyableBean>(producerCodePublicID : String) : T {
    var bean = Query.make(T).compare(T#PublicID, Equals, producerCodePublicID).select().AtMostOneRow
    if (bean == null) {
      throw new BadIdentifierException(DisplayKey.get("ProducerCodeModel.populateProducerCode.Error.CannotFindForeignKeyBeanWithPublicID", T, producerCodePublicID))
    }
    return bean
  }
}
...
uses gw.webservice.contactapi.ab1000.ABClientAPI
uses gw.webservice.contactapi.ab1000.ABClientAPIAddressBookUIDContainer
uses gw.webservice.contactapi.ab1000.ABClientAPIPendingChangeContext
 ...
 */
@gw.xml.ws.annotation.WsiWebService( "http://guidewire.com/pc/ws/gw/webservice/pc/pc1000/contact/ContactAPI" )
@Export
class ContactAPI implements ABClientAPI {
      ...
      SOAPUtil.convertToSOAPException(PCLoggerCategory.CONTACT_API, \ -> {
        ...
        if (accountContacts.hasMatch(\ ac -> not ac.canRemove())) {
          ...
          throw new SOAPException(
            DisplayKey.get("ContactAPI.Error.DeleteContact.ContactUsedByAccount", contactPublicId, account))
        }
  ...
  function activateContactOnAllAccounts(contactPublicId : String, activate : boolean) : int {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      var contact = bundle.add(PCBeanFinder.loadBeanByPublicID<Contact>(contactPublicId, Contact))
      if (contact == null) {
        throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindContactByAddressBookUID", contactPublicId))
      }

      // find all AccountContacts referencing this Contact
      var accountContacts = Query.make(AccountContact).compare(AccountContact#Contact, Equals, contact).select().toList()
      if (not activate and accountContacts.hasMatch(\ a -> a.hasRole(TC_ACCOUNTHOLDER))) {
        throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotDeactivateAccountHolder"))
      }
  ...
  function mergeContactAddressesByPublicId(contactPublicId : String, survivingAddressPublicId : String, mergedAddressPublicId : String) : String {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      if (contactToMergeOn == null) {
        throw new BadIdentifierException(DisplayKey.get("Contact.MergeAddresses.Error.NullContactArgument", "PublicID", contactPublicId))
      }
  ...
  function mergeContactAddressesByABUID(contactABUID : String, survivingAddressABUID : String, mergedAddressABUID : String) : String {
    ...
    var survivingAddress : Address
     Transaction.runWithNewBundle(\ bundle -> {
      ...
      if (contactToMergeOn == null) {
        throw new BadIdentifierException(DisplayKey.get("Contact.MergeAddresses.Error.NullContactArgument", "AddressBookUID", contactABUID))
      }
      contactToMergeOn.mergeAddresses(survivingAddress, mergedAddress)
    })
  ...
  function getAssociatedWorkOrders(addressBookUID : String, workOrderStatus : PolicyPeriodStatus) : gw.webservice.pc.pc1000.gxmodel.jobmodel.types.complex.Job[] {
    // the helper method checks for null arguments
    var contact = findContactByAddressBookUID(addressBookUID)
    if (contact == null) {
       throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindContactByAddressBookUID", addressBookUID))
    }
    return getAssociatedWorkOrders(contact, workOrderStatus)
  }

  private function getAssociatedWorkOrders(contact : Contact, workOrderStatus : PolicyPeriodStatus) : gw.webservice.pc.pc1000.gxmodel.jobmodel.types.complex.Job[] {
    var jobs = contact.AssociationFinder.findWorkOrders(workOrderStatus)
    var infos = jobs.map(\ job -> new gw.webservice.pc.pc1000.gxmodel.jobmodel.Job(job).$TypeInstance)
  ...
  @WsiPermissions({SystemPermissionType.TC_VIEWSUBMISSION, SystemPermissionType.TC_VIEWRENEWAL, SystemPermissionType.TC_VIEWPOLCHANGE, SystemPermissionType.TC_VIEWREINSTATE, SystemPermissionType.TC_VIEWCANCELLATION, SystemPermissionType.TC_VIEWREWRITE, SystemPermissionType.TC_VIEWREWRNEWACCT, SystemPermissionType.TC_VIEWISSUANCE, SystemPermissionType.TC_VIEWAUDIT})
  @Returns("Array of Job objects representing all work orders associated with the Contact")
  function getAssociatedWorkOrdersByPublicID(publicId : String, workOrderStatus : PolicyPeriodStatus) : gw.webservice.pc.pc1000.gxmodel.jobmodel.types.complex.Job[] {
    // helper checks for null public ID
    var contact = findContactByPublicID(publicId)
    if (contact == null) {
       throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindContactByPublicID", publicId))
    }
    return getAssociatedWorkOrders(contact, workOrderStatus)
  }
  ...
  function getAssociatedPolicies(addressBookUID : String) : gw.webservice.pc.pc1000.gxmodel.policyperiodmodel.types.complex.PolicyPeriod[] {
    // helper checks for null public ID
    var contact = findContactByAddressBookUID(addressBookUID)
    if (contact == null) {
       throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindContactByAddressBookUID", addressBookUID))
  ...
  private function getAssociatedPolicies(contact : Contact) : gw.webservice.pc.pc1000.gxmodel.policyperiodmodel.types.complex.PolicyPeriod[] {
    var periods = contact.AssociationFinder.findPolicyPeriods()
    var infos = periods.map(\ period ->
      new gw.webservice.pc.pc1000.gxmodel.policyperiodmodel.PolicyPeriod(period.getSlice(period.EditEffectiveDate)).$TypeInstance
    )
    return infos.toTypedArray()
  }
  ...
  @WsiPermissions({SystemPermissionType.TC_VIEWPOLICYFILE, SystemPermissionType.TC_PFILEDETAILS})
  @Returns("array of PolicyPeriods associated with the Contact")
  function getAssociatedPoliciesByPublicID(publicId : String) : gw.webservice.pc.pc1000.gxmodel.policyperiodmodel.types.complex.PolicyPeriod[] {
    // helper checks for null public ID
    var contact = findContactByPublicID(publicId)
    if (contact == null) {
       throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindContactByPublicID", publicId))
  ...
  function getAssociatedAccounts(addressBookUID : String) : gw.webservice.pc.pc1000.gxmodel.accountmodel.types.complex.Account[] {
  ...
  private function getAssociatedAccounts(contact : Contact) : gw.webservice.pc.pc1000.gxmodel.accountmodel.types.complex.Account[]{
    var accounts = contact.AssociationFinder.findAccounts()
    var infos = accounts.map(\ account ->
       new gw.webservice.pc.pc1000.gxmodel.accountmodel.Account(account).$TypeInstance
    )
    return infos
  }
  ...
  @WsiPermissions({SystemPermissionType.TC_VIEWACCOUNT, SystemPermissionType.TC_SEARCHACCOUNTS})
  @Returns("array of Accounts associated with the Contact")
  function getAssociatedAccountsByPublicID(publicId : String) : gw.webservice.pc.pc1000.gxmodel.accountmodel.types.complex.Account[] {
    // helper checks for null public ID
    var contact = findContactByPublicID(publicId)
    if (contact == null) {
       throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindContactByPublicID", publicId))
  ...
  function addContact(externalContact : gw.webservice.pc.pc1000.gxmodel.contactmodel.types.complex.Contact) : String {
    SOAPUtil.require(externalContact, "externalContact")
    if (externalContact.isContactExist()) {
      throw new BadIdentifierException(DisplayKey.get("Webservice.Error.ContactAlreadyExists"))
    }
    var contact : Contact
    Transaction.runWithNewBundle(\ bundle -> {
    ...
    try {
      Transaction.runWithNewBundle(\ bundle -> {
        SOAPUtil.convertToSOAPException(PCLoggerCategory.CONTACT_API, \ -> {
          ...
          if (contact == null) {
            throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindContactByAddressBookUID", contactXML.LinkID))
          } else {
            if (contact.AutoSync != AutoSync.TC_ALLOW) {
              throw new IllegalStateException(DisplayKey.get("Webservice.Error.CannotUpdateContactUnlessAutoSyncIsAllowed", contact.AddressBookUID))
            }
  ...
  override function removeContact(addressBookUID : String) {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      SOAPUtil.convertToSOAPException(PCLoggerCategory.CONTACT_API, \ -> {
        ...
        if (contact != null) {
          if (contact.AutoSync != AutoSync.TC_ALLOW) {
            throw new IllegalStateException(DisplayKey.get("Webservice.Error.CannotRemoveContactUnlessAutoSyncIsAllowed", addressBookUID))
          }
  ...
  private function createActivityForDelinkContactFromCtC(AddressBookUID : String) {
    var query = Query.make(AccountContact).join("Contact").compare(Contact#AddressBookUID, Equals, AddressBookUID).select()
    ...
    for( accountContact in query.iterator()) {
      ...
      activity.Subject = DisplayKey.get("Web.ContactManager.Warning.DeleteContact.Subject", accountContact)
      activity.Description = DisplayKey.get("Web.ContactManager.Warning.DeleteContact.Description")
  ...
  private function findContact(addressBookUID : String) : Contact {
    return Query.make(Contact).compare(Contact#AddressBookUID, Equals, addressBookUID).select().AtMostOneRow
  }

  private function findAddress(addressABUID : String) : Address {
    return Query.make(Address).compare(Address#AddressBookUID, Equals, addressABUID).select().AtMostOneRow
  }

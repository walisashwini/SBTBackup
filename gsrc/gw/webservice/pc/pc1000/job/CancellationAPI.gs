 ...
 */
@WsiWebService( "http://guidewire.com/pc/ws/gw/webservice/pc/pc1000/job/CancellationAPI" )
@Export
class CancellationAPI {
  ...
  @Param("policyNumber", "The number of the period that should be canceled.")
  @Param("cancellationDate", "The effective date for the cancellation. Cannot be null.")
  @Param("recalculateEffDate", "If true, Policy Center will recalculate the effective date for the cancellation to meet all legal requirements, "
      + "and if that date is after the cancellation date then it will be used instead of the cancellation date")
  @Param("cancellationSource", "Who initiates the cancellation (e.g., the carrier or the insured).")
  @Param("reasonCode", "Typekey indicating the reason for the cancellation.")
  @Param("refundCalcMethod", "Method used to determine the refund. If null, will be defaulted by reasonCode")
  @Param("description", "Description for the cancellation.  May be null.")
  ...
  function beginCancellation(policyNumber : String, cancellationDate : Date,
      ...
      reasonCode : ReasonCode, refundCalcMethod : CalculationMethod,
  ...
  @Param("newDescription", "Description for the cancellation.  May be null.")
  @Param("transactionId", "The unique id for this request. If this is duplicated with that of any other previous requests, AlreadyExecutedException will be thrown")
  @WsiPermissions({SystemPermissionType.TC_CANCELRESCHEDULE})
  @Returns("The unique job number for the rescheduled Cancellation job.")
   ...
   *
   * @Param policyNumber The Policy number of the period that is being cancelled.
  ...
  @Param("policyNumber", "The Policy number of the period that is being cancelled. Cannot be null.")
  @Param("effectiveDate", "The date that the cancellation will become effective on or after.")
  @Param("source", "Who initiated the cancellation (e.g., the carrier or the insured). May be null.")
  @Param("reason", "Typekey indicating the reason for the cancellation. May be null.")
  @Param("transactionId", "The unique id for this request. If this is duplicated with that of any other previous requests, AlreadyExecutedException will be thrown")
  @WsiPermissions({SystemPermissionType.TC_CANCELRESCIND})
  function rescindCancellation(policyNumber : String, effectiveDate :  Date,
    ...
    SOAPUtil.tryCatch(\ bundle -> {
      ...
      var activePeriods = periods.where(\period -> period.Active)
      if (activePeriods.Count != 1) {
        throw new BadIdentifierException(DisplayKey.get("CancellationAPI.Rescind.Error.NumberOfMatchingJobs", activePeriods.Count))
      }
      _rescindCancellation(bundle.add(activePeriods.single().Cancellation))
      return null
    }, transactionId)
  }
  ...
  @Throws(RequiredFieldException, "If required field is missing")
  @Throws(BadIdentifierException, "If cannot find entity with given identifier")
  @Param("jobNumber", "The number of the Cancellation job to rescind; cannot be null.")
  @WsiPermissions({SystemPermissionType.TC_CANCELRESCIND})
  function rescindCancellationByJobNumber(jobNumber : String) {
    ...
    if (numJobs != 1) {
      throw new BadIdentifierException(DisplayKey.get("CancellationAPI.Rescind.Error.NumberOfMatchingJobs", numJobs))
    }
    var job = jobs.FirstResult
    if (not (job typeis Cancellation)) {
      throw new BadIdentifierException(
        DisplayKey.get("CancellationAPI.Rescind.Error.NotCancellation", typeof job))
    }
  ...
  @Throws(RequiredFieldException, "If required field is missing")
  @Throws(BadIdentifierException, "If cannot find entity with given identifier")
  @Param("policyNumber", "The number of the period that was canceled.  May be null if cancellationDate is non-null.")
  @Param("cancellationDate", "The effective date of the cancellation. May be null if policyNumber is non-null")
  @Param("cancellationSource", "Who initiated the cancellation (e.g., the carrier or the insured). May be null.")
  @Param("reasonCode", "Typekey indicating the reason for the cancellation. May be null.")
  @Param("calculationMethod", "Method used to refund. May be null.")
  @WsiPermissions({SystemPermissionType.TC_VIEWCANCELLATION})
  @Returns("An array of job numbers of all Cancellation jobs matching the provided criteria.")
  ...
  {
    // Check arguments
    if(policyNumber == null && cancellationDate == null) {
      if(policyNumber == null) SOAPUtil.require(policyNumber, "policyNumber");
      if(cancellationDate == null) SOAPUtil.require(cancellationDate, "cancellationDate");
    }

    var builder = new PolicyPeriodSummaryQueryBuilder()
    builder.withPolicyNumber(policyNumber)
    builder.withEditEffectiveDateWithin(cancellationDate.addDays(-1), cancellationDate.addDays(1))

    var jobQueryBuilder = new JobQueryBuilder()
        .cast(TC_CANCELLATION)
        .withCancellationSource(cancellationSource)
        .withCancellationReasonCode(reasonCode)
    builder.withRefundCalcMethod(calculationMethod)
    builder.withJob(jobQueryBuilder)

    return (builder.build().select() as gw.api.database.IQueryBeanResult<PolicyPeriodSummary>)*.Job*.JobNumber
  }
  ...
  private function _rescindCancellation(cancellation: Cancellation) {
    ...
    if(period.Status == PolicyPeriodStatus.TC_BOUND){
      throw new EntityStateException(DisplayKey.get("CancellationAPI.Rescind.Error.AlreadyCompleted"))
    }
    if (period.Status == PolicyPeriodStatus.TC_RESCINDED or
      ...
      throw new EntityStateException(DisplayKey.get("CancellationAPI.Rescind.Error.AlreadyRescinded"))
    }
    Transaction.runWithNewBundle( \bundle -> {
      try {
        period = bundle.add(period)
        if(period.Status == PolicyPeriodStatus.TC_CANCELING){
          period.ActiveWorkflow.invokeTrigger(WorkflowTriggerKey.TC_RESCIND)
        }else{

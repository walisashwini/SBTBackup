 ...
 * xml data.
 */
@Deprecated("PC10.0.0. Use SubmissionAPI instead", "PC10.0.0")
@Export
@WsiWebService("http://guidewire.com/pc/ws/gw/webservice/pc/pc1000/policy/ImportPolicyAPI")
class ImportPolicyAPI {
  ...
  function quoteSubmission(accountNumber : String,
                                ...
                                parseOptions : String) : gw.webservice.pc.pc1000.policy.QuoteResponse {
    ...
    if(producerCode == null){
      throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindProducerCode", producerCodeId))
    }
    var product = ProductLookup.getByPublicID(productCode)
    if(product == null){
      throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindProductCode", productCode))
    }
    ...
    return new gw.webservice.pc.pc1000.policy.QuoteResponse(policyPeriod.Job.JobNumber, errors.toTypedArray())
  }
}
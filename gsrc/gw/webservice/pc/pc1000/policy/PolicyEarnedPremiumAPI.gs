...
@WsiWebService( "http://guidewire.com/pc/ws/gw/webservice/pc/pc1000/policy/PolicyEarnedPremiumAPI" )
@Export
class PolicyEarnedPremiumAPI {
  ...
  public function calcEarnedPremiumByPolicyNumber(policyNumber : String, periodAsOfDate : Date, earnedAsOfDate : Date, includeEBUR : boolean ) :  List<gw.webservice.pc.pc1000.policy.PolicyEarnedPremiumInfo> {
    ...
    var results = new ArrayList<gw.webservice.pc.pc1000.policy.PolicyEarnedPremiumInfo>()
    var period = Policy.finder.findPolicyPeriodByPolicyNumberAndAsOfDate(policyNumber, periodAsOfDate)
    if(period == null){
      throw new BadIdentifierException(DisplayKey.get("Webservice.Error.CannotFindPolicyForPolicyNumber", policyNumber, periodAsOfDate))
    }

    // Check Earned date to be meaningful
    if (earnedAsOfDate.compareIgnoreTime(period.PeriodStart) < 0) {
      throw new IllegalArgumentException(DisplayKey.get("Webservice.Error.EarnedBeforePeriodStart", earnedAsOfDate, period.PeriodStart))
    }
    ...
    if (includeEBUR and not period.IsReportingPolicy) {
      throw new IllegalArgumentException(DisplayKey.get("Webservice.Error.IncludeEBURNotValidForPolicy"))
    }
  ...
  private function createInfo(txs : List<Transaction>, passAsOf : Date, lastReportedDate : Date, passIncludeEBUR : boolean, line : String) : gw.webservice.pc.pc1000.policy.PolicyEarnedPremiumInfo {
    var info = new gw.webservice.pc.pc1000.policy.PolicyEarnedPremiumInfo()

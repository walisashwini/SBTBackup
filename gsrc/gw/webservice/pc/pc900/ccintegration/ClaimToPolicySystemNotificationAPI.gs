...
*/
@Export
@gw.xml.ws.annotation.WsiWebService( "http://guidewire.com/pc/ws/gw/webservice/pc/pc900/ccintegration/ClaimToPolicySystemNotificationAPI" )
  ...
  //------------------------ private helper methods
    private function addLargeLossActivityToPolicy(
      ...
      var activity = activityPattern.createPolicyActivity(policy.Bundle,
        policy,
        DisplayKey.get("LargeLoss.ActivitySubject", policyNumber),
        DisplayKey.get("LargeLoss.ActivityDescription", convertMonetaryAmountToString(grossTotalIncurred), today),
        null, TC_HIGH, true, null, null)
    ...
    var descriptionEval = \ ->
      DisplayKey.get("LargeLoss.Description", convertMonetaryAmountToString(grossTotalIncurred),
  ...
  : PolicyPeriod {
    ...
    if( policyPeriod == null ) {
      throw new BadIdentifierException(
        DisplayKey.get("LargeLoss.ClaimExceedsThreshold.NoPolicyFound", 
          policyNumber, lossDate))
    }
    return policyPeriod
  }

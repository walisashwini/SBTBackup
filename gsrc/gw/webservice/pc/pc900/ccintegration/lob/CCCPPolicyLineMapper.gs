...
uses gw.webservice.pc.pc900.ccintegration.entities.types.complex.CCBuildingRU
...
class CCCPPolicyLineMapper extends CCBasePolicyLineMapper {
      ...
      if (meetsLocationFilteringCriteria(cploc.Location)) {
        for (bld in cploc.Buildings.sortBy(\ b -> b.Building.BuildingNum)) {
          if (_policyGen.meetsPolicySystemIDFilteringCriteria(bld.TypeIDString)) {
            ...
            ccBld.AddressLine1Kanji = loc.AddressLine1Kanji
            ccBld.AddressLine2Kanji = loc.AddressLine2Kanji

            ccBld.City = loc.City
            ccBld.CityKanji = loc.CityKanji
  ...
  override function getLineCoverages() : List<entity.Coverage> {
    return _cpLine.CPLineCoverages?.toList() as List<entity.Coverage>
  }
...
//  }
  ...
  private function blanketCovTermExists(ccCov : CCCoverage) : Boolean {
    if (ccCov.CovTerms != null) {
      for (covTerm in ccCov.CovTerms) {
        if (covTerm.$TypeInstance typeis CCClassificationCovTerm) {
          if ("pc_custom_blanket".equalsIgnoreCase((covTerm.$TypeInstance as CCClassificationCovTerm).Code)) {return true}
        }
      }
    }
    return false
  ...
  private function addblanketCovTerm(ccCov: CCCoverage, blanketCov : entity.CPBlanketCov) {
    addCustomClassificationCovTerm(ccCov, blanketCov.TypeIDString, "pc_custom_blanket", 1000,
                                   null, DisplayKey.get("Integration.CCIntegration.BlanketCovTermDescription"))
  }

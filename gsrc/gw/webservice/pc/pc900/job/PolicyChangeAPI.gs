...
@gw.xml.ws.annotation.WsiWebService( "http://guidewire.com/pc/ws/gw/webservice/pc/pc900/job/PolicyChangeAPI" )
@Export
...
class PolicyChangeAPI {
  ...
  function startManualPolicyChange(policyNumber: String, effectiveDate: Date) : String {
    ...
    if (policy == null) {
      throw new BadIdentifierException(DisplayKey.get("PolicyChangeAPI.Error.InvalidPolicyNumber", policyNumber))
    }
    
    var error = Plugins.get(IPolicyPlugin).canStartPolicyChange(policy, effectiveDate)
    if (error != null) {
      throw new EntityStateException(DisplayKey.get("PolicyChangeAPI.Error.CannotStart", error))
    }

...
@gw.xml.ws.annotation.WsiWebService( "http://guidewire.com/pc/ws/gw/webservice/pc/pc900/job/ReinstatementAPI" )
@Export
...
class ReinstatementAPI
{
  ...
  function beginReinstatement(policyNumber : String, reinstateCode : ReinstateCode) : String {
    ...
    if (cancellationPeriod == null) {
      throw new EntityStateException(DisplayKey.get("ReinstatementAPI.BeginReinstatement.Error.InvalidPolicyNumber", policyNumber))
    }
    Transaction.runWithNewBundle(\bundle -> {
      var error = Plugins.get(IPolicyPlugin).canStartReinstatement(cancellationPeriod)
      if (error != null) {
        throw new EntityStateException(DisplayKey.get("ReinstatementAPI.BeginReinstatement.Error", policyNumber, error))
      }
      try {

...
class SystemToolsAPI {
  ...
  /**
   * Return the Performance Report with the given public ID. The report is returned as an array of bytes that can be
   * saved as a zip file.
   *
   * @return Requested Performance Report
   */
  @WsiPermissions({SystemPermissionType.TC_TOOLSINFOVIEW, SystemPermissionType.TC_SOAPADMIN})
  @Throws(WsiAuthenticationException,"On permission or authentication errors")
  public function getPerfReport(reportPublicId : String) : byte[] {
    return new SystemToolsImpl().getPerfReport(reportPublicId)
  }

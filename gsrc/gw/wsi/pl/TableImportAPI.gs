...
class TableImportAPI {
  ...
  /**
   * Return the Load History Report with the given public ID. The report is renturned as an array of bytes that can be saved
   * as a zip file
   *
   * @param reportPublicId               Public ID of the requested Load History report
   *
   * @return byte[]                      Requested Load History report
   */
  @Throws(WsiAuthenticationException,"On permission or authentication errors")
  public function getLoadHistoryReport(reportPublicId : String) : byte[] {
    return new TableImportImpl().getLoadHistoryReport(reportPublicId)
  }

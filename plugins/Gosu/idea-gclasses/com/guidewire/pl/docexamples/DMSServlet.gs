...
//      return
  ...
  override function init(config: ServletConfig) {
    _urlRoot = getConfigProperty("url-root", config, null)
    if (_urlRoot != null) {
    ...
    _rootPath = getConfigProperty("root-dir", config, _rootPath)
    _rootDir = new File(_rootPath)
    _rootDir.mkdirs()
    PLLoggerCategory.DOCUMENT.info("DMSServlet-init rootDir=" + _rootDir)

    _dbDriver = getConfigProperty("db-driver", config, _dbDriver)
    var dbDriver = Class.forName(_dbDriver)
    if (dbDriver == null) {
      throw new ServletException("Failed to load driver " + _dbDriver)
    }
    _dbURL = getConfigProperty("db-url", config, null)
    _dbUsername = getConfigProperty("db-username", config, _dbUsername)
    _dbPassword = getConfigProperty("db-password", config, _dbPassword)
    if (_dbURL == null) {
      _purgeableUrl = "jdbc:h2:file:" + new File(_rootDir, "db").CanonicalPath
      _dbURL = _purgeableUrl
    }
    PLLoggerCategory.DOCUMENT.info("DMSServlet-init dbURL=" + _dbURL)
  ...
  private function getConfigProperty(name : String, config : ServletConfig, defaultStr : String) : String {
    var str = System.getProperty("gw.document.DMSServlet." + name)
    if (str != null) {
      return str;
    }

...
enhancement AddressEnhancement : Address
{
  ...
  private function assertLinkAddressConsistency(otherAddress : Address, contact : Contact) {
    if (otherAddress == null){
      throw new IllegalArgumentException(DisplayKey.get("Web.Addresses.Linked.NullAddress"))
    }
    if (this.LinkedAddress != null){
      throw new DisplayableException(DisplayKey.get("Web.Addresses.Linked.AlreadyLinked"))
    }
    if (contact != null and contact.AllAddresses.hasMatch(\ a -> a.isLinkedToAddress(otherAddress))) {
      throw new DisplayableException(DisplayKey.get("Web.Addresses.Linked.ContactAlreadyLinked"))
    }
  }
  ...
  function unlink() {
    var linkedAddress = this.LinkedAddress
    if (linkedAddress == null) {
      throw new IllegalStateException(DisplayKey.get("Web.Addresses.Linked.UnlinkException"))
    }
  ...
  private function joinLinkedAddressTable(table : Table<Contact>) {
    table.join("LinkedAddress").compare(LinkedAddress#ID, Equals, this.LinkedAddress.ID)
  }
  ...
  /**
   * @return true if this address is not linked to any external address book
   */
  ...
  property get NeedsNewExternalID() : boolean {
    return IsLocalOnly and ExternalIDNotSet
  }

  private property get ExternalIDNotSet() : boolean {

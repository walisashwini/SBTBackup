...
enhancement FormPatternEnhancement: entity.FormPattern {
  ...
  private static function duplicateFormPattern(sourceFormPattern: FormPattern): FormPattern {
    ...
    dupFormPattern.Code = DisplayKey.get("Web.Admin.FormPatternDetail.BasicsDV.CodeCopy", sourceFormPattern.Code)
    dupFormPattern.FormNumber = DisplayKey.get("Web.Admin.FormPatternDetail.BasicsDV.NumberCopy", sourceFormPattern.FormNumber)
    ...
    for (var sourceFormPatternCoveragePartTypes in sourceFormPattern.FormPatternCoveragePartTypes) {
      dupFormPattern.addToFormPatternCoveragePartTypes(sourceFormPatternCoveragePartTypes.shallowCopy() as FormPatternCoveragePartType)
    }
  ...
  function addProduct(product: Product): Product {
    if (this.FormPatternProducts.firstWhere(\f -> f.ProductCode == product.PublicID) != null) {
      throw new IllegalStateException("Product \"${product.PublicID}\" is already added to this FormPattern")
    }
    var formPatternProduct = new FormPatternProduct()
    formPatternProduct.ProductCode = product.PublicID
    this.addToFormPatternProducts(formPatternProduct)
    return product
  }

  function removeProduct(product: Product) {
    var formPatternProduct = this.FormPatternProducts.firstWhere(\f -> f.ProductCode == product.PublicID)
    if (formPatternProduct == null) {
      throw new IllegalStateException("Product \"${product.PublicID}\" not found in this FormPattern")
    }
  ...
  /**
   * @return an array of {@link gw.api.productmodel.Product} POJOs designated by this FormPattern's FormPatternProducts
  ...
  /**
   * @return an unsorted array of all {@link gw.api.productmodel.Product} POJOs that are not already designated by this
   * FormPattern's FormPatternProducts array. Ignores "Availability" as determined by the Product availability lookups.
   */
  property get AvailableProducts(): Product[] {
    var addedProductCodes = this.FormPatternProducts.map(\f -> f.ProductCode).toSet()
    return PCDependenciesGateway.getProductModel().getAllInstances(Product).where(\p -> not addedProductCodes.contains(p.PublicID)).toTypedArray()
  }

  /**
   * @return an array of Job typekeys designated by this FormPattern's FormPatternJobs array, in the same order as the
   * array entries.
   */
  ...
  /**
   * @return an unsorted array of all Job typekeys that are  not already designated by this FormPattern's
   * FormPatternJobs array.
   */
  property get AvailableJobs(): typekey.Job[] {
    var addedJobs = this.FormPatternJobs.map(\f -> f.JobType).toSet()
    var filteredJobs = (typekey.Job.Type as ITypeList).getTypeKeysByFilterName("FormPatternJobs") as List<typekey.Job>
  ...
  /**
   * @return an array of CoveragePartType typekeys designated by this FormPattern's FormPatternCoveragePartTypes array,
   * in the same order as the array entries.
   */
  property get CoveragePartTypes(): typekey.CoveragePartType[] {
    return this.FormPatternCoveragePartTypes.map(\f -> f.CoveragePartType)
  }
  ...
  /**
   * @return the form numbers of all form patterns that have InternalRemovalEndorsement set to true, plus this one's
   * current RemovalEndorsementFormNumber, regardless of whether it is valid.
   */
  property get RemovalEndorsementFormNumbersRange(): String[] {
    var q = Query.make(FormPattern)
    q.compare(FormPattern#InternalRemovalEndorsement, Equals, true)
  ...
  /**
   * @return references to all other form patterns with the same GroupCode, or empty array if GroupCode is null; if the
   * current value of UseInsteadOfCode refers to a form pattern that no longer exists, the results will have a reference
   * for that code, but its FormPattern will be null.
   */
  ...
  function makeGroupQuery(excludeThis: boolean): Query<FormPattern> {
    ...
    var q = Query.make(FormPattern)
    if (excludeThis) {
      q.compare(FormPattern#Code, NotEquals, this.Code)
    }
    q.or(\or1 -> {
      or1.compare(FormPattern#InternalGroupCode, Equals, this.GroupCode)
      or1.and(\and1 -> {
        and1.compare(FormPattern#FormNumber, Equals, this.GroupCode)
        and1.compare(FormPattern#InternalGroupCode, Equals, null)
      })
    })
    return q
  }
  ...
  /**
   * @return references to all classes that implement {@link gw.forms.GenericFormInference} and are applicable to given
   * policyLine. If the current value of GenericInferenceClass refers to a class that does not exist or is invalid, the
   * results will still contain a reference for it.
   */
  function getGenericInferenceClassRange(policyLine: PolicyLinePattern): FormInferenceClassRef[] {
    ...
    var refsList = inferenceClasses.map(\i -> new FormInferenceClassRef(i as Type<GenericFormInference>))
  ...
  property get GenericInferenceClassRef(): FormInferenceClassRef {
    if (this.GenericInferenceClass != null) {
      ...
      return gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(gw.forms.GenericFormInference.Type, inferenceClassType) ? new FormInferenceClassRef(inferenceClassType as Type<GenericFormInference>) : new FormInferenceClassRef(this.GenericInferenceClass)
    } else {
  ...
  /**
   * @return a list of all {@link gw.api.productmodel.PolicyLinePattern} POJOs that are shared by this FormPattern's
   * selected products; if the current value of PolicyLinePatternCode refers to a pattern that does not exist or is
   * invalid, the results will still contain a reference for it.
   */
  property get PolicyLinePatternRange(): List<PatternRef<PolicyLinePattern>> {
    ...
    this.Products.each(\p -> {
      lines = lines.intersect(p.LinePatterns.toSet()).toList()
    })
  ...
  function onPolicyLineChange() {
    this.GenericInferenceClass = null;
    this.clearCoveragePartTypes()
    this.clearCustomInferenceFields();
  }

  function loadPolicyLinePatternRef(code: String): PatternRef<PolicyLinePattern> {
    var pattern = gw.api.productmodel.PolicyLinePatternLookup.getByPublicID(code)
    return pattern != null ? new PatternRef<PolicyLinePattern>(pattern) : new PatternRef<PolicyLinePattern>(code)
  }

  /**
   * Helper function to reset dependent values that are not editable because of UI changes.
   */
   ...
   */
  static property get AllPatterns(): Collection<FormPattern> {
    var q = Query.make(FormPattern)
...
*/
  ...
  /**
   * This property determines the text that will end up in the FormDescription field on the Form.  By default this will
   * just be the description on the FormPattern, but it can be overridden by subclasses to give more details about, for
   * example, why the form is being added or what it relates to
   */
   ...
   */
  property get InferenceClass(): IType {
  ...
  property get CreatesMultipleForms(): boolean {
    return gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(gw.forms.CreatesMultipleForms.Type, this.InferenceClass)
  }

...
class AffinityGroupValidation extends PCValidationBase {
  ...
  function validateAffinityGroupProducerCodes() {
    ...
    if (AffinityGroup.AffinityGroupType == AffinityGroupType.TC_CLOSED) {
      if (AffinityGroup.AffinityGroupProducerCodes.IsEmpty) {
        Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.ProducerCodeRequired"))
      } else {
        ...
        if (producerCodesNotMatchingOrganization.HasElements) {                                                                       
          ...
          Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.ProducerCodesNotMatchingOrganization", displayableString, AffinityGroup.Organization.DisplayName))
        }
      }
    }
    if (AffinityGroup.AffinityGroupType == AffinityGroupType.TC_OPEN and not AffinityGroup.AffinityGroupProducerCodes.IsEmpty) {
      Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.AffinityGroupProducerCodesNotAllowed"))
    }
  }
  ...
  function validateOrganizationOnAffinityGroup() {
    ...
    if (AffinityGroup.AffinityGroupType == AffinityGroupType.TC_OPEN and AffinityGroup.Organization != null) {
      Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.AffinityGroupOrganizationNotAllowed"))
    }
  }
    ...
    AffinityGroup.Jurisdictions
      ...
      .eachValue(\ dup -> { 
         Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT,
           DisplayKey.get("Web.Admin.JurisdictionExists", dup.first().Jurisdiction.Description))                           
         })
  }
  ...
  function validateAffinityGroupDates() {
    ...
    if (startDate != null and endDate != null){
      if(startDate.compareTo(endDate) == 0) {
        Result.addWarning(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.AffinityGroupSameStartAndEndDates"))
      } else if (startDate.compareTo(endDate) > 0 ) {
        Result.addError(AffinityGroup, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Admin.AffinityGroupEndDateBeforeStartDate"))
      }
    }
  }
}

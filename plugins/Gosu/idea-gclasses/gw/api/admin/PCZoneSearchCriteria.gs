...
class PCZoneSearchCriteria extends SearchCriteria<Zone> {
  ...
  function getRelevantZoneTypesForCountry() : ArrayList<ZoneType> {
    if (Country == null) {
      throw new DisplayableException(DisplayKey.get("Web.Admin.PolicyHold.CountryNotDefined"))
    }
  ...
  override protected property get MinimumSearchCriteriaMessage() : String {
    if (ZoneType == null) {
      return DisplayKey.get("Web.Admin.PolicyHold.RegionTypeMustBeSelected")
    }
    if (Country == null) {
      return DisplayKey.get("Web.Admin.PolicyHold.CountryNotDefined")
    }
    if (Country == typekey.Country.TC_US and ZoneType != typekey.ZoneType.TC_STATE
        and LinkedZone == null) {
      return DisplayKey.get("Web.Admin.PolicyHold.NonStateRegionRequiresState")
    }
    return null
  }

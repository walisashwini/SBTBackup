...
class CPBuildingBuilder extends DataBuilder<CPBuilding, CPBuildingBuilder> {
  ...
  private function constructWithAddPopulator() {
    addPopulator(Integer.MAX_VALUE, \building -> {
      ...
      if (building.ClassCode == null) {
        ...
        if (classCode == null) {
          throw DisplayKey.get("Builder.CPBuilding.Error.InvalidClassCode", _classCode)
        }
  ...
  function withCPBuildingAdditionalInterest(addInterestBuilder : CPBuildingAdditionalInterestBuilder) : CPBuildingBuilder {
    addPopulator(new BuilderArrayPopulator<CPBuilding>(CPBuilding.Type.TypeInfo.getProperty("AdditionalInterests") as IArrayPropertyInfo, addInterestBuilder))
    return this
  }

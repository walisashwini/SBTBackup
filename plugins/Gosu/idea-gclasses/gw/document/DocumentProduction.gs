...
class DocumentProduction {
  ...
  public static final var REQUIRED_SYMBOLS: String = "requiredsymbols"

  private static var _logger = PCLoggerCategory.DOCUMENT
  ...
  */
  static function createAndStoreBestDocumentSynchronously( docType : DocumentType, document : Document, policyPeriod : PolicyPeriod) : boolean {
    ...
    if (template == null) {
      _logger.warn(DisplayKey.get("DocumentProduction.Warning.NoTemplateForPeriod", policyPeriod, docType, policyPeriod.PeriodStart))
      return false
    }
    _logger.warn(DisplayKey.get("DocumentProduction.Debug.SelectedTemplate", policyPeriod, docType, template.TemplateId))
  ...
  public static function createAndStoreDocumentSynchronously(template : IDocumentTemplateDescriptor, parameters : Map, document : Document) {
    ...
    if (dci.ResponseType != DocumentContentsInfo.ContentResponseType.DOCUMENT_CONTENTS) {
      throw new RuntimeException(DisplayKey.get("DocumentProduction.Error.ResponseNotDocumentContents"))
    }
  ...
  public static function createDocumentSynchronously(template : IDocumentTemplateDescriptor, parameters : Map, document : Document) : DocumentContentsInfo {
    ...
    var docContentInfo : DocumentContentsInfo
    LocaleUtil.runAsCurrentLanguage( locale , \ ->  {
      ...
      if (documentProductionPlugin == null) {
        throw new DisplayableException(DisplayKey.get("DocumentProduction.Error.PluginDisabled"))
      }
      docContentInfo = documentProductionPlugin.createDocumentSynchronously(template, parameters, document )
      }
    )
  ...
  **********************************************************************************************/
  ...
  private static function adjustDocumentName(documentContentSource : IDocumentContentSource, document : Document) {
    ...
    while(documentContentSource.isDocument( document )) {
     ...
     document.Name = DisplayKey.get("Java.Document.DocumentDuplicateNameAdjustment", originalName, i)
    }
  }
  ...
  private static function getBestMatchPolicyPeriodDocumentTemplate(date : Date, docType : DocumentType, policyPeriod : PolicyPeriod) : IDocumentTemplateDescriptor {
    var valuesToMatch = new HashMap<String, Object>()
     ...
     keywords.add(policyPeriod.Policy.Product.PublicID)
     ...
     valuesToMatch.put(KEYWORDS_ATTRIB, keywords?.toTypedArray())
  ...
  static function getBestMatchPolicyPeriodDocumentTemplate(date : Date, docType : DocumentType, valuesToMatch : Map<String, Object>) : IDocumentTemplateDescriptor {
     ...
     var searchMap : Map<String, Object> = { TYPE_ATTRIB -> docType.Code, REQUIRED_SYMBOLS -> "PolicyPeriod" }
     var templates = dts.getDocumentTemplates( date, searchMap, 0 )
     if (templates.Count == 0) {
       throw new RuntimeException(DisplayKey.get("DocumentProduction.Error.TemplateNotFound", docType))
     }
  ...
  private static function calcTemplateScore(template : IDocumentTemplateDescriptor, valuesToMatch : Map<String, Object>) : int {
    ...
    for (propName in valuesToMatch.Keys) {
      ...
      if (valueToMatch typeis String) {
        ...
        throw new IllegalArgumentException(DisplayKey.get("DocumentProduction.Error.ValueToMatchType", propName, valueToMatch.getClass().getName()))
      }
    }
    return score
  }

...
class JobProcessUWIssueEvaluator {
   ...
   *   * checkedAtXXXXX
   *
   * Also, checkedAtAllButRateRelease will be run at any given blocking point excepts BlocksRateRelease,
   * and checkedAtAllButRateReleaseAndQuote will be run at any given blocking point excepts BlocksRateRelease
   * and BlocksQuote
   * 
  ...
  private var _checkedAtAllButRateRelease: List<UWIssueCheckingSet> as CheckedAtAllButRateRelease
  ...
  private var _checkedAtRateRelease : List<UWIssueCheckingSet> as CheckedAtRateRelease = {UWIssueCheckingSet.TC_PRERATERELEASE}
  private var _checkedAtQuoteRelease : List<UWIssueCheckingSet> as CheckedAtQuoteRelease = {UWIssueCheckingSet.TC_PRERATERELEASE, UWIssueCheckingSet.TC_PREQUOTERELEASE, UWIssueCheckingSet.TC_MVR}
  ...
  private var _checkedAtAllButRateReleaseAndQuote: List<UWIssueCheckingSet> as CheckAtAllButRateReleaseAndQuote = {UWIssueCheckingSet.TC_REINSURANCE}
  ...
  static function forIssuance() : JobProcessUWIssueEvaluator {
    return new JobProcessUWIssueEvaluator() {
      :CheckedAtAllButRateRelease = {UWIssueCheckingSet.TC_ALL, UWIssueCheckingSet.TC_UWHOLD, UWIssueCheckingSet.TC_REGULATORYHOLD, UWIssueCheckingSet.TC_REFERRAL}
    }
  }
  ...
  static function forRenewal() : JobProcessUWIssueEvaluator {
    return new JobProcessUWIssueEvaluator() {
      ...
      :CheckAtAllButRateReleaseAndQuote = {UWIssueCheckingSet.TC_REINSURANCE}
    }
  }
  ...
  protected function findBlockingIssues(branch : PolicyPeriod, bp : UWIssueBlockingPoint) : entity.UWIssue[] {
    var analyzer = new UWIssueAutomaticApprovabilityAnalyzer(branch.UWIssuesActiveOnly.whereBlocking(bp),
        CurrentAuthorityProfiles.arrays("Grants").toArray(new UWAuthorityGrant[0]), bp, branch.JobProcess.AutomatedProcess)
  ...
  private function checkingSetsFor(bp : UWIssueBlockingPoint) : List<UWIssueCheckingSet> {
    if (bp == UWIssueBlockingPoint.TC_BLOCKSRATERELEASE) {
      return CheckedAtRateRelease.copy()
    }
    else {
      var checkingSets = CheckedAtAllButRateRelease.copy()
      if (bp != UWIssueBlockingPoint.TC_BLOCKSQUOTE)
        checkingSets.addAll(_checkedAtAllButRateReleaseAndQuote)

...
enhancement RewriteNewAccountEnhancement : RewriteNewAccount {
  ...
  static function startRewriteNewAccount(policyPeriods : PolicyPeriod[], targetAccount : Account) {
    ...
    targetAccount.validateAccountHolderSuitableForPoliciesProductType(:policyPeriods = policyPeriods, :forRewrite = true)
    ...
    for (policyPeriod in policyPeriods) {
      Transaction.runWithNewBundle(\ bundle -> {
        ...
        sourceTerm.createCustomHistoryEvent(CustomHistoryType.TC_REWR_NEW_ACCT_CREATED, 
          \ -> DisplayKey.get("Job.RewriteNewAccount.History.SourceAccount.Description", policyPeriod.PolicyNumber, targetAccount))
        ...
        job.createRoleActivity(typekey.UserRole.TC_CREATOR, activityPattern, 
          DisplayKey.get("Job.RewriteNewAccount.Activity.Subject", targetAccount, policyPeriod.Policy.Account, policyPeriod.PolicyNumber),
  ...
  static function validateRewriteForDuplicates(policyPeriods : PolicyPeriod[]) {
    ...
    if (multiplePeriodsForPolicy.HasElements){
      throw new DisplayableException(DisplayKey.get("Job.RewriteNewAccount.Error.PeriodsFromSamePolicy"))
    }
  }

  private static function createActivityDescription(policyPeriod : PolicyPeriod) : String {
    var description = DisplayKey.get("Job.RewriteNewAccount.Activity.Description")
    var activities = Activity.finder.findOpenActivitiesByPolicy(policyPeriod.Policy)
    if (activities.Count > 0) {
      description += "\n" + DisplayKey.get("Job.RewriteNewAccount.Activity.Description.OpenActivities", policyPeriod.PolicyNumber)
    }
    return description    
  }
}

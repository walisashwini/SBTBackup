...
enhancement SubmissionEnhancement : Submission {
  ...
  function copyPolicyPeriod(period : PolicyPeriod): Submission {
    if(!canCopyPolicyPeriod(period)) {
      throw new DisplayableException(DisplayKey.get("Submission.Copy.Error.CannotCopy", period.ID, period.BranchName, this.DisplayName))
    }
    ...
    // clone policy period
    var copyPeriod = period.getSlice(period.PeriodStart).copyBranchIntoNewPolicy(copyPolicy)
    ...
    copyPeriod.setDoNotDestroy(false, \ -> "");
  ...
  function canCopyPolicyPeriod(period : PolicyPeriod) : boolean {
    if(period.Job != this) 
      throw DisplayKey.get("Submission.Copy.Error.PeriodNotOnJob", period.DisplayName)
    return true
  }
  ...
  property get QuickerQuickQuote() : boolean {
    return this.QuoteType == QuoteType.TC_QUICK and this.Policy.Product.QuickerQuickQuoteAllowed
  }
}

...
uses gw.api.productmodel.SingleAuditSchedulePatternLookup
...
class AuditScheduler {
  ...
  static function scheduleSingleCheckingAudit(period : PolicyPeriod) {
    var pattern = SingleAuditSchedulePatternLookup.selectByPublicID(period.SingleCheckingPatternCode)
    if (pattern != null) {
      var startDate = period.PeriodStart
      var endDate = pattern.EndDateDelayUnit == TC_BUSINESSDAYS
  ...
  static function scheduleNewAudit(period : PolicyPeriod,
    ...
    if (available.Empty or not available.contains(type)) {
      throw (DisplayKey.get("Web.Policy.PolicyPeriod.Validation.AuditCreation.NoMoreAudits", type))
    }
    if (method == null) {
      throw (DisplayKey.get("Web.Policy.PolicyPeriod.Validation.AuditCreation.MissingMethod"))
    }
    if (datesOutOfRange(period, startDate, endDate)) {
      throw (DisplayKey.get("Web.Policy.PolicyPeriod.Validation.AuditCreation.DatesOutOfRange"))
    }
    if ((type == TC_PREMIUMREPORT) and datesOverlapReport(period, startDate, endDate)) {
      throw (DisplayKey.get("Web.Policy.PolicyPeriod.Validation.AuditCreation.OverlappingDates"))
    }
  ...
  private static function scheduleAuditSeries(period : PolicyPeriod, patternName : String, scheduleType : AuditScheduleType) {
    var pattern = SeriesAuditSchedulePatternLookup.selectByPublicID(patternName)
    if (pattern != null) {

...
abstract class AbstractSideBySideBaseDataConfig implements MatchableTreeTraverserConfig {
  ...
  static private var _logger : Logger as SxSLogger = PCLoggerCategory.SXS
  ...
  static private var sideBySideProperties = new LocklessLazyVar<Map<IType,Set<String>>>() {
    override function init() : Map<IType,Set<String>> {
      var superTypeToPropertiesMap : Map<IEntityType,Set<String>> = {
        ...
        PolicyPeriod -> {"RIRiskVersionLists", "QuoteHidden", "EditLocked", "DepositCollected", "InvoicingMethod", "InvoiceStreamCode", "InvoiceStreamOverrides","NewInvoiceStream",
                         ...
                         "AltBillingAccountNumber", "AllocationOfRemainder", "RefundCalcMethod", "BillingMethod",
                         "TotalCostRPT", "TotalPremiumRPT", "TaxSurchargesRPT", "TransactionCostRPT", "TransactionPremiumRPT", "BaseState", "WrittenDate", "RateAsOfDate", "DoNotDestroy",
                         "WorksheetContainer", "TermNumber"},
        EffectiveDatedFields -> {"BillingContact"},
        Coverage -> {"ReferenceDateInternal"},
        Exclusion -> {"ReferenceDateInternal"},
        Modifier -> {"ReferenceDateInternal"},
        PolicyLine -> {"NumAddInsured"}
  ...
  override function shouldCopyArrayElement(targetBean : KeyableBean) : boolean {
    var shouldCopy = !isSideBySideType(typeof targetBean);
    _logger.trace("shouldCopyBean(" + typeof targetBean + ") : " + shouldCopy)
    return shouldCopy
  }

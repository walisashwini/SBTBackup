...
class BAPolicyLineMethods extends AbstractPolicyLineMethodsImpl {
  ...
  override property get AllCoverages() : Coverage[] {
    ...
    covs.addAll(_line.Vehicles.arrays("Coverages").toArray(new BusinessVehicleCov[0]).toList())
    covs.addAll(_line.Jurisdictions.arrays("Coverages").toArray(new BAStateCov[0]).toList())
  ...
  override function canSafelyDeleteLocation( location : PolicyLocation ) : String {
    ...
    if (not (currentOrFutureVehiclesMap["current"] == null or currentOrFutureVehiclesMap["current"].Empty)) {
      ...
      return DisplayKey.get("BusinessAuto.Location.CannotDelete.HasVehicles", currentVehiclesStr)
      ...
      return DisplayKey.get("BusinessAuto.Location.CannotDelete.HasFutureVehicles", futureVehiclesStr, futureDatesStr)
    }
    return super.canSafelyDeleteLocation(location)
  }

  override property get LocationsInUseOnOrAfterSliceDate(): Set<Key> {
    var locationsInUseOnOrAfterSliceDate = _line.VersionList.Vehicles.allVersionsFlat<BusinessVehicle>()
        .where(\veh -> veh.ExpirationDate > _line.SliceDate)*.Location*.FixedId
  ...
  override function shouldQuoteAsynchronously() : boolean {
    return _line.Branch.BusinessAutoLine.Vehicles.Count > AsyncQuoteCoverableThresholdByLine.BAVehicleThreshold
  }
}

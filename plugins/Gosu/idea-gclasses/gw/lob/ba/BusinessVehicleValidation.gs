...
class BusinessVehicleValidation extends PCValidationBase {
  ...
  function vin() {
    ...
    if (Vehicle.Vin == null) {
      Result.addError(Vehicle, TC_DEFAULT, 
        DisplayKey.get("Web.Policy.BA.Validation.NoVin",  Vehicle.VehicleNumber, Vehicle.Location))
    }
  }
  ...
  function lengthOfLeaseRequiredIfLeased() {
    ...
    if (Vehicle.LeaseOrRent and Vehicle.LengthOfLease == null) {
      Result.addError(Vehicle, TC_DEFAULT, 
        DisplayKey.get("Web.Policy.BA.Validation.LengthOfLeaseRequired", Vehicle.VehicleNumber))
    }
  }
  ...
  function addlInterestDetailUnique() {
    ...
    for (detail in Vehicle.AdditionalInterestDetails) {
      ...
      if (thisSet.Count < oldCount - 1) {
        Result.addError(Vehicle, TC_DEFAULT, DisplayKey.get("EntityName.PolicyLine.Validation.AddlInterestDetailUnique", detail.DisplayName), "BusinessVehicles")
  ...
  function mutuallyExclusivePhysDamageCoverages() {
    ...
    if (Vehicle.BAComprehensiveCovExists and Vehicle.BASpecCausesLossCovExists) {
      Result.addError( Vehicle, TC_DEFAULT, DisplayKey.get("Web.Policy.BA.Validation.mutuallyExclusivePhysDamage") )
    }
  }

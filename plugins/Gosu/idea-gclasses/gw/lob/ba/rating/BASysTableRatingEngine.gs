...
class BASysTableRatingEngine extends AbstractRatingEngine<BusinessAutoLine> {
  ...
  // Commercial Auto is rated on a 365-day term
  ...
  protected override function rateSlice(lineVersion : BusinessAutoLine) {
    ...
    if (lineVersion.Branch.isCanceledSlice()) {
      ...
      for (stateCov in lineVersion.Jurisdictions.arrays("Coverages").toArray(new BAStateCov[0])) {
        rateStateCov(stateCov)
      }

      for (vehCov in lineVersion.Vehicles.arrays("Coverages").toArray(new BusinessVehicleCov[0])) {
  ...
  private function rateCancellationShortRatePenalty(lineVersion : BusinessAutoLine) {
    ...
    if ( lineVersion.Branch.RefundCalcMethod == TC_SHORTRATE ) { // context.CancelMethod returns null if the period is not canceled
      var basesMap = getAmountSumByState( getCostsUpToRatedOrder( TC_CANCELSHORTRATEPENALTY ).where( \ cost -> cost.ProrationMethod != TC_FLAT) )
      for ( st in basesMap.Keys ) {
        var subtotal = basesMap.get( st )
        if ( subtotal != 0 ) {
          var p = Prorater.forRounding(Branch.Policy.Product.QuoteRoundingLevel, Branch.Policy.Product.QuoteRoundingMode, TC_PRORATABYDAYS)
          var termSubTotal =  getCostsUpToRatedOrder( BARatedOrderType.TC_CANCELSHORTRATEPENALTY ).where( \ cost -> cost.ProrationMethod != TC_FLAT && cost.JurisdictionArg.State == st ).sum(\ c -> p.prorate(lineVersion.Branch.PeriodStart, p.findEndOfRatedTerm(lineVersion.Branch.PeriodStart, c.NumDaysInRatedTerm),
                                          lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, c.ActualTermAmount))
          var cost = new BAJurisdictionCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, lineVersion.Branch.PreferredSettlementCurrency, RateCache, TC_CANCELSHORTRATEPENALTY,
          jurisMap.get(st).first(),  TC_CANCELSHORTRATEPENALTY, TC_NONSTDPREMIUM, TC_PREMIUM)
          cost.NumDaysInRatedTerm = lineVersion.Branch.NumDaysInPeriod
          cost.Basis              = termSubTotal - subtotal
  ...
  private function rateMinimumPremium(lineVersion : BusinessAutoLine) {
    if (lineVersion.Branch.RefundCalcMethod != TC_FLAT ) {  // context.CancelMethod returns null if the period is not canceled
      var subtotal = getCostsUpToRatedOrder(TC_MINIMUMPREMIUM).sum(\c -> c.ActualAmount)
  ...
  private function rateStateTaxes(lineVersion : BusinessAutoLine) {
    var jurisMap = getRepresentativeJurisdictionAutoMap(lineVersion)
    var basesMap = getAmountSumByState( getCostsUpToRatedOrder( TC_STATETAX ) )
    for ( st in basesMap.Keys ) {
      var subtotal = basesMap.get( st )
      if ( subtotal != 0 ) {
        var jurisdictions = jurisMap.get(st)
        //var jurisdictionFixedId = jurisdictions*.FixedId.max() // get the latest
        var cost = new BAJurisdictionCostData(lineVersion.Branch.PeriodStart, lineVersion.Branch.PeriodEnd, TaxRatingCurrency, RateCache, TC_STATETAX,
            jurisMap.get(st).first(), TC_STATETAX, TC_TAXSURCHARGE, TC_TAXES)
        cost.NumDaysInRatedTerm = lineVersion.Branch.NumDaysInPeriod
        cost.Basis              = subtotal
...
//    cost.RatedOrder         = "CoveragePremium" // AHK - This is currently set when the cost is created; maybe it should live on BACostData instead
  ...
  {
    var antiLockBrakes = vehicle.BusinessVehicleModifiers.firstWhere(\ mod -> mod.Pattern.PublicID == "BAVehAntiLockBrakes").BooleanModifier
  ...
  {
    var antiTheft = (vehicle.BusinessVehicleModifiers.firstWhere(\ mod -> mod.Pattern.PublicID == "BAVehAntiTheft").TypeKeyModifier != null)
  ...
  {
    var safeDrivingCert = vehicle.BusinessVehicleModifiers.firstWhere(\ mod -> mod.Pattern.PublicID == "BAVehSafeDriver").BooleanModifier
  ...
  {
    return new RateAdjFactorSearchCriteria( "bapVehicleCostNew", ratingDate ).matchInRange( vehicle.CostNew as Double, JurisdictionMappingUtil.getJurisdiction(vehicle.Location) )
  }

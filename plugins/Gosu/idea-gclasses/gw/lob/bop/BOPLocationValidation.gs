    ...
    if (Context.isAtLeast(TC_QUOTABLE)) {    
      if (_location.Buildings.IsEmpty) {
        Result.addError(_location, TC_QUOTABLE, 
          DisplayKey.get("Web.Policy.BOP.Validation.AtLeastOneBuildingPerLocation", _location.Location.DisplayName))
      }
    }
  }
  ...
  private function checkWindHailHasOnlyOneDeductible() {
    ...
    if (_location.BOPLocWindHailCovExists) {
      ...
      and cov.BOPWindHailDedTerm.OptionValue.OptionCode != null) {
        Result.addError( cov, TC_DEFAULT, DisplayKey.get("Web.Policy.BOP.Validation.WindHailOnlyOneDeductibleAllowed"))
      }
    }
  }
}

  ...
  function checkRequiredAnswers() : void {
    ...
    for ( answer in _answers ) {
      if ( answer.Question.isQuestionVisible( _container ) and answer.Question.Required and not answer.hasAnswer() ) {
        Result.addError( answer as KeyableBean, TC_QUOTABLE, DisplayKey.get("Validation.Answer.MissingRequired",  answer.Question.Text ), _stepId )
      }
    }
  }
    ...
    for(answer in _answers) {
      if(answer.Question.isQuestionVisible( _container ) and answer.Question.isIncorrect(answer)) {
        if(BlockingAction.TC_WARNUSER.equals(answer.Question.BlockingAction))
          Result.addWarning(answer as KeyableBean, TC_DEFAULT, answer.Question.FailureMessage, _stepId); //show warning on the page without blocking user from advancing further
        if(BlockingAction.TC_BLOCKUSER.equals(answer.Question.BlockingAction))
          Result.addError(answer as KeyableBean, TC_DEFAULT, answer.Question.FailureMessage, _stepId); //block user on a page
      }
    }     
  }
}

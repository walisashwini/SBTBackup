...
abstract class LineSpecificLocationContainerBase<L extends LineSpecificLocation> implements LineSpecificLocationContainer {
  ...
  override function addNewLineSpecificLocation(): LineSpecificLocation {
    return addToLineSpecificLocations(Period.Policy.Account.newLocation(), true)
  }

  override function addToLineSpecificLocations(accountLocation: AccountLocation): LineSpecificLocation {
    return addToLineSpecificLocations(accountLocation, false)
  }

  private function addToLineSpecificLocations(accountLocation: AccountLocation, isNew: boolean): LineSpecificLocation {
    if (accountLocation.Account != null  // builders might have accountLocations not attached to accounts yet because of ordering issues
      ...
      throw DisplayKey.get("Web.Policy.LocationContainer.Location.Error.AccountLocationOnDifferentAccount", accountLocation, _container)
    }
    if (not isNew and LineSpecificLocations.hasMatch(\l -> l.PolicyLocation.AccountLocation == accountLocation)) {
      throw DisplayKey.get("Web.Policy.LocationContainer.Location.Error.AlreadyIsALineSpecificLocation", accountLocation, L.Type.RelativeName)
    }
    var policyLocation: PolicyLocation = null
    if (not isNew) {
      policyLocation = Period.PolicyLocations.firstWhere(\pl -> pl.AccountLocation == accountLocation)
    }

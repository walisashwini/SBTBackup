...
class SideBySideUtil {
  ...
  static function maybeCreateSideBySidePeriods(period : PolicyPeriod, numPeriods : int, quoteAll: boolean) {
    ...
    if ( (job.ActivePeriods.Count+numPeriods) >= PCConfigParameters.SubmissionMaxSideBySideQuotes.getValue()) {
      throw new IllegalStateException(DisplayKey.get("Web.Job.Warning.MaxSideBySideQuotes", job))
    }
    ...
    if (quoteAll) {
      ...
      if (!hasOOSorPreemption(job)) {
        var issues = SideBySideProcess.getQuotes(targetPeriods?.toSet())
  ...
  public static function addNewSideBySidePeriod(period : PolicyPeriod) : PolicyPeriod {
    ...
    if (job.SideBySide) {
      ...
      throw new IllegalStateException(DisplayKey.get("Web.Job.Warning.CannotAddToNonSideBySideJob"))
    }
  }
   ...
   * is made for each slice date defined in sliceDates. Base data copy is not performed if the passed
   * in period is withdrawn and not active.
   *
  ...
  static function executeSideBySideCopy(period : PolicyPeriod, sliceDates : Date[]) {
    if (!period.Job.SideBySide or
        period.UWLockedAndNoOverride or !period.isActive()) {
      return
    }
    ...
    if (validationContext != null) {
      ...
      if (policyPeriod.FailedOOSEValidation) {
        ...
        var errorMessages(list : List<IProductModelSyncIssueWrapper>)
          = \ issues -> {
              issues.where(\i -> (i as ProductModelSyncIssueWrapper).ShouldDisplayDuringQuote)
                    .each(\i -> jobWizHelper.addSyncIssueToWebMessages(i as ProductModelSyncIssueWrapper))
            }
  ...
  static function getDisplayValue(pLine : PolicyLinePattern, pattern : ProductModelClass) : String {
    var dkTypeName = "Web.Job.SideBySide." + ProductModelTypeHelper.getEntityType(pattern).RelativeName + "." + pLine.PublicID
    var dkValue = getDisplayKeyValue(dkTypeName, pattern.PublicID)
    return dkValue == null ? pattern.DisplayName : dkValue
  }

  static function getDisplayKeyValue(root : String, suffix : String) : String {
    var dispName = DisplayKey.get(root)
    if (suffix.length > 0) {
      dispName += "." + suffix
    }
    var dkValue = DisplayKey.get(dispName)
    if (dkValue != dispName) {
      return dkValue
    } else {
      return null
   ...
   **/
   static function getPeriodWarningsMessage(period : PolicyPeriod, validationContext : PCValidationContext, sxsException : Exception ) : String {
    ...
    if (period.Job.OOSJob and period.OOSConflicts.HasElements) {
      result.add(DisplayKey.get("Web.Job.SideBySide.Errors.UnhandledOOSConflict"))
    }

    if (period.FailedOOSEValidation) {
      result.add(DisplayKey.get("Web.Job.SideBySide.Errors.FailedOOSEValidation"))
    }

    if (period.hasAnyUnhandledPreemptions()) {
      result.add(DisplayKey.get("Web.Job.SideBySide.Errors.UnhandledPreemption"))
    }
    ...
    if ( (sxsException != null) and
      ...
      if (sxsException typeis DisplayableException) {
        if (sxsException.LocalizedMessages.Count > 0) {
          sxsException.LocalizedMessages.each(\msg -> {
            result.add(msg)
          })
        } else {
          result.add(DisplayKey.get("Web.Job.SideBySide.Errors.UnexpectedException"))
        }
      } else {
    ...
    if (period.QuoteHidden) {
      result.add(DisplayKey.get("Web.Job.SideBySide.Errors.QuoteHidden"))
    }
    var validation = validationContext.Result
    if (validation.Errors.Count > 0) {
      result.add(DisplayKey.get("Web.Job.SideBySide.Errors.ValidationErrors"))
    }
    if (validation.Warnings.Count > 0) {
      result.add(DisplayKey.get("Web.Job.SideBySide.Errors.ValidationWarnings"))
    }
  ...
  static function calculateBlockingPointErrorText(period : PolicyPeriod) : String {
    ...
    if (earliestBP == null or earliestBP == UWIssueBlockingPoint.TC_NONBLOCKING) {
      ...
      return DisplayKey.get("Web.Job.SideBySide.Errors.UWIssuesBlockQuote")
    } else if (!earliestBP.comesAfter(UWIssueBlockingPoint.TC_BLOCKSRATERELEASE)) {
      return DisplayKey.get("Web.Job.SideBySide.Errors.UWIssuesBlockRate")
      ...
      return DisplayKey.get("Web.Job.SideBySide.Errors.UWIssuesBlockBind")
    } else if (!earliestBP.comesAfter(UWIssueBlockingPoint.TC_BLOCKSISSUANCE)) {
      return DisplayKey.get("Web.Job.SideBySide.Errors.UWIssuesBlockIssuance")
    } else {
      return DisplayKey.get("Web.Job.SideBySide.Errors.UWIssuesBlockOther")
    }
  }
   ...
   * LastResult.
   *
   * If the period is rated then requestPostRatingQuote will be run automatically.
   *
  ...
  static function returnToSelectedPeriod(period : PolicyPeriod,
    ...
    // Finish the quote on the fly if it's in rated status
    if (period.Status == TC_RATED) {
      try {
        period.JobProcess.requestPostRatingQuote(jobWizHelper)
      } catch (e: UWAuthorityBlocksProgressException) {
        // We don't care about the UWAuthorityBlocksProgressException at this point since UWIssues are
        // already generated and attached to the period, so do nothing here but silence the exception.
      }
    }
  ...
  /**
   * Opens the passed in period for edit if rated/quoted then syncs the
   * periods to the offering currently set on the period.
   * @param period The period to which we will open for edit if rated/
   *               quoted and sync its offering.
   */
  @Deprecated(:value="Use PolicyPeriod#editIfRatedOrQuotedAndForceSync() instead.", :version="10.0.0")
  static function editIfRatedOrQuotedAndForceSync(period: PolicyPeriod) {
    period.editIfRatedOrQuotedAndForceSync()
  }
  ...
  static function setLastResultFromValidationContextAndGetQuotes(policyPeriods: PolicyPeriod[], periodInfos: SideBySidePolicyPeriodInfo[]) {
    if (null!=policyPeriods and policyPeriods.HasElements and (policyPeriods.Count == periodInfos.Count)) {
      ...
      periodInfos.each(\pi -> {
        pi.SxSException = null
      })
    }

    var quoteIssues = SideBySideProcess.getQuotes(policyPeriods?.toSet())
    copyQuoteIssuesIntoPolicyPeriods(quoteIssues, policyPeriods?.toList())
  ...
  private static function displayExceptions(issues : List<Pair<PolicyPeriod,Exception>>) {
    var exceptionMsg = DisplayKey.get("Web.Job.SideBySide.Errors.QuoteException") + "\n"
  ...
  public static function buildListOfVehicleMaps(periodInfos : SideBySidePolicyPeriodInfo[]) : List<Map<SideBySidePolicyPeriodInfo, PersonalVehicle>> {
    ...
    periodInfos*.Period*.PersonalAutoLine.arrays("Vehicles").toArray(new PersonalVehicle[0]).each(\vehicle -> {
  ...
  static function getSideBySideBlockingMessages(periods : PolicyPeriod[]) : List<String> {
    ...
    if (periods.hasMatch(\ p -> p.hasAnyUnhandledPreemptions())) {
      result.add(DisplayKey.get("Web.Job.SideBySide.Errors.ResolveUnhandledPreemption"))
    }

    var job = periods.first().Job
    if (null!=job and job.OOSJob and job.ActivePeriods.hasMatch(\ p -> p.OOSConflicts.HasElements)) {
      result.add(DisplayKey.get("Web.Job.SideBySide.Errors.ResolveUnhandledOOSConflicts"))
    }
    return result
  }
  ...
  static function syncCoveragesAndRefresh(periodInfo : SideBySidePolicyPeriodInfo, jobWizHelper : JobWizardHelper) {
    ...
    if (jobWizHelper == null) {
      ...
      periodInfo.Period.AllCoverables.each(\c -> {
          var currentIssues = periodInfo.SxSCache.CovIssues.computeIfAbsent(c, \k -> c.syncCoverages())
      ...
      if (issues.HasElements) {
        jobWizHelper.addSyncIssueToWebMessages(mostSevere, DisplayKey.get("Web.Job.SideBySide.ValidationIssueMessageForBranch", periodInfo.Period.BranchName))
  ...
  static function withdrawSelectedPeriod(periodInfo : SideBySidePolicyPeriodInfo, jobWizHelper : JobWizardHelper) {
    ...
    if (not period.JobProcess.canWithdraw().Okay ) {
      jobWizHelper.addErrorWebMessage(DisplayKey.get("Java.PolicyPeriod.WithdrawPeriodFailed", period.BranchName))
    } else {
   ...
   */
  @Deprecated(:value="Use SideBySideUIHelper.attemptSideBySide(numPeriods: int) instead.", :version="10.0.0")
   ...
   */
  @Deprecated(:value="Use SideBySideUIHelper#canAttemptSideBySide() instead.", :version="10.0.0")
  ...
  static function canAttemptSideBySide(policyPeriod : PolicyPeriod, job : Job, additionalStatus : PolicyPeriodStatus) : boolean {
    ...
    var isStatusFulfilled = PolicyPeriodStatusSets.SIDE_BY_SIDE_EDITABLE_STATUSES.contains(status) or
        (additionalStatus == null ? false : status == additionalStatus)

    return !job.SideBySide and !job.hasMultiplePeriods() and isStatusFulfilled
  }
   ...
   */
  @Deprecated(:value="Use SideBySideUIHelper#getVersionLabel() instead.", :version="10.0.0")

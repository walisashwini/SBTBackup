...
abstract class AbstractAutoNumbersContainerAdapter<T extends EffDated> implements AutoNumbersContainer {
  ...
  override function number(elementType : IType, entity : EffDated) {
  ...
  override function renumber(elementType : IType) {
    var props = NumberPropertiesMap.get(elementType)
    var sequence = _owner[props.SequenceProp.Name] as AutoNumberSequence
  ...
  private function currentAndFutureEntities(arrayProp : IPropertyInfo) : T[] {
    ...
    var currentAndFutureList : List<EffDated> = {}// = (_owner[arrayProp.Name]?.asArrayOf(EffDated)).toList()
    if (_owner[arrayProp.Name] != null) {
      currentAndFutureList = Coercions.cast<EffDated[]>(_owner[arrayProp.Name]).toList()
    }
    period.OOSSlices.each(\p -> {
      var container = containerInBranch(p)
      if(container != null and container[arrayProp.Name] != null) {
        (Coercions.cast<EffDated[]>(container[arrayProp.Name])).each(\entity -> {
          if(not currentAndFutureList.contains(entity)) currentAndFutureList.add(entity)
        })
      }
    })
    return Coercions.cast<T[]>(currentAndFutureList.toTypedArray())
  }

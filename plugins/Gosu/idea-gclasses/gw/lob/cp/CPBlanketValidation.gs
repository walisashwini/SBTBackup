...
class CPBlanketValidation extends PCValidationBase {
  ...
  function validateBlanketLimit(){
    ...
    if(Blanket.BuildingCoverages.HasElements){
      if (buildingLimitSum != blanketLimit as double) {
        Result.addWarning(Blanket, TC_DEFAULT, DisplayKey.get("Web.Policy.CP.Validation.BlanketLimitDoesNotMatchBlanketedCovLimit"))
      }
    }
  }
      ...
      if (Blanket.BuildingCoverages.HasElements) {
        ...
        for (cov in Blanket.BuildingCoverages) {
          if (getCoinsuranceMismatch(cov) or getDeductibleMismatch(cov)) {
            Result.addError(Blanket, TC_DEFAULT, DisplayKey.get("Web.Policy.CP.Validation.MustHaveMatchingDeductibleAndCoinsurance", Blanket.CPBlanketNum), "CPBlanket")
          }
        }
      }
    }
  ...
  function validateAtleastTwoCoveragePerBlanket(){
    ...
    if(Blanket.BuildingCoverages.Count < 2){
      Result.addWarning(Blanket, TC_DEFAULT,DisplayKey.get("Web.Policy.CP.Validation.MustHaveAtLeastTwoCoverage"))
    }
  }

...
enhancement GLCostSetEnhancement<T extends GLCost> : Set<T> {
  ...
  reified function getOtherPremiumAndSurcharges(currency : Currency) : GL_CostWrapper[] {
    ...
    var header1 = DisplayKey.get("Web.SubmissionWizard.Quote.GL.Subtotal.TotalPremium", state)
    ...
    var header2 = DisplayKey.get("Web.SubmissionWizard.Quote.GL.Subtotal.TotalCost", state)
    ordered.add(new GL_CostWrapper(10000000, header2, this.AmountSum(currency), true))
    return ordered.toTypedArray()
  }

  reified function getOtherPremiumAndSurchargesForAudit(currency : Currency) : AuditCostWrapper[] {
  ...
  private reified function getAuditCostWrapperForTotalPremium(currency : Currency) : AuditCostWrapper {
    var description = DisplayKey.get("Web.SubmissionWizard.Quote.GL.Subtotal.TotalPremium", this.first().State)
  ...
  private reified function getAuditCostWrapperForTotalCost(currency : Currency) : AuditCostWrapper  {
    var description = DisplayKey.get("Web.SubmissionWizard.Quote.GL.Subtotal.TotalCost", this.first().State)

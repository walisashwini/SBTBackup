...
enhancement IMLocationEnhancement : entity.IMLocation {
  ...
  function hasSign() : boolean {
    ...
    if (imSingPart.IMSigns != null) {
      return imSingPart.IMSigns.hasMatch(\ i -> i.IMLocation.PublicID == this.PublicID)
    }
    return false
  }
  ...
  function isReferencedByAccountsReceivable() : boolean {
    if( this.IMLine.IMAccountsRecPart.IMAccountsReceivables != null) {
      return this.IMLine.IMAccountsRecPart.IMAccountsReceivables.hasMatch(\ ar -> ar.IMBuilding.IMLocation.FixedId == this.FixedId)
    }
     return false
  }

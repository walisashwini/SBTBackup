...
class IMPolicyLineMethods extends AbstractPolicyLineMethodsImpl {
  ...
  override property get AllCoverages() : Coverage[] {
    ...
    if (_line.IMSignPart != null) {
      coverages.addAll(_line.IMSignPart.IMSigns.arrays("Coverages").toArray(new entity.IMSignCov[0]).toList())
    }
    ...
    if (_line.ContractorsEquipPart != null) {
      ...
      coverages.addAll(_line.ContractorsEquipPart.ContractorsEquipments.arrays("Coverages").toArray(new ContractorsEquipCov[0]).toList())
    }
    ...
    if( _line.IMAccountsRecPart != null ) {
      ...
      coverages.addAll(_line.IMAccountsRecPart.IMAccountsReceivables.arrays("Coverages").toArray(new IMAccountsRecCov[0]).toList())
    }
  ...
  override property get AllExclusions() : Exclusion[] {
    ...
    return exclusions?.toTypedArray()
  }
  ...
  override property get AllCoverables() : Coverable[] {
    ...
    if (imsignpart != null) {
      list.addAll( imsignpart.IMSigns?.toList() as Collection<Coverable> )
    }
    var contrEqPart = _line.ContractorsEquipPart
    if (_line.ContractorsEquipPart != null) {
      list.add( contrEqPart )
      list.addAll( contrEqPart.ContractorsEquipments?.toList() as Collection<Coverable> )
    }
    var accountsRecPart = _line.IMAccountsRecPart
    if( accountsRecPart != null ) {
      list.add( accountsRecPart )
      list.addAll(accountsRecPart.IMAccountsReceivables?.toList() as Collection<Coverable>)
    }
  ...
  override function canSafelyDeleteLocation(location : PolicyLocation) : String {
    ...
    if(currentOrFutureIMLocationsAtLocation["current"].HasElements)
      return DisplayKey.get("InlandMarine.Location.CannotDelete.HasIMLocation", location)
    if(currentOrFutureIMLocationsAtLocation["future"].HasElements) {
      ...
      return DisplayKey.get("InlandMarine.Location.CannotDelete.HasFutureIMLocation", location, futureDatesStr)
    }
    return super.canSafelyDeleteLocation(location)
  }

  override property get LocationsInUseOnOrAfterSliceDate(): Set<Key> {
    var imReferencedLocations = _line.VersionList.IMLocations.allVersionsFlat<IMLocation>().where(\imLoc -> imLoc.ExpirationDate > _line.SliceDate)*.PolicyLocation*.FixedId
    return imReferencedLocations.union({_line.Branch.PrimaryLocation.FixedId})
  }
  ...
  override protected function getCannotDeleteBuildingMessage(building : Building) : String {
    return DisplayKey.get("InlandMarine.Building.CannotDelete.HasIMBuilding", building)
  }

  override protected function getCannotDeleteBuildingFutureMessage(building : Building, dates : String) : String {
    return DisplayKey.get("InlandMarine.Building.CannotDelete.HasFutureIMBuilding", building, dates)
  }
  ...
  override function shouldQuoteAsynchronously() : boolean {
    return _line.Branch.IMLine.IMSignPart.IMSigns.Count >  AsyncQuoteCoverableThresholdByLine.IMSignThreshold

...
class PALineVehiclesValidator extends PolicyLineValidation<entity.PersonalAutoLine>  {
  ...
  private function vinIsUniqueInPeriod() {
    ...
    if (vehicles.map(\ v -> v.Vin).toSet().Count != vehicles.Count) {
      Result.addError(paLine, ValidationLevel.TC_DEFAULT, DisplayKey.get("Web.Policy.PA.Validation.VinNumbers"), VEHICLES_WIZARD_STEP)
    }
  }
  ...
  function allGaragesInSameState() {
    ...
    if (Line.Branch.Job typeis PolicyChange) {
      ...
      errorMessage = DisplayKey.get("Web.Policy.PA.Validation.ChangeBaseState", baseState)
    } else {
      baseState = paLine.Vehicles.first().GarageLocation.State
      errorMessage = DisplayKey.get("Web.Policy.PA.Validation.GaragesInSameState")
    }
    if (paLine.Vehicles.hasMatch(\ vehicle -> vehicle.GarageLocation.State != baseState)) {
      Result.addError(paLine, ValidationLevel.TC_DEFAULT, errorMessage, VEHICLES_WIZARD_STEP)
    }
  }

  private function expiringGarageOOSWarning() {
    if (Line.Branch.Job.isOOSJob()) {
      paLine.Vehicles.each(\vehicle -> {
        if (Line.Branch.EditEffectiveDateRange.includes(vehicle.EffectiveDate)) {
          var vehicleExpirationDate = vehicle.ExpirationDate
          var garageExpirationDate = vehicle.GarageLocation.VersionList.getAllVersions().map(\elt -> elt.ExpirationDate).max()
          if (garageExpirationDate < vehicleExpirationDate) {
            var garageExpiresMsg = DisplayKey.get("Web.Policy.PA.Validation.GarageExpiringEndsVehicleWorkaround", vehicle.VehicleNumber, garageExpirationDate)
            Result.addWarning(paLine, ValidationLevel.TC_DEFAULT, garageExpiresMsg, VEHICLES_WIZARD_STEP)
          }
        }
      })
    }

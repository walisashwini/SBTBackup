...
enhancement PersonalAutoLineEnhancement : entity.PersonalAutoLine {
   ...
   * </ul>
   * Also creates coverages, conditions, and exclusions on this vehicle
  ...
  function createAndAddVehicle() : PersonalVehicle {
    ...
    vehicle.createCoveragesConditionsAndExclusions()
  ...
  */
  ...
  function addNewPolicyDriverForContact(contact : Contact) : PolicyDriver {
    if (this.PolicyDrivers.hasMatch(\ driver -> driver.AccountContactRole.AccountContact.Contact == contact)) {
      throw new DisplayableException(DisplayKey.get("Web.PolicyDriver.Error.AlreadyExists", contact))
    }
    var policyDriver = this.Branch.addNewPolicyContactRoleForContact(contact, TC_POLICYDRIVER) as PolicyDriver
  ...
  function addNewPolicyDriverOfContactType(contactType : typekey.ContactType) : PolicyDriver {
    ...
    acctContact.addNewRole( TC_DRIVER )
  ...
  property get UnassignedDrivers() : AccountContact[] {
    ...
    var candidates = this.Branch.Policy.Account.getAccountContactsWithAnyRole({TC_DRIVER, TC_ACCOUNTHOLDER, TC_NAMEDINSURED, TC_SECONDARYCONTACT})
      .where(\ ac -> plugin.canBeRole(ac.ContactType, TC_DRIVER))
      .toSet()
  ...
  /**
   * Get a booleanAnswer of QuestionCode
   * @param questionCode: String
   * @return boolean
   */
  function getQuestionBooleanValue(questionCode: String) : boolean {
    var branch = this.Branch
     return(branch.PeriodAnswers.singleWhere(\e->e.QuestionCode == questionCode).BooleanAnswer)
  }
}

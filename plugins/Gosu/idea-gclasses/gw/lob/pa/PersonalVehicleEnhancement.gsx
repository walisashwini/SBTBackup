...
enhancement PersonalVehicleEnhancement : PersonalVehicle {
  ...
  property get VehicleCost() : MonetaryAmount {
    ...
    var covPatterns = this.PolicyLine.Pattern.getCoverageCategoryByPublicId("PAPPhysDamGrp").coveragePatternsForEntity(PersonalVehicle)
  ...
  property get LineCost() : MonetaryAmount {
    ...
    var liabilityCategory = this.PolicyLine.Pattern.getCoverageCategoryByPublicId("PAPLiabGrp")
    ...
    for (covPattern in liabilityCovPatterns) {
      var costs = this.PALine.getAllCostsForCoverage(this.PALine, covPattern)
          .cast(PACost).where(\pvcc -> pvcc.Vehicle == this) // Use cost that corresponds with vehicle
      if (costs.size() > 0) {
        total += costs.first().ActualAmountBilling
      }
    }
    var pipCategory = this.PolicyLine.Pattern.getCoverageCategoryByPublicId("PAPip")
    if (pipCategory != null) {

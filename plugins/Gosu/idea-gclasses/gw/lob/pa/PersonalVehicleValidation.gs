...
class PersonalVehicleValidation extends PCValidationBase {
  ...
  */
  function lengthOfLeaseRequiredIfLeased() {
    ...
    if (Vehicle.LeaseOrRent and Vehicle.LengthOfLease == null) {
      Result.addFieldError(Vehicle, "LengthOfLease", TC_DEFAULT, 
          DisplayKey.get("Web.Policy.PA.Validation.LengthOfLeaseRequired"), VEHICLES_WIZARD_STEP)
    }
  }
  ...
  */
  function addlInterestDetailUnique() {
    ...
    for (detail in Vehicle.AdditionalInterestDetails) {
      ...
      if (thisSet.Count < oldCount - 1) {
        Result.addError(Vehicle, TC_DEFAULT, DisplayKey.get("EntityName.PolicyLine.Validation.AddlInterestDetailUnique", detail.DisplayName), VEHICLES_WIZARD_STEP)
  ...
  /**
   * Checks Vehicle required fields: Type, VIN, LicenseState, CostNew, ModelYear, Make, Model
   */
  function requiredFields() {
    Context.addToVisited(this, "requiredFields")
    if (Vehicle.VehicleType == null) {
      Result.addFieldError(Vehicle, "VehicleType", TC_DEFAULT, 
          DisplayKey.get("Web.Policy.PA.Validation.VehicleTypeRequired"), VEHICLES_WIZARD_STEP)
    }

    if (Vehicle.LicenseState == null) {
      Result.addFieldWarning(Vehicle, "LicenseState", TC_DEFAULT,
          DisplayKey.get("Web.Policy.PA.Validation.LicenseStateRequired"), VEHICLES_WIZARD_STEP)
    }

    if (not Vehicle.Vin.HasContent) {
      Result.addFieldError(Vehicle, "Vin", TC_DEFAULT, 
          DisplayKey.get("Web.Policy.PA.Validation.VINRequired"), VEHICLES_WIZARD_STEP)
    }
    if (Vehicle.CostNew == null) {
      Result.addFieldError(Vehicle, "CostNew", TC_DEFAULT, 
          DisplayKey.get("Web.Policy.PA.Validation.CostNewRequired"), VEHICLES_WIZARD_STEP)
    } else  if (not Vehicle.CostNew.IsPositive) {
      Result.addFieldError(Vehicle, "CostNew", TC_DEFAULT, 
          DisplayKey.get("Web.Policy.PA.Validation.PositiveCostNewRequired"), VEHICLES_WIZARD_STEP)
    }

    if (Vehicle.Year == null) {
      Result.addFieldError(Vehicle, "Year", TC_QUOTABLE,
          DisplayKey.get("Web.Policy.PA.Validation.ModelYearRequired"), VEHICLES_WIZARD_STEP)
    }

    if (Vehicle.Make == null) {
      Result.addFieldError(Vehicle, "Make", TC_QUOTABLE,
          DisplayKey.get("Web.Policy.PA.Validation.MakeRequired"), VEHICLES_WIZARD_STEP)
    }

    if (Vehicle.Model == null) {
      Result.addFieldError(Vehicle, "Model", TC_QUOTABLE,
          DisplayKey.get("Web.Policy.PA.Validation.ModelRequired"), VEHICLES_WIZARD_STEP)
    }
  }

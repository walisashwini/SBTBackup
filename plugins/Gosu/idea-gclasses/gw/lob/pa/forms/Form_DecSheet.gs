...
class Form_DecSheet extends PAFormData
{
  ...
  override function populateInferenceData( context: FormInferenceContext, specialCaseStates: Set<Jurisdiction> ) : void {
    ...
    var namedInsuredsSet = context.Period.PolicyLocations.arrays("LocationNamedInsureds").toArray(new LocationNamedInsured[0])*.NamedInsured*.AccountContactRole*.AccountContact*.Contact*.DisplayName.toSet()
    ...
    _drivers = (paLine.PolicyDrivers?.toList()).sortBy( \ d -> d.DisplayName )
    _vehicles = (paLine.Vehicles?.toList()).sortBy( \ v -> v.DisplayName )
    ...
    _totalTax = period.TaxAndSurchargesRPT.Amount as String

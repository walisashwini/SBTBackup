...
uses entity.PersonalAutoCov
uses entity.PersonalVehicleCov
...
class PARatingEngine extends AbstractRatingEngine<PersonalAutoLine> {
  ...
  /**
   * Various rate routine names
   * For tests we want the routine names to be unique to the test. This will only work correctly in single-threaded mode.
   * Don't remove this initialization, and don't reset in constructor.
   */
  static var _pa_veh_cov_premium_rr: String              as PAVehCovPremiumRR              = "pa_veh_cov_premium_rr"
  static var _pa_cov_premium_rr: String                  as PACovPremiumRR                 = "pa_cov_premium_rr"
  static var _pa_cov_flatrate_rr: String                 as PACovFlatrateRR                = "pa_cov_flatrate_rr"
  static var _pa_cancellation_short_rate_penalty: String as PACancellationShortRatePenalty = "pa_cancellation_short_rate_penalty"
  static var _pa_assign_driver_style1_rr: String         as PAAssignDriverStyle1RR         = "pa_assign_driver_style1_rr"
  static var _pa_assign_driver_style2_rr: String         as PAAssignDriverStyle2RR         = "pa_assign_driver_style2_rr"
  static var _pa_pip_nj_basic_rr: String                 as PAPipNJBasicRR                 = "pa_pip_nj_basic_rr"
  static var _pa_state_tax_rr: String                    as PAStateTaxRR                   = "pa_state_tax_rr"
  ...
  construct(line : PersonalAutoLine, minimumRatingLevel : RateBookStatus) {
    ...
    _rateBook = getRateBook(line.Branch.PeriodStart, line.Branch.Offering.PublicID)
  }

  /**
   * For tests.
   */
  static function resetRateRoutineNames() {
    PAVehCovPremiumRR              = "pa_veh_cov_premium_rr"
    PACovPremiumRR                 = "pa_cov_premium_rr"
    PACovFlatrateRR                = "pa_cov_flatrate_rr"
    PACancellationShortRatePenalty = "pa_cancellation_short_rate_penalty"
    PAAssignDriverStyle1RR         = "pa_assign_driver_style1_rr"
    PAAssignDriverStyle2RR         = "pa_assign_driver_style2_rr"
    PAPipNJBasicRR                 = "pa_pip_nj_basic_rr"
    PAStateTaxRR                   = "pa_state_tax_rr"
  }
  ...
  //
  // This is one way of rating for a six month term: use the actual six months from the original start of the term.
  ...
  private function rateVehicleCoverage(cov : PersonalVehicleCov, assignedDriver : VehicleDriver) {
    ...
    // call rate routine (which fills in BaseRate, AdjustedRate, and TermAmount
    RateBook.executeCalcRoutine(_pa_veh_cov_premium_rr, data, data, rateRoutineParameterMap)
  ...
  private function getAssignedDriverUsingStyle1(veh : PersonalVehicle) : VehicleDriver {
    ...
    RateBook.executeCalcRoutine(_pa_assign_driver_style1_rr, null, worksheetContainer, assignDriverParameterMap)
    ...
    PolicyLine.addRatingWorksheet(veh, ws)
  ...
  protected function getAssignedDriverUsingStyle2(veh : PersonalVehicle) : VehicleDriver {
    ...
    veh.Drivers.each(\ currentDriver -> {
      ...
      var worksheetContainer = new NoCostWorksheetContainer()
      RateBook.executeCalcRoutine(_pa_assign_driver_style2_rr, null, worksheetContainer, assignDriverParameterMap)
      var ws = new Worksheet() {:Description = "Driver assignment for vehicle ${veh.DisplayName}, driver ${currentDriver.DisplayName}" ,
        :WorksheetEntries = worksheetContainer.WorksheetEntries}
      PolicyLine.addRatingWorksheet(veh, ws, currentDriver.PublicID)
    })
  ...
  protected function rateLineCoverage(cov : PersonalAutoCov, vehicle : PersonalVehicle) {
    ...
    switch (typeof cov) {
      ...
      case PAUMPDCov:
        ...
        rateLineCoverage(cov, vehicle, _pa_cov_premium_rr)
        break
      case PALimitedMexicoCov :
        rateLineCoverage(cov, vehicle, _pa_cov_flatrate_rr)
        break
      case PAPIP_NJ:
  ...
  private function ratePAPIP_NJBasic(cov : PAPIP_NJ, vehicle : PersonalVehicle) {
    ...
    RateBook.executeCalcRoutine(_pa_pip_nj_basic_rr, data, data, rateRoutineParameterMap)
  ...
  private function rateCancellationShortRatePenalty(line : PersonalAutoLine) {
    ...
    if (line.Branch.RefundCalcMethod == TC_SHORTRATE and subtotal != 0) {
      ...
      RateBook.executeCalcRoutine(_pa_cancellation_short_rate_penalty,  cost, cost, rateRoutineParameterMap)
  ...
  private function rateTaxes(line : PersonalAutoLine) {
    ...
    if (subtotal != 0) {
      ...
      RateBook.executeCalcRoutine(_pa_state_tax_rr, cost, cost, rateRoutineParameterMap)
      cost.copyStandardColumnsToActualColumns()

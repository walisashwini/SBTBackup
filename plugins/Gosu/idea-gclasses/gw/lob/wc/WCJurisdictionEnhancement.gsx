...
enhancement WCJurisdictionEnhancement: WCJurisdiction {
  property get AnniversaryDate(): Date {
    return Coercions.makeDateFrom(this.getFieldValue("AnniversaryDateInternal"))
  }
  ...
  property set AnniversaryDate(value: Date) {
    if (value == null) {
      throw new DisplayableException(DisplayKey.get("Java.Job.WC.AnniversaryNotNull"))
    }
  ...
  property get ReferenceDate(): Date {
    return Coercions.makeDateFrom(this.getFieldValue("ReferenceDateInternal"))
  }
  ...
  function getRPSD(startDate: Date, type: RPSDType): RatingPeriodStartDate {
    ...
    if (ratingPeriodsList.isEmpty()) {
      ...
      throw new IllegalStateException(DisplayKey.get("WorkersComp.Jurisdiction.MultipleRPSDs", startDate, type))
    } else {
  ...
  function getPriorRatingDate(date: Date): Date {
    ...
    if (anniversary == null and this.WCLine != null) {
      var effDate = PeriodStart
      var wcAnni = this.AnniversaryDate
      if (wcAnni != null) {
    ...
    if (anniversary != null) {
      ...
      ratingDateList.add(0, PeriodStart)
    }
    ratingDateList.add(PeriodEnd)
    ...
    // we got this far but did not find an acceptable date - if date is between period start & end, use period start
    if (DateUtil.compareIgnoreTime(date, PeriodStart) >= 0 and DateUtil.compareIgnoreTime(date, PeriodEnd) < 0) {
      return PeriodStart
    }
    throw new IllegalStateException(DisplayKey.get("WorkersComp.Jurisdiction.NoDateFound", date, StringUtils.join(ratingDateList.iterator(), ", ")))
  }

  function calculateUpcomingAnniversaryDate(): Date {
    var anniversaryCal = getTrimmedCalendar(this.AnniversaryDate)
    var effectiveCal = getTrimmedCalendar(PeriodStart)
  ...
  property get AuditRatingPeriods(): List<WCRatingPeriod> {
    ...
    var rpsds = this.getSingleVersionsOfRatingPeriodStartDates().map(\rpsd -> rpsd.StartDate).where(\d -> d > start and d < end).toList()
    rpsds.add(start)
    rpsds.add(end)
  ...
  function canAddRPSD(date: Date): String {
    ...
    if (SplitDates.hasMatch(\r -> r == date)){
      return DisplayKey.get("Web.Policy.WC.DuplicatedDate", this.State, date.ShortFormat)
    }
    if (not this.Branch.includes(date)) {
      return DisplayKey.get("Java.Job.WC.RPSDOutsidePeriod", date, PeriodStart, PeriodEnd)
    }
    return null
  }

  function addRatingPeriodStartDateWithNoARDCheck(splitDate: Date, type: RPSDType): RatingPeriodStartDate {
    var date = DateTimeUtil.setHourMinuteSecondFromDate(splitDate, PeriodStart)
    var rpsd = new RatingPeriodStartDate(this.Branch)
    rpsd.StartDate = date
    rpsd.Type = type
    this.addToRatingPeriodStartDates(rpsd)
    rpsd = rpsd.getUnsliced()
    rpsd.EffectiveDate = PeriodStart
    return rpsd
  }
  ...
  function getSingleVersionsOfRatingPeriodStartDates(): RatingPeriodStartDate[] {
  ...
  function updateRPSDsAfterPeriodChange(oldPeriodStart: Date) {
    ...
    for (rpsdVL in ratingPeriodStartDates) {
      ...
      if (versions.size() != 1) {
        throw new IllegalStateException(DisplayKey.get("WorkersComp.Jurisdiction.CannotSplitRPSD"))
      }
    ...
    var anniDate = this.AnniversaryDate
  ...
  function updateRPSDsInNewPeriod() {
    ...
    var oldARD1 = this.getBasedOn().AnniversaryDate
    var oldARD2 = DateTimeUtil.addYears(this.getBasedOn().AnniversaryDate, 1)
    var periodStart = PeriodStart
  ...
  function updateExposures() {
    for (beanVL in this.WCLine.VersionList.WCCoveredEmployees) {
      ...
      if (location != null and location.State == StateJurisdictionMappingUtil.getStateMappingForJurisdiction(this.getState())) {
  ...
  function getPossibleGoverningLaws(): SpecialCov[] {
    var typeKeys = SpecialCov.getTypeKeys(false)
        .where(\typeKey -> typeKey.hasCategory(typekey.LiabilityAct.TC_WORKERSCOMP))
  ...
  private function checkIncludesDateIgnoreTime(date: Date) {
    if (PeriodStart.compareIgnoreTime(date) > 0 or PeriodEnd.compareIgnoreTime(date) <= 0) {
      throw DisplayKey.get("WorkersComp.Jurisdiction.DateOutsidePeriod", date, PeriodStart, PeriodEnd)
    }
  }
  ...
  private function splitWCCoveredEmployees() {
    var ratingPeriodStartDates = getSingleVersionsOfRatingPeriodStartDates()

...
class WCLineValidation extends PolicyLineValidation<entity.WorkersCompLine> {
  ...
  function payrollForAllPolicyLocations() {
    ...
    for (location in wcLine.PolicyLocations) {
      ...
      if (coveredEmployees.Empty) {
        if (location.isInUse()) {
          Result.addWarning(location, TC_QUOTABLE,
              DisplayKey.get("Web.Policy.WC.Validation.LocationWithoutPayroll", {location}))
        } else {
          Result.addWarning(location, TC_QUOTABLE,
              DisplayKey.get("Web.Policy.WC.Validation.LocationForNonCoveredState", {location}))
        }
      }
    }
  }
  ...
  function industryCodeHasValue() {
    ...
    if (Context.isAtLeast(TC_QUOTABLE) and wcLine.Branch.PrimaryNamedInsured.IndustryCode == null) {
      Result.addError( wcLine, TC_DEFAULT, DisplayKey.get("Web.Policy.WC.Validation.IndustryCode"), "PolicyInfo")
    }
  }
  
  function atLeastOneJurisdiction() {
    Context.addToVisited(this, "atLeastOneJurisdiction")
    if (wcLine.Jurisdictions.IsEmpty and Context.isAtLeast(TC_DEFAULT)) {
      var msg = DisplayKey.get("Web.Policy.WC.Validation.AtLeastOneJurisdiction")
      if (Context.isAtLeast(TC_QUOTABLE)) {
        Result.addError(wcLine, TC_QUOTABLE, msg, "WorkersCompCoverageConfig")
      } else {
        Result.addWarning(wcLine, TC_DEFAULT, msg, "WorkersCompCoverageConfig")
      }
    }
  }
  ...
  function receivedDateFilledInPriorToSubmit() {
    if (wcLine.Branch.Job typeis Audit) {
      if (wcLine.Branch.Audit.AuditInformation.ReceivedDate == null) {
        Result.addError(wcLine,
                            TC_QUOTABLE,
                            DisplayKey.get("Web.AuditWizard.Summary.NullReceivedDate"),
                            DisplayKey.get("Web.AuditWizardMenu.Summary"))
      }
    }
  }
  ...
  function waiverOfSubrogation() {
    ...
    for (classCode in waiverAmountByClassCode.keySet()) {
      ...
      if (waiverAmount > basisAmount) {
        Result.addError(wcLine, TC_DEFAULT, 
            DisplayKey.get("Web.Policy.WC.Validation.WaivedAmountExceedsOverallBasis", classCode.Code, waiverAmount , basisAmount))
      }
    }
  ...
  function retrospectiveRating() {
    ...
    if (plan.FirstComputationDate.DayOfMonth != 1 
      ...
      Result.addError(wcLine, TC_DEFAULT, 
                      DisplayKey.get("Web.Policy.WC.Validation.RetroRatingPlanComputationDateBeginningOfMonth"))
    }
    ...
    if (plan.ComputationInterval < 1) {
       Result.addError(wcLine,TC_DEFAULT, DisplayKey.get("Web.Policy.WC.Validation.RetroRatingPlanComputationIntervalPositive"))
    } else if (nMonth % plan.ComputationInterval != 0) {
       Result.addError(wcLine, TC_DEFAULT, DisplayKey.get("Web.Policy.WC.Validation.RetroRatingPlanTimeSpan"))
    }
 
    if (plan.FirstComputationDate < policyPeriod.PeriodEnd) {
      Result.addError(wcLine, TC_DEFAULT,
                      DisplayKey.get("Web.Policy.WC.Validation.RetroRatingPlanComputationDateNotPriorExpiration", policyPeriod.PeriodEnd))
    }

    if (plan.LastComputationDate <=  plan.FirstComputationDate) {
      Result.addError(wcLine, TC_DEFAULT, DisplayKey.get("Web.Policy.WC.Validation.RetroRatingPlanComputationDateLastAfterFirst"))
    }

    for (letter in plan.LettersOfCredit) {
      if (letter.ValidTo <= letter.ValidFrom) {
        Result.addError(wcLine, TC_DEFAULT, DisplayKey.get("Web.Policy.WC.Validation.LetterOfCreditExpirationDate", letter.IssuerName))
      }

      if (letter.ValidFrom > plan.FirstComputationDate or
          letter.ValidTo < plan.LastComputationDate) {
        Result.addError(wcLine, TC_DEFAULT, DisplayKey.get("Web.Policy.WC.Validation.LetterOfCreditDoesNotCoverWholePeriod", letter.IssuerName))
      }
    }
  }
   ...
   var monopolisticStates = StateSet.get(StateSet.WC_MONOPOLISTIC)
    ...
    for (exposure in wcLine.WCCoveredEmployees) {
      if (monopolisticStates.contains(exposure.Location.State) 
            and (exposure.SpecialCov ==TC_STAT or exposure.SpecialCov ==TC_LTDM or exposure.SpecialCov ==TC_VOCO)) {
        Result.addError(wcLine, TC_DEFAULT, DisplayKey.get("Web.Policy.WC.Validation.ExposuresNotForMonopolisticStates"))
      }
    }
  
    for (exposure in wcLine.WCCoveredEmployees) {
      if (!monopolisticStates.contains(exposure.Location.State) and (exposure.SpecialCov ==TC_STOP)) {
        Result.addError(wcLine, TC_DEFAULT, DisplayKey.get("Web.Policy.WC.Validation.StopGapOnlyForMonopolisticStates"))
      }
    }
  }
  ...
  function atLeastOneFedLiabClass() {
    ...
    if (wcLine.WCFedEmpLiabCovExists and wcLine.WCFedCoveredEmployees.Count <= 0) {
      Result.addError(wcLine, TC_DEFAULT, 
        DisplayKey.get("Web.Policy.WC.Validation.AtLeastOneFedLiabClass"))
    }
  }
  ...
  function federalClassCodesAreValid() {
    ...
    if (wcLine.WCFedEmpLiabCov != null) {
      for (employee in wcLine.WCFedCoveredEmployees) {
        ...
        var domain = (typekey.WCClassCodeFederalDomains.get(wcLine.WCFedEmpLiabCov.FedEmpLiabActTerm.Value.Code)).Code
                
        if (!wcLine.doesClassCodeExist(classCode, state, domain, previousCode, classIndicator)) {
          Result.addError(wcLine, TC_DEFAULT, 
              DisplayKey.get("Web.Policy.WC.Validation.UnavailableClassCode", classCode),
              "WorkersCompOptions")
        }        
      }
    }
  ...
  function excludedStatesContainMonopolisticStates() {
    ...
    if (Context.isAtLeast(TC_DEFAULT) and cov.WCOtherStatesOptTerm.Value == TC_ALLEXCEPT) {
      var monopolisticStates = StateSet.get(StateSet.WC_MONOPOLISTIC)
      ...
      if (notExclStates.Count > 0) {
        Result.addError(wcLine, TC_DEFAULT, 
              DisplayKey.get("WorkersComp.InsuredStates.MustExcludeMonopolisticState", notExclStates.join(", ")))
      }
    }
  }  
    ...
    for (interStateID in interStateIDs) {
      ...
      if (anniversaries.Count > 1 and interStateID.OfficialIDValue != null) {
        ...
        if (emptyIntraStates.Count > 1) {
          Result.addError(wcLine, TC_DEFAULT, 
            DisplayKey.get("Web.Policy.WC.Validation.DifferentAnniversaryDates", emptyIntraStates.join(","), interStateID.OfficialIDType.Description, interStateID.OfficialIDValue))
        }
      }
      ...
      if (modifiers.Count > 1) {
        Result.addError(wcLine, TC_DEFAULT, 
          DisplayKey.get("Web.Policy.WC.Validation.DifferentExpModValues", jurisdictions*.State.join(","), interStateID.OfficialIDType.Description, interStateID.OfficialIDValue))
      }
    }
  }
  /**
  ...
  function noSplitJurisdictions() {
    for (jVL in wcLine.VersionList.Jurisdictions) {
      if (jVL.AllVersions.size() > 1) {
        Result.addInvariantError(wcLine, DisplayKey.get("Web.Policy.WC.Validation.SplitJurisdiction",  jVL.AllVersions.first().State ))
      } else {
        var jurisdiction = jVL.AllVersions.first()
        if (jurisdiction.EffectiveDate != jurisdiction.Branch.PeriodStart) {
          Result.addInvariantError(wcLine, DisplayKey.get("Web.Policy.WC.Validation.JurisdictionEffectiveDate",  jurisdiction, jurisdiction.EffectiveDate, jurisdiction.Branch.PeriodStart ))
        } else if (jurisdiction.ExpirationDate != jurisdiction.Branch.PeriodEnd) {
          Result.addInvariantError(wcLine, DisplayKey.get("Web.Policy.WC.Validation.JurisdictionExpirationDate",  jurisdiction, jurisdiction.ExpirationDate, jurisdiction.Branch.PeriodEnd ))
        }
      }
    }
  }

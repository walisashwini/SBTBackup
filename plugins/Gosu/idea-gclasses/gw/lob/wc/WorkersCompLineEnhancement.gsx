...
enhancement WorkersCompLineEnhancement : entity.WorkersCompLine {
  ...
  property get InterstateNamedInsuredOfficialIDs(): OfficialID[] {
    ...
    return this.Branch.getNamedInsuredOfficialIDsForState(null)
        .where(\ id -> id.OfficialIDType != TC_SSN and id.OfficialIDType != TC_FEIN and
  ...
  function getCoveredEmployeeInRatingPeriod(ratingPeriod: WCRatingPeriod) : WCCoveredEmployee[]{
    return this.WCCoveredEmployeeVLs.map(\versionList ->
  ...
  function getAvailableParticipatingPlanIDs() : WCParticipatingPlanID[] {
    ...
    var jurisdictions = this.Branch.LocationStates
  ...
  function removeJurisdiction(jurisdiction : WCJurisdiction) {
    if (not jurisdiction.CanRemove) {
      throw new DisplayableException(DisplayKey.get("WorkersComp.Jurisdiction.CannotRemove", jurisdiction))
    }
  ...
  function getWCCoveredEmployees(state : State) : WCCoveredEmployee[] {
    ...
    for (wcCoveredEmployee in wcCoveredEmployees) {
      if (wcCoveredEmployee.getLocation().State == state) {
  ...
  function getWCFedCoveredEmployees(state : State) : WCFedCoveredEmployee[] {
    ...
    for (wcFedCoveredEmployee in wcFedCoveredEmployees) {
      if (wcFedCoveredEmployee.getLocation().State == state) {
  ...
  function validateIncludedStates(includedStates : String): String {
    var coveredStates: Set<String> = this.Jurisdictions*.State*.Code?.toSet()
    ...
    if (incStates != null) {
      for (state in incStates.toArray()) {
        if (monopolisticStates.contains(state)) {
          return DisplayKey.get("WorkersComp.InsuredStates.CannotInsureMonopolisticState", state.Description)
        }
        if (coveredStates.contains(state.Code)) {
          return DisplayKey.get("WorkersComp.InsuredStates.CannotInsureCoveredState", state.Description)
        }
      }
    } else {
      return DisplayKey.get("WorkersComp.InsuredStates.ListRequired")
    }
    return null
  }

  function validateExcludedStatesContainMonopolisticStates(excludedStates : String) : String {
    if (this.WCOtherStatesInsurance.WCOtherStatesOptTerm.Value == TC_ALLEXCEPT) {
      var monopolisticStates = StateSet.get(StateSet.WC_MONOPOLISTIC)
      ...
      if (notExclStates.Count > 0) {
        return DisplayKey.get("WorkersComp.InsuredStates.MustExcludeMonopolisticState", notExclStates.join(", "))
      }
    }
    return null
  }
    ...
    if (inclStates != null) {
      for (state in inclStates.toArray()) {
        if (nonMonopolisticStates.contains(state)) {
          return DisplayKey.get("WorkersComp.InsuredStates.CannotIncludeNonMonopolisticState", state.Description)
        }
      }
    }
    return null
  ...
  function firstMatchingClassCodeOrThrow(code : String, refDateState : Jurisdiction, wcDomain: String, peviousCode : WCClassCode) : WCClassCode{
    ...
    if (retVal == null){
      throw new gw.api.util.DisplayableException(DisplayKey.get("Java.ClassCodePickerWidget.WCInvalidCode", code, refDateState))
    }
    return retVal
  }
  ...
  function addNewOwnerOfficerForContact(contact : Contact) : PolicyOwnerOfficer {
    ...
    var policyOwnerOfficer = this.Branch.addNewPolicyContactRoleForContact(contact, TC_POLICYOWNEROFFICER) as PolicyOwnerOfficer
  ...
  property get UnassignedOwnerOfficers() : AccountContact[] {
    ...
    var accountContactRoleType = plugin.getAccountContactRoleTypeFor(TC_POLICYOWNEROFFICER)
  ...
  property get PolicyLaborClientDetailExistingCandidates() : AccountContact[] {
    var plugin = Plugins.get(IContactConfigPlugin)
    var accountContactRoleType = plugin.getAccountContactRoleTypeFor(TC_POLICYLABORCLIENT)
  ...
  property get PolicyLaborContractorDetailExistingCandidates() : AccountContact[] {
    var plugin = Plugins.get(IContactConfigPlugin)
    var accountContactRoleType = plugin.getAccountContactRoleTypeFor(TC_POLICYLABORCONTRACTOR)
    return this.Branch.Policy.Account.getAccountContactsWithRole(accountContactRoleType)
  }
  ...
  function addNewLaborClientDetailForContact(contact : Contact) : WCLaborContactDetail {
    ...
    if (policyLaborClient == null) {
      policyLaborClient = this.Branch.addNewPolicyContactRoleForContact(contact, TC_POLICYLABORCLIENT) as PolicyLaborClient
  ...
  function addNewLaborContractorDetailForContact(contact : Contact) : WCLaborContactDetail {
    ...
    if (policyLaborContractor == null) {
      policyLaborContractor = this.Branch.addNewPolicyContactRoleForContact(contact, TC_POLICYLABORCONTRACTOR) as PolicyLaborContractor
  ...
  function setDefaultExcludedStates(cov : WCOtherStatesInsurance) {
    ...
    if (cov.WCOtherStatesOptTerm.Value == TC_ALLEXCEPT and excludedStates == null) {

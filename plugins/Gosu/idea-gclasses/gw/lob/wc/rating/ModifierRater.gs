...
class ModifierRater {
    ...
    if (mod != null) {
      ...
      if (stdRate != null) {  // if there is no rate, then we assume that no modification is necessary
        ...
        if (not convertedRate.IsZero // create cost no matter what
            or mod.PatternCode == ModifierPattern.ExpMod.PublicID // always show experience modifier
          ) {          
   ...
   */
  private static function getStdRate( modifier : WCModifier, factorName : String, ratingDate : Date, ratingState : Jurisdiction ) : BigDecimal
  {
    var rate : BigDecimal

...
class WCSysTableRatingEngine extends AbstractRatingEngine<WorkersCompLine> {
  ...
  // Worker's Comp is rated on a 365-day term
 override protected property get NumDaysInCoverageRatedTerm() : int {
  ...
  private function rateJurisdictionCosts()
  {
    ...
    getRatingSubtotal( TC_WC_STANDARD, all, null, null )
  ...
  function convertRateByUsage( rate : BigDecimal, usage : RateConversionType ) : BigDecimal
  {
    switch ( usage )
    {
      case TC_AS_IS       : return rate
      case TC_CREDIT      : return rate.negate()
      case TC_DIFF_FROM_1 : return rate.subtract( BigDecimal.ONE )
  ...
  private function processWCRatingStepsByPeriod( ratingPeriod : WCRatingPeriod, fromStep : int, toStep : int, atRatingPeriodLevel : boolean )
    ...
    if (Branch.Job typeis Audit && Branch.Audit.AuditInformation.AuditScheduleType == TC_PREMIUMREPORT) {
  ...
  {
    if ( Branch.RefundCalcMethod == TC_SHORTRATE )
    {
  ...
  private function calcBlendedPremiumDiscountRate( ratingPeriod : WCRatingPeriod, step : WCRatingStepExt ) : BigDecimal
  {
    if (Branch.Job typeis Audit && Branch.Audit.AuditInformation.IsPremiumReport) {
      ...
      var policyStandardPrem = getRatingSubtotal( TC_WC_STANDARD, all, null, null )
  ...
  {
    if ( Branch.RefundCalcMethod != TC_FLAT )
    {
      // Prorate the "minimum premium to meet" in all cases except for short-rate cancellations.
      var numDaysToProrate = Branch.RefundCalcMethod == TC_SHORTRATE ? Branch.NumDaysInPeriod : Branch.NumDaysInUncanceledPeriod

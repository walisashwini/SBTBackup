...
class RateRoutinePopupHelper {
  ...
  static function variableValidationExpression(step : CalcStepDefinition,
    ...
    if (availLocalVariables.contains(step.StoreLocation)) {
      return DisplayKey.get("Validation.Rating.RateRoutineDefinition.LocalVariable.ExistingName")
    } else {
  ...
  static private function checkComparedToNull(otherOperand : CalcStepDefinitionOperand) : String {
    ...
    if ((targetOpType != null) and constantComparison(otherOperand, targetOpType)) {
      return DisplayKey.get("Validation.Rating.CompareNullWithConstant")
    }
    return null
  }
  ...
  static function untypedConstantValidationExpression(otherOperand : CalcStepDefinitionOperand, op: CalcStepValueDelegate) : String {
    ...
    // TypeOfConstant would reject a malformed string with a generic message about an untyped constant.
    // Instead, if the constant value contains a double quote, call it String so we do string validation.
    var typeOfConstant = op.ConstantValue?.contains("\"") ? String : op.TypeOfConstant
  ...
  private static function stringConstantValidationExpression(valueCandidate: String) : String {
    if (valueCandidate.contains('"' as String)) {
      ...
      if (valueCandidate.startsWith("\"") and valueCandidate.endsWith("\"")) {
        ...
        if (matcher.matches()) {
          ...
          return DisplayKey.get("Validation.Rating.UnescapedQuotes")
        }
      } 
    }  
    return DisplayKey.get("Validation.Rating.UntypedConstantValue")
  }

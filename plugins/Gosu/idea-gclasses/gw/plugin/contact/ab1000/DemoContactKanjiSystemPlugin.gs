 ...
 */
@Export
class DemoContactKanjiSystemPlugin implements ContactSystemPlugin {
  public static var DEMOPERSON_FIRSTNAME : String = "Demo"
  public static var DEMOPERSON_LASTNAME : String = "External-Person"
  public static var DEMOPERSON_FIRSTNAME_KANJI : String = "豊田"
  public static var DEMOPERSON_LASTNAME_KANJI : String = "英二"
  public static var DEMOPERSON_PARTICLE : String = "du"
  public static var DEMOPERSON_NAME : String = DEMOPERSON_FIRSTNAME + " " + DEMOPERSON_LASTNAME
  public static var DEMOPERSON_NAME_KANJI : String = DEMOPERSON_FIRSTNAME_KANJI+DEMOPERSON_LASTNAME_KANJI
  public static var DEMOCOMPANY_NAME : String = "Demo External-Company Corp."
  public static var DEMOCOMPANY_NAME_KANJI : String = "トヨタ自動車株式会社"
  ...
  override function retrieveContact(contactUid: String, creator : ContactCreator) : Contact {
    ...
    if (mockContact == null) {
      throw new DisplayableException(DisplayKey.get("ContactAPI.CantFindContact", contactUid))
    }
    var contactType = _mapper.NameMapper.getLocalEntityName(mockContact.EntityType)
    var contact = creator.loadOrCreateContact(mockContact.LinkID, typekey.Contact.get(contactType))
    if (contact.AutoSync == AutoSync.TC_DISALLOW) {
      throw new IllegalStateException(DisplayKey.get("Web.Contact.Error.CannotRetrieveContactWhenAutoSyncDisallowed", contactUid))
    } else {
  ...
  override function searchContacts(criteria: ContactSearchCriteria) : ContactResult[] {
    var results = _sampleABContactAPISearchResult
      ...
      .map(\ abci -> new gw.plugin.contact.ab1000.ContactResultFromSearch(abci))
      .toTypedArray()
    return results
  }
  ...
  private function init() {
    var abContactAPISearchResult0 = new ABContactAPISearchResult() {
      ...
      ,:DateOfBirth = Coercions.makeDateFrom(Date.Today.addYears(-30).Calendar)
      ,:PrimaryPhone = PrimaryPhoneType.TC_HOME.Code
      ,:FirstNameKanji = DEMOPERSON_FIRSTNAME_KANJI
      ,:LastNameKanji = DEMOPERSON_LASTNAME_KANJI
      ,:Particle = DEMOPERSON_PARTICLE
    }
    ...
    var abContactAPISearchResult1 = new ABContactAPISearchResult() {
      ...
      ,:WorkPhone = DEMOCONTACT_WORKPHONE
      ,:HomePhone = DEMOCONTACT_HOMEPHONE
      ,:PrimaryPhone = PrimaryPhoneType.TC_WORK.Code
      ,:NameKanji = DEMOCOMPANY_NAME_KANJI
    }
  ...
  private function createSoapAddress(i : String) : XmlBackedInstance {
    ...
    var nameMapper = ContactIntegrationMapperFactory.ContactIntegrationMapper.NameMapper

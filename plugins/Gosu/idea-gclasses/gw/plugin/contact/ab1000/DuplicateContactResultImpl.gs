...
uses wsi.remote.gw.webservice.ab.ab1000.abcontactapi.types.complex.ABContactAPIFindDuplicatesResult
uses entity.Contact

@Export
class DuplicateContactResultImpl extends gw.plugin.contact.ab1000.AbstractContactResultExternal implements DuplicateContactResult {
  ...
  construct(rawResult : ABContactAPIFindDuplicatesResult) {
    ...
    _cellPhoneCountry = PhoneCountryCode.get(rawResult.CellPhoneCountry)
    ...
    _faxPhoneCountry = PhoneCountryCode.get(rawResult.FaxPhoneCountry)
    ...
    _homePhoneCountry = PhoneCountryCode.get(rawResult.HomePhoneCountry)
    ...
    _prefix = NamePrefix.get(rawResult.Prefix)
    _city = rawResult.PrimaryAddress.City
    _country = Country.get(rawResult.PrimaryAddress.Country)
    ...
    _state = State.get(rawResult.PrimaryAddress.State)
    _addressType = AddressType.get(rawResult.PrimaryAddress.AddressType)
    _valUntil = rawResult.PrimaryAddress.ValidUntil
    _phoneType = PrimaryPhoneType.get(rawResult.PrimaryPhone)
    _score = rawResult.Score
    _suffix = NameSuffix.get(rawResult.Suffix)
    _vendorType = VendorType.get(rawResult.VendorType)
    _workPhone = rawResult.WorkPhone
    _workPhoneCountry = PhoneCountryCode.get(rawResult.WorkPhoneCountry)

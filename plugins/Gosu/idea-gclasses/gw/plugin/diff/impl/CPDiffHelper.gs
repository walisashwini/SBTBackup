...
class CPDiffHelper extends DiffHelper<CommercialPropertyLine> {
  ...
  private function addBlanketDiffs(diffItems : List<DiffItem>) : List<DiffItem> {
    ...
    var basedOnCovs = Line1.CPLocations.arrays("Buildings").arrays("Coverages").toArray(new CPBuildingCov[0]).toList()
    ...
    //noinspection MultipleAsteriskDotOperators
    for (buildingCov in Line2.CPLocations.arrays("Buildings").arrays("Coverages").toArray(new CPBuildingCov[0])) {

 ...
 /**
  ...
  private function shouldKeep(diffItem : DiffItem) : boolean {
    ...
    if (DifferenceReason != typekey.DiffReason.TC_INTEGRATION) {
      ...
      if (TypesToFilterForDiffReasonsExceptIntegration.hasMatch(\ typeToFilter -> TypeUtil.isNominallyOrStructurallyAssignable(typeToFilter, typeof diffItem.Bean))) {
        return false
      ...
      if (DifferenceReason != typekey.DiffReason.TC_APPLYCHANGES) {
        if (TypesToFilterForDiffReasonsExceptApplyChanges.hasMatch(\ typeToFilter -> TypeUtil.isNominallyOrStructurallyAssignable(typeToFilter, typeof diffItem.Bean))) {
          return false
   ...
   */
  reified final function addSplittableDiffs<S extends EffDated, K>(diffItems : List<DiffItem>,
  ...
  final function filterRemovedThenReAddedItems(type : IEntityType, diffItems : List<DiffItem>) : List<DiffItem> {
    ...
    var matchingDiffTypes = diffItems.where( \ d -> TypeUtil.isNominallyOrStructurallyAssignable(type, typeof d.Bean))

...
class PolicyLineVerificationPluginImpl implements PolicyLineVerificationPlugin {
    ...
    for (lineCode in ProductModelUtils.getAllPolicyLines()) {
      if (not linesByName.contains(lineCode)) {
        issues.add(new ProductModelClassVerificationIssue(IssueLevel.ERROR,
            DisplayKey.get("ValidationError.ProductModel.PolicyLine.Configuration.MissingTypeKey", lineCode), null))
      }
    }
  }
  ...
  internal function verifyConfigurationForLine(line : InstalledPolicyLine, issues : Collection<ProductModelVerificationIssue>) {
    ...
    try {
      ...
      // handle the case when the class is not found below
    }
    if (foundClass == null) {
      issues.add(new ProductModelClassVerificationIssue(IssueLevel.ERROR,
          DisplayKey.get("ValidationError.ProductModel.PolicyLine.Configuration.ClassNotFound", className), className))
      return
    }
    if (not foundClass.Valid) {
      issues.add(new ProductModelClassVerificationIssue(IssueLevel.ERROR,
          DisplayKey.get("ValidationError.ProductModel.PolicyLine.Configuration.ClassInvalid", className), className))
      return
    }
    var constructor = foundClass.TypeInfo.getCallableConstructor({})
    if (constructor == null) {
      issues.add(new ProductModelClassVerificationIssue(IssueLevel.ERROR,
          DisplayKey.get("ValidationError.ProductModel.PolicyLine.Configuration.ConstructorNotFound", className), className))
    }
    if (not (gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(PolicyLineConfiguration.Type, foundClass))) {
      issues.add(new ProductModelClassVerificationIssue(IssueLevel.ERROR,
          DisplayKey.get("ValidationError.ProductModel.PolicyLine.Configuration.IncorrectSupertype", className), className))
    }
  }
}

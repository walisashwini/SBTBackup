...
class PolicyTermPlugin implements IPolicyTermPlugin {
  ...
  override function calculatePeriodEnd( effDate : Date, term : TermType, policyPeriod : PolicyPeriod ): Date {
    return calculatePeriodEnd(effDate, term, policyPeriod, false, shouldPerformDateReconciliation(policyPeriod))
  }

  /**
   * Given the effective date, the term for a PolicyPeriod, and the basedOn PolicyPeriod,
   * return the default expiration date.  This should always return a non-null date.
   *
   * @param effDate  the policy period effective or start date
   * @param term the policy period term
   * @param basedOnPolicyPeriod the basedOn policy period
   * @param shouldPerformDateReconciliation whether to perform date reconciliation
   *
   * Implementation Note: Called from Java.
   * Every TermType should be mentioned, the period expiration date must be set.
   * The time part of the returned expiration date will be ignored, in favor
   ...
   * @return The calculated period end date
   */
  override function calculatePeriodEndFromBasedOn(effDate : Date, term : TermType, basedOnPolicyPeriod : PolicyPeriod,
                                                  shouldPerformDateReconciliation : boolean): Date {
    return calculatePeriodEnd(effDate, term, basedOnPolicyPeriod, true, shouldPerformDateReconciliation)
  }

  private function calculatePeriodEnd(effDate : Date, term : TermType, policyPeriod : PolicyPeriod,
                                      isBasedOnPolicyPeriod : boolean, shouldPerformDateReconciliation : boolean): Date {
    if (effDate == null) {
      return null
    }
    var futureDate : Date
    switch (term) {
      ...
      case TermType.TC_OTHER:
        ...
        break
      default:
        throw new DisplayableException(DisplayKey.get("PolicyTermPlugin.CalculatePeriodEnd.MissingFormulaForTermType", term.Code))
    }
    ...
    if (shouldPerformDateReconciliation) {
      var basedOnPolicyPeriod = policyPeriod
      if(!isBasedOnPolicyPeriod){
        basedOnPolicyPeriod = policyPeriod?.BasedOn
      }
      futureDate = reconcileEndDate(effDate, futureDate, basedOnPolicyPeriod, term)
    }
    return futureDate
  }
  ...
  override function calculatePolicyTerm(policyPeriod : PolicyPeriod, periodStart: Date, periodEnd: Date): TermType {
    //If the TermType is known, just return the type
    if(policyPeriod.SelectedTermType != null) {
      return policyPeriod.SelectedTermType
    }
    //otherwise do a calculation based on dates, make it time-insensitive
   ...
   */
  private function reconcileEndDate(effDate : Date, futureDate : Date, basedOnpolicyPeriod : PolicyPeriod, term : TermType) : Date {
    if (TermType.TC_HALFYEAR == term) {
      return reconcileEndDateToPreviousTermStartDateIfPossible(effDate, futureDate, basedOnpolicyPeriod)
   ...
   **/
  private function reconcileEndDateToPreviousTermStartDateIfPossible(effDate : Date, futureDate : Date, basedOnPolicyPeriod : PolicyPeriod) : Date {
    if (basedOnPolicyPeriod != null                                    // need previous period
        and !basedOnPolicyPeriod.Archived
        and DateUtil.compareIgnoreTime(effDate, basedOnPolicyPeriod.PeriodEnd) == 0  // no gap in coverage
        and basedOnPolicyPeriod.TermType == TermType.TC_HALFYEAR) {    // previous term also 1/2 year
      if (isLastDayOfMonth(basedOnPolicyPeriod.PeriodStart)) {
        return PCDateUtil.setToEndOfMonth(futureDate)
      } else {
        var dayOfMonthIfTreatedAsAnnualTerm = DateUtil.getDayOfMonth(basedOnPolicyPeriod.PeriodStart)

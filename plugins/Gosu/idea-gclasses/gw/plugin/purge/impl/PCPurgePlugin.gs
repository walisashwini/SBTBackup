...
//    }
  ...
  override function prepareForPurge(context : PurgeContext) {
    ...
    } else if (extendedContext.PurgePolicyTerm) {
      extendedContext.PurgedPublicID = extendedContext.PolicyTerm.PublicID
  ...
  */
  private function checkForArchiveObjectsAttached(policyPeriod : PolicyPeriod) {
    if (policyPeriod.hasArchiveObjectsAttached()) {
      ArchivingLogger.warn("Preparing to purge PolicyPeriod graph (PublicID: ${policyPeriod.PublicID}) that has archived objects attached. This is not a supported action.")
    }
  }
  ...
  override function skipPolicyPeriodForPurge(purgeCandidate : PolicyPeriod) : boolean {
     ...
     * OOTB, we prevent purging any policy period that has contingencies attached to it.
     ...
     */
    return purgeCandidate.Contingencies.HasElements
  }

  override property get AllowedJobSubtypesForPruning() : List<typekey.Job> {
    /*
     ...
     */
    return {
      typekey.Job.TC_SUBMISSION,
      typekey.Job.TC_POLICYCHANGE,
      typekey.Job.TC_RENEWAL
    }
  }

  override property get AllowedJobSubtypesForPurging() : List<typekey.Job> {
    /*

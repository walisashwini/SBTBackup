...
class PCWorksheetExtractPlugin implements WorksheetExtractPlugin, InitializablePlugin {
  ...
  override function isExtracted(container: entity.WorksheetContainer): boolean {
   ...
   */
  override function buildWorksheetCandidatesQuery(query: IQueryBuilder <entity.WorksheetContainer>) {
    query.compare("CanPurge", Equals, false)
    query.join("Branch").join("Job").compare(Job#CloseDate, Relop.NotEquals, null)
  }

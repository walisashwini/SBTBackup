...
enhancement PolicyEnhancement : Policy {
  ...
  property get HasOutstandingContingencies() : boolean {
    return PendingContingencies.HasElements
  }

  property get PendingContingencies() : Contingency[] {
    return sortContingencies(this.Contingencies.where(\contingency -> contingency.Status == TC_PENDING))
  }

  property get ResolvedOrWaivedContingencies() : Contingency[] {
    return sortContingencies(this.Contingencies.where(\contingency -> contingency.Status == TC_RESOLVED or
        contingency.Status == TC_WAIVED))
  }

  property get FailedOrActionInitiatedContingencies() : Contingency[] {
    // OOTB we don't use the 'FAILED' status, but we still want to show them for legacy reasons.
    return sortContingencies(this.Contingencies.where(\contingency -> contingency.Status == TC_FAILED or
        contingency.Status == TC_ACTION_INITIATED))
  }

  private function sortContingencies(contingencies : Contingency[]) : Contingency[] {
    return contingencies.sortBy(\contingency -> contingency.DueDate)
  }

  property get ContingencyWarningMessages() : List<String> {
    var warnings : List<String> = {}
    if (HasOutstandingContingencies) {
      warnings.add(DisplayKey.get("Web.Contingency.Warning.OutstandingContingency"))
    }
    return warnings
  }
  ...
  function findLastCoveredPeriod() : PolicyPeriod {
    ...
    results.orderByDescending(QuerySelectColumns.path(Paths.make(PolicyPeriod#PeriodStart)))
  ...
  function addReferralReason(issueTypeCode : String, key : String,
    ...
    if (key == null) {
      throw new IllegalArgumentException(DisplayKey.get("Policy.AddReferralReason.MissingKey"))
    }
  ...
  private function findReferralIssueType(issueTypeCode : String) : UWIssueType {
    if (issueTypeCode == null) {
      throw new IllegalArgumentException(DisplayKey.get("Policy.AddReferralReason.MissingIssueTypeCode"))
    }
    var issueType = UWIssueType.finder.findUWIssueTypeByCode(issueTypeCode)
    if (issueType == null) {
      throw new IllegalArgumentException(DisplayKey.get("Policy.AddReferralReason.NonexistentIssueTypeCode", issueTypeCode))
    }
    if (issueType.CheckingSet != TC_REFERRAL) {
      throw new IllegalArgumentException(DisplayKey.get("Policy.AddReferralReason.InvalidIssueTypeCode", issueTypeCode))
    }
    return issueType
  }
  ...
  static function retrieveBoundOrLegacyRenewalPeriod(policyNumber : String, retrieveAsOfDate : Date) : PolicyPeriod {
    ...
    if (period == null) {
      ...
      if (period != null) {
        period = period.getSlice(getAsOfDate(period, retrieveAsOfDate))
      }
    }
    return period
  }

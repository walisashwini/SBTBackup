...
uses gw.job.AvailableUWCompanies
...
enhancement PolicyPeriodBaseEnhancement: PolicyPeriod {
  ...
  function autoSelectUWCompany() {
    ...
    if (availableUWCompanies.Empty) {
      //APD specific code where manual products are initialized with the default APD UW company if no other is available
      if (this.Policy.ProductCode == "Manual") {
        this.UWCompany = AvailableUWCompanies.defaultAPDUWCompany()
        return
      } else {
        this.UWCompany = null
        return

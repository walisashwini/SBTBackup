...
abstract class CostDataWithOverrideSupport<S extends entity.Cost, V extends PolicyLine> extends CostData<S, V> {
   ...
   */
  protected var _priorCost : ICostDataInitWrapper
   ...
   */
  protected var _defaultRoundingLevel : int as DefaultRoundingLevel

  /**
   * local cache of the rounding mode config that is on the policy line
   */
  protected var _defaultRoundingMode : RoundingMode as DefaultRoundingMode
  ...
  /**
   * Construct a new CostData spanning the given date range and cost code
   * <em>In order to use the override support on this CostData, you will need to call init(V)</em>
   * @param effDate The effective date for this CostData
   * @param expDate The expiration date for this CostData
   * @param costCode The cost code for this CostData
   */
  construct(effDate : Date, expDate : Date, costCode : CostCode) { //APD specific constructor that also uses CostCode
  ...
  /**
   * Construct a new CostData spanning the given date range with given currency, rate cache and cost code
   * @param effDate The effective date for this CostData
   * @param expDate The expiration date for this CostData
   * @param c The currency for this CostData
   * @param rateCache The policy period's cache of recorded foreign exchange rate conversions
   * @param costCode The cost code for this CostData
   */
  construct(effDate : Date, expDate : Date, c : Currency, rateCache : PolicyPeriodFXRateCache, costCode : CostCode) {  //APD specific constructor that also uses CostCode
  ...
  public function init(line : V) {
    ...
    init(createCostDataInitWrapper(getExistingCost(line)),
        line.Branch.Policy.Product.QuoteRoundingLevel,
        line.Branch.Policy.Product.QuoteRoundingMode)
  }

  public function init(costOverrideWrapper : ICostDataInitWrapper, defaultRoundingLevel : int, defaultRoundingMode : RoundingMode) {
    if (_initialized) {
      throw new IllegalStateException("Cannot call init() twice, or call it on a CostData build from an existing cost.")
    }
    ...
    if (costOverrideWrapper != null) {
      _priorCost = costOverrideWrapper
      ...
      // CostData
      copyOverridesFromCostOverrideWrapper(_priorCost)
    }
   ...
   */
   construct(c : S) {
      super(c)
      _priorCost = createCostDataInitWrapper(c)
      _overrideLevel = OverrideLevel.getLevelFromCostData(this)
  ...
  function setAdjRateAndHandleOverrides(calcAdjRate : BigDecimal,
    ...
    if (calcAdjRate == null) {
      throw new IllegalArgumentException("Cannot use function setAdjRateAndHandleOverrides to set AdjustedRate to null.")
    }
   ...
   * @param explicitStandardTermAmt Value for StandardTermAmount if BaseRate or AdjustedRate was overridden
  ...
  function scaleValue(value : BigDecimal, scale : int) : BigDecimal {
    if (value == null) {
      ...
      return value.setScale(scale, RoundingModeToUse)
    }
  }

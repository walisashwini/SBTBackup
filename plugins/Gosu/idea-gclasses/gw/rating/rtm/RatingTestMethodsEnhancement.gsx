...
enhancement RatingTestMethodsEnhancement : gw.rating.rtm.RatingTestMethods {
    ...
    for (entity in entityFilter) {
      ...
      switch (entity) {
        ...
        case "CalcRoutineDefinition" :
          ...
          break
        default:
          throw new IllegalArgumentException(entity + " is not an entity for which cleanup is supported")
      }
    }
  ...
  private reified function findTaggedEntity<T extends KeyableBean>(tag : String) : List<T>{
    if (tag == null) {
      return {}
    }
    var q = Query.make(T)
  ...
  static function getParam(columnName : String, physicalColumnName : String) : RateTableColumnBuilder {
  ...
  // CalcStepDefinitionOperandEnhancementTest
  // ALSO BY rtm tests, PARateTablesBuilder, SampleDataTablesBuilder
  static function getMatchOp(column : RateTableColumnBuilder) : RateTableMatchOpBuilder {
  ...
  static function getMatchOp(column : RateTableColumnBuilder, argumentSource : String, argSrcBldr : RateTableArgumentSourceBuilder) : RateTableMatchOpBuilder {
  ...
  static function getMatchOp(column : RateTableColumnBuilder, argSrcBldrs : List<RateTableArgumentSourceBuilder>) : RateTableMatchOpBuilder {
    // For a 1-column match, the param name is usually the same as the column name.
    var paramName = column.ColumnName.toUpperCase()
  ...
  static function getMatchOp(columns : RateTableColumnBuilder[]) : RateTableMatchOpBuilder {
  ...
  static function getMatchOp(columns : RateTableColumnBuilder[], paramName : String, argumentSource : String, argSrcBldr : RateTableArgumentSourceBuilder) : RateTableMatchOpBuilder {
    var matchOpBuilder = new RateTableMatchOpBuilder()
    columns.each(\ c -> matchOpBuilder.addParam(c))
  ...
  // USED EXCLUSIVELY BY PARateTablesBuilder, SampleDataTablesBuilder and internally
  static function getMatchOp(columns : RateTableColumnBuilder[], paramName : String, argSrcBldrs : List<RateTableArgumentSourceBuilder>) : RateTableMatchOpBuilder {
    var matchOpBuilder = new RateTableMatchOpBuilder()
    columns.each(\ c -> matchOpBuilder.addParam(c))
  ...
  static function makeStoreRateTableLookupStep (bundle : Bundle, order : int, variableName : String,
                                            tableCode : String,
                                            factorName : String,
  ...
  static function makeStoreRateTableLookupStepBuilder (order : int, variableName : String,
                                            tableCode : String,
                                            factorName : String, 
                                            paramConstantMap : Map<String,String>,
                                            argSources : List<CalcStepDefinitionArgument>
  ...
  static function makeTableLookupStep(bundle : Bundle, order : int, opType : CalcStepOperatorType,
                                           tableCode : String,
                                           targetFactor : String,
                                           paramConstantMap : Map<String,String>,
                                           argSources : List<CalcStepDefinitionArgument>) : CalcStepDefinition {
  ...
  static function makeTableLookupStepBuilder(order : int, opType : CalcStepOperatorType,
                                           tableCode : String,
                                           targetFactor : String,
                                           paramConstantMap : Map<String,String>,
  ...
  static public function createPAMultiSetTableDef(tableCode : String, multiSetParamSet : CalcRoutineParameterSet) : RateTableDefinitionBuilder {
    ...
    var multiSetArgSrcBuilderB2 = new RateTableArgumentSourceBuilder()
                                         .withRoot(TC_DRIVERASSIGNMENTINFO)
                                         .withArgumentSource("YoungestDriverAge")

    var ageParamBldr1 = getParam(1, "AGE", "int1", RateTableDataType.TC_INTEGER).withColumnLabel("AGE")
    var ageParamBldr2 = getParam(2, "AGE2", "int2", RateTableDataType.TC_INTEGER).withColumnLabel("AGE2")

    var multiSetMatchOpBldr1 = getMatchOp(ageParamBldr1,{multiSetArgSrcBuilderA, multiSetArgSrcBuilderB})
            .withMatchOpDefinition(SampleDataTablesBuilder.EXACT_MATCH_OP_DEF)
    var multiSetMatchOpBldr2 = getMatchOp(ageParamBldr2,{multiSetArgSrcBuilderB2})
            .withMatchOpDefinition(SampleDataTablesBuilder.EXACT_MATCH_OP_DEF)
    ...
    var argSrcSetB = new RateTableArgumentSourceSetBuilder()
                                      ...
                                      .addArgumentSource(multiSetArgSrcBuilderB2)
    ...
    return new RateTableDefinitionBuilder()
      ...
      .addMatchOperation(multiSetMatchOpBldr1)
      .addMatchOperation(multiSetMatchOpBldr2)
  }
  ...
  static function basicRateflowScenario(withData : boolean = true) : RatingScenario {
    // Base Rate Table
    ...
    if (withData) {
      ...
      rateBookBuilder.includesRateTable(baseTable)
    }
    ...
    if (withData) {
      ...
      rateBookBuilder.includesRateTable(boolParamTable)
    }
  ...
  static function testRangeTableBuilder(testRangeTableName : String): RateTableDefinitionBuilder {
    return
        ...
        }).withOpName("RangeOp").withMatchOpDefinition(RateTableMatchOpDefinitionBuilder.RangeMatchMaxExclOp))
            .addMatchOperation(getMatchOp({
            getParam(2, "ExactMatchOp", "str1", RateTableDataType.TC_STRING)
        }).withOpName("ExactMatchOp").withMatchOpDefinition(RateTableMatchOpDefinitionBuilder.ExactMatchOp))
  ...
  static function createTestTableWithSingleParamMatchOp(matchOpName: String, colName: String, physColName: String,
    ...
    var matchOpBuilder = getMatchOp(getParam(0, colName, physColName, rateTableDataType))
        ...
        .withMatchOpDefinition(rateTableMatchOpDefBuilder)
  ...
  static function createTestTableWithSingleMultiParamMatchOp(matchOpName: String, colName: String, physColName: String,
                                                             rateTableDataType: RateTableDataType,
                                                             rateTableMatchOpDefBuilder: RateTableMatchOpDefinitionBuilder) : RateTableDefinition {
    final var rateTableColumnBuilder = getParam(0, colName, physColName, rateTableDataType)
    rateTableColumnBuilder.withMultiSelect(true)
    var matchOpBuilder = getMatchOp(rateTableColumnBuilder)
        .withMatchOpName(matchOpName)
        .withMatchOpDefinition(rateTableMatchOpDefBuilder)

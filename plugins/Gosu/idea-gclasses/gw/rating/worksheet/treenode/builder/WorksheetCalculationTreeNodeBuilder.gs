...
class WorksheetCalculationTreeNodeBuilder extends WorksheetTreeNodeBuilder<WorksheetCalculation> {
  ...
  function populateRoundingNode(operand : WorksheetFunction, roundingNode : WorksheetTreeNodeLeaf) {
    ...
    var operatorValue = operand.WorksheetOperands.get(2).Value as String
    var roundingType = RoundingModeType.get(operatorValue)
    //fallback to stored value because of change to how rounding mode stored in worksheet blob
    roundingNode.Operator = roundingType == null ? operatorValue : getRoundingModeOperator(roundingType)
  }
  ...
  private function createFunctionExpansionTreeNodeContainer(functionOperand : WorksheetFunction, operandBuilder : StringBuilder) : WorksheetTreeNodeContainer {
    var functionExpansionLabel = DisplayKey.get("Web.Policy.RatingWorksheet.Node.FunctionExpansionLabel", operandBuilder.toString())
    var functionIndicator = DisplayKey.get("Web.Policy.RatingWorksheet.Node.FunctionExpansionIndicator")

        ...
        this.eachWithIndex(\ agreement2, j -> {
          ...
          if (j > i
            ...
            if (from == null or to == null) {
              errors.add(DisplayKey.get("Web.Reinsurance.Program.Validation.OverlappingCoverageError", agreement1.Name, agreement2.Name))
            } else {
              errors.add(DisplayKey.get("Web.Reinsurance.RIRisk.Validation.OverlappingCoverageError", agreement1.Name, agreement2.Name, DateFormat.getDateInstance().format(from)))
            }
          }
        })
    })

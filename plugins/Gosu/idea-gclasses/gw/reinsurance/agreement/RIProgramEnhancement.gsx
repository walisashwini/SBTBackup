...
enhancement RIProgramEnhancement : entity.RIProgram {
  ...
  function isAttachedToAnyRIRisk() : boolean {
    ...
    q.or(\ restriction -> {
      restriction.compare(RIRisk#PolicyAttachmentProgram, Relop.Equals, this.ID)
      restriction.compare(RIRisk#LossDateAttachmentProgram, Relop.Equals, this.ID)
    })
    return not q.select().Empty
  }

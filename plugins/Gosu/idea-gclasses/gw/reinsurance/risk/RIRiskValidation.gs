...
class RIRiskValidation extends PCValidationBase {
  ...
  private function checkAllowedAttachmentTypes() {
    ...
    if (_risk.TotalRisk == null) {
      ...
      if (invalidAttachments.HasElements) {
        Result.addError(_risk, TC_DEFAULT, DisplayKey.get("Web.Reinsurance.RIRisk.Validation.InvalidAttachmentForRiskWithNoTIV"))
      }
    }
  }
  ...
  private function validateCoverageRanges() {
    ...
    errors.each(\ e -> {
      Result.addError(_risk, TC_DEFAULT, e)
    })
  }
  ...
  private function checkFacultativeAgreements() {
    for(agreement in this._risk.PolicyAttachmentAgreements){
      ...
      for(error in errors){
        Result.addError(_risk, TC_DEFAULT, error)
      }
    }
  }
    ...
    for (attachment in _risk.Attachments) {
      ...
      if (agreement typeis Facultative) {
        if (agreement.AttachmentPoint > this._risk.TotalRisk) {
          ...
          Result.addWarning(_risk, TC_DEFAULT, DisplayKey.get("Web.Reinsurance.RIRisk.Validation.AgreementAttachmentAbovePML", agreement.Name, attachmentPoint, totalRisk, _risk.Reinsurable.DisplayName))
        }
      }
      if (agreement typeis PerRisk) {
        if (attachment.MaxCeding > attachment.CededRisk) {
          Result.addWarning(_risk, TC_DEFAULT, DisplayKey.get("Web.Reinsurance.RIRisk.Validation.AgreementDoesNotCedeToCapacity", agreement.Name, _risk.Reinsurable.DisplayName))
        }
      }
    }
  }
  ...
  private function checkMaxFacPropSumIsLessThanMaxPropRetention() {
    ...
    if (totalMaxPropShareCeded > propShare) {
        Result.addError(_risk, TC_DEFAULT, DisplayKey.get("Web.Reinsurance.RIRisk.Validation.FacPropExceedsMaxPropRetention", DateFormat.getDateInstance().format(_risk.EffectiveDate)))
    }
  }
    ...
    if (_risk.TotalInsuredValue == null) {
      _risk.Attachments.ProportionalFacAttachmentsForCalc.each(\ a -> {
        if ((a.Agreement.AmountOfCoverage != null and a.Agreement.AmountOfCoverage.IsPositive)
          ...
          Result.addError(_risk, TC_DEFAULT, DisplayKey.get("Web.Reinsurance.RIRisk.Validation.FacPropNullTIVRequiresCededPercentage", a.Agreement.Name))
        }
      })
    }
  }
  ...
  private function checkAllTreatiesAssociatedWithRiskDoesIncludeRICoverageGroup() {
    ...
    _risk.Attachments.Treaties.each(\ t -> {
      if (!t.Agreement.CoverageGroups.contains(coverageGroup)) {
        Result.addError(_risk, TC_DEFAULT, DisplayKey.get("Web.Reinsurance.RIRisk.Validation.TreatyAssociatedWithRiskDoesNotApplyToRICoverageGroup"))
      }
    })
  }

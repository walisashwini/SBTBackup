...
class SearchSamplePolicyData extends AbstractSampleDataCollection {
 ...
 private function changePNIAtMidTerm(testName : String, basedOn : PolicyPeriod) {
   ...
   var accountContact = basedOn.Policy.Account.AccountContacts.firstWhere(\ ac -> ac.Contact != basedOn.PNIContactDenorm
                                                                                  and not ac.Roles.hasMatch(\ r -> r.Subtype == TC_NAMEDINSURED))
   pChange.changePrimaryNamedInsuredTo(accountContact.Contact)
          ...
          runBlockAsUser(underwriter, "SearchSamplePolicyData -- ${underwriter}", \ -> {
            ...
            if (underwriter == "aapplegate") {
              if (i % 4 != 0) {
                ...
                loadSubmission(jobNum, account, PCCoercions.makeProductModel<Product>("PersonalAuto"), SampleDataConstants.getBaseDateMinus(0, 3, 0), namedInsureds, true, addIDs("solr:" + jobNum))
              } else {
                loadSubmission(jobNum, account, PCCoercions.makeProductModel<Product>("BusinessOwners"), SampleDataConstants.getBaseDateMinus(0, 9, 0), namedInsureds, true, addIDs("solr:" + jobNum))
              }
            } else {
              switch (i % 4) {
                case 0:
                case 1:
                  loadSubmission(jobNum, account, PCCoercions.makeProductModel<Product>("BusinessOwners"), SampleDataConstants.getBaseDateMinus(0, 4, 0), namedInsureds, true, addIDs("solr:" + jobNum))
                  break
                case 2:
                  loadSubmission(jobNum, account, PCCoercions.makeProductModel<Product>("BusinessAuto"), SampleDataConstants.getBaseDateMinus(0, 5, 0), namedInsureds, true, addIDs("solr:" + jobNum))
                  break
                case 3:
                  loadSubmission(jobNum, account, PCCoercions.makeProductModel<Product>("WorkersComp"), SampleDataConstants.getBaseDateMinus(0, 7, 0), namedInsureds, true, addIDs("solr:" + jobNum))

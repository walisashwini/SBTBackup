...
@Export
class SolrPolicyDeleteRequest extends AbstractDeleteRequest implements ISolrPolicyDeleteRequest {
  static final var _store = SolrSearchStore.ACTIVE
  static final var _logger : org.slf4j.Logger = PCSolrUtils.Logger

  override function processContact(contact : Contact, solrContext : SolrIndexContext){
  ...
  override function processPeriod(period : PolicyPeriod, solrContext : SolrIndexContext, isPurged : boolean= false){
  ...
  private function addDeleteQuery(period : PolicyPeriod, solrContext : SolrIndexContext, isPurged : boolean){
    ...
    if(isPurged) {
      ...
      if (period.Preempted or period.Job typeis Submission or period.Job typeis Rewrite
          or PolicyPeriodStatus.TF_INELIGIBLEFORSOLRINDEX.TypeKeys.contains(period.Status)) {

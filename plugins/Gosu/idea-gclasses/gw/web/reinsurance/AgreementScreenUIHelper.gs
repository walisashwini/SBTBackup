...
class AgreementScreenUIHelper {
  ...
  function delete() {
    if (_agreement typeis Treaty and _agreement.isAttachedToAnyProgram()) {
      throw new DisplayableException(DisplayKey.get("Web.Reinsurance.Treaty.DeleteError"))
    } else if (_agreement typeis Facultative and _agreement.getAttachments(gw.transaction.Transaction.getCurrent()).HasElements) {
      throw new DisplayableException(DisplayKey.get("Web.Reinsurance.FacAgreement.DeleteError"))
    } else {
  ...
  function validateAgreementNumberIsUnique() : String {
    ...
    if (query.select().Empty) {
      ...
      return DisplayKey.get("Web.Reinsurance.Agreement.Verify.DuplicateAgreementNumber", _agreement.AgreementNumber)
    }
  }
}

...
uses gw.webservice.pc.pc1000.ccintegration.entities.anonymous.elements.CCCoverage_CovTerms
uses gw.webservice.pc.pc1000.ccintegration.entities.anonymous.elements.CCPolicy_Coverages
uses gw.webservice.pc.pc1000.ccintegration.entities.anonymous.elements.CCVehicle_Lienholders
...
uses gw.webservice.pc.pc1000.ccintegration.entities.types.complex.CCNumericCovTerm
uses gw.webservice.pc.pc1000.ccintegration.entities.types.complex.CCPolicy
uses gw.webservice.pc.pc1000.ccintegration.entities.types.complex.CCPolicyCoverage
...
uses gw.webservice.pc.pc1000.ccintegration.entities.types.complex.CCVehicle
uses gw.webservice.pc.pc1000.ccintegration.entities.types.complex.CCVehicleOwner
...
class CCBasePolicyLineMapper {
  ...
  protected var _policyGen : gw.webservice.pc.pc1000.ccintegration.CCPolicyGenerator;
  protected var _contactGen : gw.webservice.pc.pc1000.ccintegration.CCContactGenerator;
  ...
  protected function getLineCoverages() : List<Coverage> {
  ...
  protected function populateCoverage(ccCov : CCCoverage, pcCov : Coverage) {
    ccCov.Currency = pcCov.Currency.Code
    ...
    for(covTerm in pcCov.CovTerms.sortBy(\ term -> term.Pattern.Priority) ) {
      if (!isCovTermExcluded(covTerm)) {
        ...
        for(ccCovTerm in ccCovTerms) {
          processCovTerm(ccCov, covTerm, ccCovTerm);
  ...
  protected function processCovTerm(ccCov : CCCoverage, covTerm : CovTerm, ccCovTerm : CCCovTerm) {
    ...
    if (ccCovTerm typeis CCFinancialCovTerm) {
      ...
      // The deductible field on coverage in ClaimCenter is deprecated.
      // If you are still using this field in ClaimCenter and need it to be retrieved during policy retrieval/refresh,
      // then uncomment this section.
      // if( covTerm.ModelType == CovTermModelType.TC_DEDUCTIBLE )  {
        // ccCov.Deductible = ccCovTerm.FinancialAmount
      // }
      if( covTerm.ModelType == CovTermModelType.TC_LIMIT and ccCovTerm.ModelAggregation != null )  {
  ...
  protected function getCCCovTerms( covTerm : CovTerm) : CCCovTerm[] {
    ...
    if ( covTerm typeis OptionCovTerm )  {
      if (isMoneyTerm(covTerm)) {
        ...
        if (covTerm.Value != null && covTerm.Value != 0) {
          ccOptCovTerm.FinancialAmount = covTerm.Value?.ofCurrency(covTerm.Clause.Currency)
        }
      ...
      for( packageTerm in covTerm.PackageValue.PackageTerms ) {
        if (isMoneyTerm(packageTerm)) {
          var ccPackCovTerm = new CCFinancialCovTerm()
          setBasicCovTermFields(ccPackCovTerm, covTerm)
          ccPackCovTerm.PolicySystemID = getCovTermTypeIDString(covTerm) + "." + packageTerm.PublicID
          ...
          if (packageTerm.Value != null && packageTerm.Value != 0) {
            ccPackCovTerm.FinancialAmount = packageTerm.Value?.ofCurrency(covTerm.Clause.Currency)
          }
      ...
      if (isMoneyTerm(covTerm)) {
        ...
        if (covTerm.Value != null && covTerm.Value != 0) {
          ccDirCovTerm.FinancialAmount = covTerm.Value?.ofCurrency(covTerm.Clause.Currency)
        }
    ...
    return ccCovTerms?.toTypedArray()
  }

  protected function setBasicCovTermFields(ccTerm : CCCovTerm, pcTerm : CovTerm) {
    ccTerm.PolicySystemID = getCovTermTypeIDString(pcTerm)
    ccTerm.CovTermOrder = pcTerm.Pattern.Priority
    ccTerm.CovTermPattern = CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(pcTerm.Pattern)
  }
  ...
  protected function mapToCCCoverageCode(cov : Coverage) : String {
    return CCProductModelCodeMappingUtil.getCCTypeCodeForPMType(cov.Pattern);
  }

  protected function addCustomFinancialCovTerm(ccCov : CCCoverage, pcCov : Coverage, patternCode : String, order : Integer,
                                             amount : MonetaryAmount) {
    var covTerm = new CCFinancialCovTerm();
    ccCov.CovTerms.add(new CCCoverage_CovTerms(covTerm));
    covTerm.PolicySystemID = pcCov.TypeIDString + "." + patternCode
    covTerm.CovTermPattern = patternCode;
    covTerm.CovTermOrder = order;
    covTerm.FinancialAmount = amount
  }
  ...
  protected function isMoneyTerm(packageTerm : PackageTerm) : boolean {
    return packageTerm.ValueType == CovTermModelVal.TC_MONEY
  }
  ...
  protected function mapCoinsurance(pcStringValue : String) : BigDecimal {
    ...
    if(StringUtil.isEmpty(pcStringValue)) {
      return null
    }
  ...
  protected function addVehicleOwner(ccVehicle : CCVehicle, pcContact : Contact) {
    ...
    ccVehicle.Lienholders.add(new CCVehicle_Lienholders(ccAddInt));
    ccAddInt.Lienholder = _contactGen.getOrCreateContact(pcContact);
  ...
  protected function addLocationLevelAdditionalInterest(id : Key, ccLocation : CCPolicyLocation, pcContact : Contact) {
    var ccPropContact = new CCPropertyOwner();
    var el = _policyGen.addPropertyOwner(id, ccPropContact)
    ccPropContact.Lienholder = _contactGen.getOrCreateContact(pcContact);
    ccPropContact.OwnerType = "partial_owner";  // Value in CC typelist
    ccLocation.Lienholders.add(el)
  }

  // Creates a new CCPropertyOwner for a given risk unit (must be a LocationBasedRiskUnit), using a PC contact
  protected function addRULevelAdditionalInterest(id : Key, ccRiskUnit : CCLocationBasedRU, pcContact : Contact) {
    var ccContact = _contactGen.getOrCreateContact(pcContact);
    var ccPropContact = new CCPropertyOwner();
    var el = _policyGen.addPropertyOwner(id, ccPropContact)
    ccPropContact.Lienholder = ccContact;
    ccPropContact.OwnerType = "partial_owner";  // Value in CC typelist
    ccRiskUnit.Lienholders.add(el);
    ...
    (ccRiskUnit.PolicyLocation.$TypeInstance as CCPolicyLocation).Lienholders.add(el);
  }

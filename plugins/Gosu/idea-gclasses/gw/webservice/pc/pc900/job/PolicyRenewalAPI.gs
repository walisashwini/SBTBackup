 ...
 */
@WsiWebService("http://guidewire.com/pc/ws/gw/webservice/pc/pc900/job/PolicyRenewalAPI")
@Export
...
class PolicyRenewalAPI {
  ...
  function confirmTerm(policyNumber: String, termNumber: int, transactionId: String) {
    ...
    SOAPUtil.tryCatch<String>(\bundle -> {
      ...
      if (query.Empty) {
        throw new BadIdentifierException(DisplayKey.get("PolicyRenewalAPI.ConfirmTerm.Error.TermNotFound", policyNumber, termNumber))
      }
      var period = query.FirstResult
      if (period.PolicyTerm.CheckArchived) {
        throw new BadIdentifierException(DisplayKey.get("PolicyRenewalAPI.Error.TermArchived", period.PolicyTerm))
      }
      if (query.Count == 1 and period.Status != PolicyPeriodStatus.TC_BOUND){
        throw new BadIdentifierException(DisplayKey.get("PolicyRenewalAPI.ConfirmTerm.Error.TermNotBound", period.PolicyTerm))
      }
  ...
  function notifyPaymentReceivedForRenewalOffer(offerID: String,
    ...
    return SOAPUtil.tryCatch<String>(\bundle -> {
      ...
      if (job == null or not (job typeis Renewal)) {
        throw new BadIdentifierException(DisplayKey.get("PolicyRenewalAPI.NotifyPayment.Error.RenewalNotFound", offerID))
      }
      ...
      if (not canReceivePayment(period)) {
        var description = DisplayKey.get("RenewalAPI.RenewalOffer.OfferNotReady", offerID)
      ...
      if (paymentPlan == null) {
        var description = selectedPaymentPlanCode == null
            ? DisplayKey.get("RenewalAPI.RenewalOffer.InvalidPlan",paymentAmount)
            : DisplayKey.get("RenewalAPI.RenewalOffer.InvalidPayment", selectedPaymentPlanCode, paymentAmount)
        createActivity(job, description)
        return null
      }
      try {
  ...
  function startRenewals(policyNumbers: String[]) {
    ...
    for (policyNumber in policyNumbers) {
      ...
      if (period == null) {
        throw new BadIdentifierException(DisplayKey.get("PolicyRenewalAPI.StartRenewals.Error.InvalidPolicyNumber", policyNumber))
      }
          ...
          renewal = account.createConversionRenewal(Date.CurrentDate, product, producerCode,
              \period -> {
                if (policyPeriodData != null and policyPeriodData.NotBlank){
                  var model = gw.webservice.pc.pc900.gxmodel.policyperiodmodel.PolicyPeriod.parse(policyPeriodData)
  ...
  private function getApplicablePaymentPlan(period: PolicyPeriod,
    ...
    if (selectedPaymentPlanCode == null && period.SelectedPaymentPlan != null) {
      if (period.SelectedPaymentPlan.DownPayment <= paymentAmount) {
        return paymentPlans.getByBillingId(period.SelectedPaymentPlan.BillingId)
      }
  ...
  private function startRenewal(accountNumber: String,
    ...
    var product = gw.api.productmodel.ProductLookup.getByPublicID(productCode)
    if (product == null){
      throw new BadIdentifierException(DisplayKey.get("PolicyRenewalAPI.Error.ProductNotFound", productCode))
    }
    var producerCode = gw.api.database.Query.make(ProducerCode).compare(ProducerCode#PublicID, Equals, producerCodeId).select().AtMostOneRow
  ...
  private function parseDateFromString(dateString: String, parameterName: String): Date {
    try {
      ...
      PCLoggerCategory.API.error(pe.getMessage(), pe)

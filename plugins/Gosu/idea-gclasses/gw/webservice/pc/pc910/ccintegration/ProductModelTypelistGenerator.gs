  ...
  private function genCCProductModelXML() : XMLNode {
    ...
    for (lob in LOBProductMap.Keys) {
      for (cov in lob.CoverageCategories.flatMap(\ cat -> cat.CoveragePatterns) ) {
        ...
        var CovTypeNode = createTypeCodeNode(CoverageTypeRoot, callTypecodeMappingAndLogWarningMessages(cov), cov.CodeIdentifier,  cov.Name, cov.Description)
        ...
        if (subtypeNodes == null) {
          ...
          var subCovNode = createTypeCodeNode(CoverageSubtypeRoot, callTypecodeMappingAndLogWarningMessages(cov), cov.CodeIdentifier, cov.Name, cov.Description)

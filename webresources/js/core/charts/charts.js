...
(function (gw) {
    ...
    gw.api.registerGlobalSystem(charts);
    ...
    function calculateTextSize(svg, string, className, enclosingElement) {
        ...
        if (enclosingElement) {
            ...
            // Hardening against crash where textRoot here is null
            if (textRoot) {
        ...
        // Hardening against crash where textRoot was null
        if (!text) {
            textRoot = text = svg.append("text");
        }

        // Something has gone very wrong, just need to bail and hope for the best
        if (!text) {
            return {};
        }
    ...
    charts.Chart = function(id, chartSpec, yAxisMinMaxRange, xScaleFactory, yScaleFactory, legendSeries, offset) {
        ...
        if (!isRoundTypeChart(chartSpec)) {
            possiblyChangeLabelOrientationToFitLabels(chartSpec, this, rect);
        }
    ...
    var isRoundTypeChart = function(chartSpec) {
        return chartSpec.type === 'Pie' || chartSpec.type === 'Donut';
    };

    var computeWidthAndHeight = function(chart, chartSpec, rect) {
        var isDonutOrPie = isRoundTypeChart(chartSpec);
        chart.spacing = isDonutOrPie
        ...
        var totalStrokeOffset = isDonutOrPie ? (2 * gw.globals.gwCharts.STROKE_WIDTH) : 0;
        chart.width = rect.width - chart.spacing.leftMargin - chart.spacing.rightMargin - totalStrokeOffset;
        chart.height = rect.height - chart.spacing.topMargin - chart.spacing.bottomMargin - totalStrokeOffset;
    ...
    var possiblyChangeLabelOrientationToFitLabels = function(chartSpec, chart, rect) {
        // If the orientation is set explicitly to something other than Horizontal, then don't mess with it
        if (chartSpec.xaxis.labelOrientation && chartSpec.xaxis.labelOrientation !== "Horizontal") {
            return;
        }
        ...
        var dynamicLabelOrientation = labelsFitAxis && lastLabelFits && firstLabelFits ? "Horizontal" : "Up45";
        ...
        if (dynamicLabelOrientation !== chartSpec.xaxis.labelOrientation ) {
            ...
            computeWidthAndHeight(chart, chartSpec, rect);
            // also need to recalculate dual axis spec for the same reason
            chartSpec.dualAxisSpec = getDualAxisSpec(chart, chartSpec);
        }
